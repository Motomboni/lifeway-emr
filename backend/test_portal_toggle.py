...
def test_toggle_portal():
    ...
    admin = User.objects.create_user(
        username="toggle_test_admin", password="pass", role="ADMIN"
    )
    ...
    print(
        f"  Initial: portal_enabled={patient.portal_enabled}, user.is_active={portal_user.is_active}"
    )
    ...
    request = factory.post(
        f"/api/v1/patients/{patient.id}/toggle-portal/",
        {"enabled": False},
        format="json",
    )
    ...
    receptionist = User.objects.create_user(
        username="toggle_test_receptionist", password="pass", role="RECEPTIONIST"
    )
    ...
    print("  Receptionist access: DENIED")
