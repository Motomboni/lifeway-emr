...
urlpatterns = [
    ...
    # Health check endpoints (basic liveness is public)
    path("", include("core.health_urls")),
]

if settings.API_DOCS_ENABLED:
    urlpatterns.append(path("", include("core.api_docs_urls")))
    ...
    path(
        ...
        include(
            [
                ...
                # Organizations (single-clinic Lifeway)
                ...
                path("pharmacy/", include("apps.pharmacy.nafdac_urls")),
                ...
                path("radiology/", include("apps.radiology.global_urls")),
                ...
                path(
                    "billing/hmo-providers/", include("apps.billing.hmo_provider_urls")
                ),
                # Insurance Provider management (for patient registration)
                path(
                    "billing/insurance-providers/",
                    include("apps.billing.insurance_provider_urls"),
                ),
                # Paystack webhook (public endpoint, signature-verified)
                path(
                    "billing/paystack/webhook/",
                    include("apps.billing.paystack_webhook_urls"),
                ),
                # Insurance claim management (Receptionist only)
                path(
                    "billing/insurance/", include("apps.billing.insurance_claim_urls")
                ),
                ...
                path("billing/", include("apps.billing.nhia_tariff_urls")),
                path("billing/", include("apps.billing.claim_pack_urls")),
                ...
                path("billing/", include("apps.billing.payment_reconciliation_urls")),
                ...
                # Interactive Guide (onboarding, workflows, help)
