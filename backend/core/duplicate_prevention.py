...
def check_patient_duplicate(
    ...
    if national_id:
        duplicate = (
            Patient.objects.filter(national_id=national_id, is_active=True)
    ...
    if phone:
        duplicate = (
            Patient.objects.filter(phone=phone, is_active=True)
            .exclude(pk=exclude_id)
            .first()
        )
        if duplicate:
    ...
    if email:
        duplicate = (
            Patient.objects.filter(email=email, is_active=True)
            .exclude(pk=exclude_id)
            .first()
        )
        if duplicate:
...
def check_lab_order_duplicate(visit, test_code, window_minutes=5, exclude_id=None):
    ...
    recent_orders = (
        LabOrder.objects.filter(visit=visit)
        .exclude(pk=exclude_id)
...
def check_radiology_order_duplicate(
    visit, study_code, window_minutes=5, exclude_id=None
):
    """
    ...
    recent_order = (
        RadiologyOrder.objects.filter(visit=visit, study_code=study_code)
        .exclude(pk=exclude_id)
        .order_by("-created_at")
        .first()
    )
    ...
    duplicate = (
        ...
        .exclude(pk=exclude_id)
        .exclude(pk=recent_order.pk)
        .first()
    )

    if duplicate or (timezone.now() - recent_order.created_at).total_seconds() < (
        window_minutes * 60
    ):
        raise ValidationError(
...
def check_payment_duplicate(
    visit, amount, payment_method, window_minutes=2, exclude_id=None
):
    """
    ...
    recent_payment = (
        Payment.objects.filter(
            visit=visit, amount=amount, payment_method=payment_method
        )
        .exclude(pk=exclude_id)
        .order_by("-created_at")
        .first()
    ...
    duplicate = (
        ...
        .exclude(pk=exclude_id)
        .exclude(pk=recent_payment.pk)
        .first()
    )

    if duplicate or (timezone.now() - recent_payment.created_at).total_seconds() < (
        window_minutes * 60
    ):
        raise ValidationError(
...
def check_vital_signs_duplicate(visit, window_minutes=3, exclude_id=None):
    ...
    recent_vitals = (
        VitalSigns.objects.filter(visit=visit)
        .exclude(pk=exclude_id)
    ...
    recent_count = (
        VitalSigns.objects.filter(visit=visit, recorded_at__gte=window_start)
        .exclude(pk=exclude_id)
        .count()
    )
...
def check_appointment_duplicate(
    patient, appointment_date, appointment_time=None, exclude_id=None
):
    """
