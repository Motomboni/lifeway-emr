"""
Organization scoping utilities for single-clinic Lifeway deployment.

Used across viewsets to filter data by clinic organization when required.
"""
...
def get_request_organization(request):
    ...
    # Final fallback: default org membership (single-clinic Lifeway when header missing/invalid)
    if getattr(request, "user", None) and request.user.is_authenticated:
        from core.middleware.organization_middleware import get_organization
...
def try_get_org_scoped_visit(request, visit_id):
    ...
    # Single-clinic / dev: align with filter_by_organization (unscoped when no tenant)
    if not getattr(settings, "REQUIRE_ORGANIZATION_CONTEXT", False):
        return Visit.objects.filter(pk=visit_id).first()
    return None
