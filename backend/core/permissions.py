...
class IsVisitAccessible(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if not visit:
            ...
            if visit_id:
                from core.tenant import try_get_org_scoped_visit
...
class IsPaymentCleared(permissions.BasePermission):
    """
    Permission class for clinical actions under strict Lifeway payment rules.

    Only registration must be paid before clinical access; post-consultation services
    (lab, radiology, pharmacy) do not require full visit payment clearance.
    """

    def has_permission(self, request, view):
        """Check if registration payment allows clinical access."""
        from rest_framework.exceptions import PermissionDenied
        ...
        from apps.billing.payment_gates_service import is_clinical_access_allowed

        if not is_clinical_access_allowed(visit):
            raise PermissionDenied(
                detail="Registration payment is required before this clinical action. "
...
class CanManageOfflineQueue(permissions.BasePermission):
    """Receptionist, nurse, admin, or staff can manage the offline clinic queue."""
    ...
    def has_permission(self, request, view):
        ...
        if getattr(request.user, "is_staff", False):
            return True
        user_role = getattr(request.user, "role", None) or getattr(
            request.user, "get_role", lambda: None
        )()
        return user_role in ["RECEPTIONIST", "NURSE", "ADMIN"]
...
class CanCreateVisit(permissions.BasePermission):
    """
    Permission to create visits.

    Allows: Receptionist, Admin (and superusers).
    """
...
class IsPlatformAdmin(permissions.BasePermission):
    """
    Platform or clinic administrator (not Django admin panel only).
    Matches frontend isAdmin (role ADMIN) and superusers.
    """
...
def with_tenant_permissions(*permission_classes):
    """
    Build permission instances for viewsets that override get_permissions().

    DRF replaces (not merges) DEFAULT_PERMISSION_CLASSES when get_permissions is
    defined; use this to keep IsAuthenticated + RequiresOrganization when org context is required.
    """
    from django.conf import settings

    stack = [permissions.IsAuthenticated]
    if getattr(settings, "REQUIRE_ORGANIZATION_CONTEXT", False):
        stack.append(RequiresOrganization)
    stack.extend(permission_classes)
    return [p() for p in stack]


class RequiresOrganization(permissions.BasePermission):
    """
    When enabled: authenticated requests must resolve a clinic organization.
    Superusers and explicitly exempt views/paths are allowed through.
    """

    message = (
        "Organization context required. Set X-Organization-Id header or choose a default organization."
    )
    ...
    def has_permission(self, request, view):
        ...
        if getattr(request.user, "is_superuser", False):
            return True
        if getattr(view, "allow_no_organization", False):
            return True
        path = getattr(request, "path", "") or ""
        if any(path.startswith(prefix) for prefix in self.EXEMPT_PATH_PREFIXES):
            return True
        return get_request_organization(request) is not None
