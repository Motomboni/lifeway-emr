...
class PaymentClearedGuard:
    ...
    def __call__(self, request):
        ...
        if visit and getattr(visit, "payment_status", None) == "INSURANCE_PENDING":
            try:
                ...
                if VisitInsurance.objects.filter(
                    ...
                    visit.__class__.objects.filter(pk=visit.pk).update(
        ...
        # Lifeway strict rules: registration paid unlocks clinical modules even when
        # visit.payment_status is still UNPAID (post-consultation charges pending).
        if visit and self._is_clinical_action(path):
            try:
                from apps.billing.payment_gates_service import is_clinical_access_allowed

                if is_clinical_access_allowed(visit):
                    if self._is_receptionist(request):
                        return JsonResponse(
                            {
                                "detail": "Receptionists cannot perform clinical actions. "
                                "Clinical actions must be performed by authorized clinical staff."
                            },
                            status=403,
                        )
        ...
        if visit and not visit.is_payment_cleared():
            ...
            if self._is_receptionist(request):
                ...
                if self._is_visit_retrieve_or_list(path) or self._is_payment_endpoint(
                    path
                ):
                    return self.get_response(request)
