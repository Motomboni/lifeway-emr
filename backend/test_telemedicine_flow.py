...
from apps.telemedicine.utils import create_twilio_room, generate_twilio_access_token
...
class TelemedicineFlowTester:
    ...
    def generate_access_tokens(self):
        ...
        try:
            ...
            doctor_token = generate_twilio_access_token(
                self.doctor, self.room_info["room_name"]
            )
    ...
    def simulate_session_lifecycle(self):
        ...
        try:
            ...
            print_detail("Status", self.session.status)
            print_detail(
                "Start Time", self.session.actual_start.strftime("%Y-%m-%d %H:%M:%S")
            )
            ...
            print_detail("Status", self.session.status)
            print_detail(
                "End Time", self.session.actual_end.strftime("%Y-%m-%d %H:%M:%S")
            )
    ...
    def test_transcription(self):
        ...
        from apps.telemedicine.transcription import is_transcription_configured, run_transcription

        if not is_transcription_configured():
            print_info("Transcription not configured (set TRANSCRIPTION_PROVIDER and keys/model)")
        ...
        try:
            ...
            print_info(
                "Attempting transcription (will likely fail without real recording)..."
            )
            ...
            if result:
                ...
                print_info(
                    "Transcription set to PENDING (expected without real recording)"
                )
                print_detail("Status", self.session.transcription_status)
    ...
    def display_summary(self):
        ...
        print_detail(
            "Session Detail", f"GET {base_url}/telemedicine/{self.session.id}/"
        )
        print_detail(
            "Generate Token",
            f"POST {base_url}/telemedicine/{self.session.id}/generate-token/",
        )
        print_detail(
            "Start Session", f"POST {base_url}/telemedicine/{self.session.id}/start/"
        )
        print_detail(
            "End Session", f"POST {base_url}/telemedicine/{self.session.id}/end/"
        )
