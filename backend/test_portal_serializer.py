...
def test_basic_patient_creation():
    ...
    try:
        ...
        assert (
            "temporary_password" not in response_data
            or response_data.get("temporary_password") is None
        )
...
def test_portal_account_creation():
    ...
    try:
        ...
        assert portal_user.password.startswith(
            "pbkdf2_sha256$"
        ) or portal_user.password.startswith("bcrypt")
...
def test_validation_invalid_email():
    ...
    try:
        ...
        error_message = str(serializer.errors)
        if "email" in error_message.lower() and (
            "invalid" in error_message.lower() or "format" in error_message.lower()
        ):
...
def test_atomic_transaction():
    ...
    try:
        ...
        print_info(
            f"Before: {patient_count_before} patients, {user_count_before} users"
        )
        ...
        if patient_count_after == patient_count_before:
            ...
            print_error(
                f"Patient count changed: {patient_count_before} -> {patient_count_after}"
            )
            return False
