...
def test_prepare_message():
    ...
    try:
        ...
        if len(messages["sms_body"]) <= 160:
            print_success(
                f"SMS body is SMS-friendly ({len(messages['sms_body'])} chars)"
            )
        else:
...
def test_notify_with_patient():
    ...
    try:
        ...
        email_notif = next(
            (n for n in result["notifications_sent"] if n["type"] == "email"), None
        )
        ...
        sms_notif = next(
            (n for n in result["notifications_sent"] if n["type"] == "sms"), None
        )
...
def test_message_content():
    ...
    try:
        ...
        required_in_sms = ["test@example.com", "testPass", "change"]
