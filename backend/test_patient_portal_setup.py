...
def test_patient_model_has_portal_enabled():
    ...
    try:
        ...
        assert hasattr(patient, "portal_enabled"), (
            "Patient model missing portal_enabled field"
        )
...
def test_create_patient_user():
    ...
    try:
        ...
        print(
            f"  Patient Portal User: {patient.portal_user.username if hasattr(patient, 'portal_user') else None}"
        )
...
def test_validation_non_patient_cannot_link():
    ...
    try:
        ...
        if "patient" in str(e).lower() or "role" in str(e).lower():
            print_success(
                "Validation correctly prevents non-PATIENT roles from linking"
            )
            print(f"  Error message: {e}")
            return True
        else:
...
def test_one_to_one_constraint():
    ...
    try:
        ...
        try:
            ...
            print_success(
                "One-to-one constraint enforced - second user creation failed"
            )
...
def run_all_tests():
    ...
    print_info("Testing patient portal account implementation...")
    ...
    tests = [
        ...
        (
            "Validation: PATIENT requires link",
            test_validation_patient_role_requires_patient,
        ),
        (
            "Validation: Non-PATIENT cannot link",
            test_validation_non_patient_cannot_link,
        ),
