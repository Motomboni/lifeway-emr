...
def test_api_without_portal():
    ...
    try:
        ...
        receptionist = User.objects.create_user(
            username="api_test_receptionist1", password="testpass", role="RECEPTIONIST"
        )
...
def test_api_with_portal():
    ...
    try:
        # Setup
        receptionist = User.objects.create_user(
            username="api_test_receptionist2", password="testpass", role="RECEPTIONIST"
        )
        ...
        print(
            f"  Password Length: {len(response.data['portal_credentials']['temporary_password'])}"
        )
...
def test_api_duplicate_email_error():
    ...
    try:
        ...
        existing_user = User.objects.create_user(
            username="api_test_existing@example.com", password="testpass", role="DOCTOR"
        )
        print_info(f"Created existing user: {existing_user.username}")

        receptionist = User.objects.create_user(
            username="api_test_receptionist3", password="testpass", role="RECEPTIONIST"
        )
        ...
        error_text = (
            response.data.get("error", "") + response.data.get("detail", "")
        ).lower()
...
def test_api_missing_email_error():
    ...
    try:
        receptionist = User.objects.create_user(
            username="api_test_receptionist4", password="testpass", role="RECEPTIONIST"
        )
...
def test_response_format():
    ...
    try:
        receptionist = User.objects.create_user(
            username="api_test_receptionist5", password="testpass", role="RECEPTIONIST"
        )
