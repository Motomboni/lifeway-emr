...
if __name__ == "__main__":
    ...
    if services:
        ...
        for s in services[:5]:
            print(
                f"  {s['Service Code']}: {s['Service Name'][:40]:40s} - N{s['Amount']}"
            )
        print("\nLast 5 services:")
        for s in services[-5:]:
            print(
    ...
    print("\nNext steps:")
    print("  1. Review pharmacy_services.csv")
    print(
        "  2. Run: python manage.py import_service_catalog pharmacy_services.csv --dry-run"
    )
    print("  3. If OK: python manage.py import_service_catalog pharmacy_services.csv")
