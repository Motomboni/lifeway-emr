...
from apps.visits.service_catalog_service import (
    can_order_service,
...
class ServiceCatalogVisitCreationTests(TestCase):
    ...
    def test_one_active_visit_per_patient(self):
        ...
        active_visits = Visit.objects.filter(patient=self.patient, status="OPEN")
    ...
    def test_visit_creation_is_atomic(self):
        ...
        self.assertEqual(
            Visit.objects.filter(patient=self.patient).count(), initial_count + 1
        )
    ...
    def test_can_order_service(self):
        ...
        # Receptionist can order consultation/registration services (see ServiceCatalog.can_be_ordered_by)
        self.assertTrue(can_order_service(self.gopd_service, "RECEPTIONIST"))
    ...
    def test_multiple_patients_can_have_active_visits(self):
        ...
        self.assertEqual(
            Visit.objects.filter(patient=self.patient, status="OPEN").count(), 1
        )
        self.assertEqual(
            Visit.objects.filter(patient=patient2, status="OPEN").count(), 1
        )
