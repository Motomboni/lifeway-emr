...
def log_lab_ordered(sender, instance, created, **kwargs):
    ...
    if created:
        tests = (
            ", ".join(instance.tests_requested)
            if isinstance(instance.tests_requested, list)
            else str(instance.tests_requested)
        )
        create_timeline_event(
            visit=instance.visit,
...
def log_drug_dispensed(sender, instance, created, **kwargs):
    ...
    if not created and instance.dispensed:
        ...
        if (
            hasattr(instance, "_previous_dispensed")
            and not instance._previous_dispensed
        ):
            create_timeline_event(
                visit=instance.visit,
...
def log_payment_confirmed(sender, instance, created, **kwargs):
    ...
    if not created and instance.bill_status == "PAID":
        ...
        if (
            hasattr(instance, "_previous_bill_status")
            and instance._previous_bill_status != "PAID"
        ):
            service_name = (
                instance.service_catalog.name
                if instance.service_catalog
                else instance.source_service_name
            )
            create_timeline_event(
                visit=instance.visit,
...
def store_previous_bill_status(sender, instance, **kwargs):
    ...
    if instance.pk:
        try:
            ...
            old_instance = BillingLineItem.objects.only("bill_status").get(
                pk=instance.pk
            )
...
def log_procedure_completed(sender, instance, created, **kwargs):
    ...
    if not created and instance.status == ProcedureTask.Status.COMPLETED:
        ...
        if (
            ...
            and instance._previous_status != ProcedureTask.Status.COMPLETED
        ):
            create_timeline_event(
                visit=instance.visit,
