...
class VisitSerializer(serializers.ModelSerializer):
    ...
    payment_gates = serializers.SerializerMethodField()
...
class VisitCreateSerializer(VisitSerializer):
    ...
    def validate_patient(self, value):
        """Ensure patient exists, is active, and belongs to current organization."""
        ...
        if request and hasattr(request, "organization") and request.organization:
            if value.organization_id != request.organization.id:
                raise serializers.ValidationError(
                    "Patient does not belong to the current organization."
                )
        return value
    ...
    def validate(self, attrs):
        ...
        if "patient" not in attrs:
            raise serializers.ValidationError("Patient is required to create a visit.")
        ...
        if not payment_type or payment_type == "CASH":
            ...
            if active_insurance:
                ...
                is_valid = active_insurance.valid_from <= today and (
                    active_insurance.valid_to is None
                    or active_insurance.valid_to >= today
                )
...
class VisitReadSerializer(VisitSerializer):
    ...
    class Meta(VisitSerializer.Meta):
        fields = list(VisitSerializer.Meta.fields) + [
            "patient_details",
            "patient_retainership",
        ]
    ...
    def get_patient_details(self, obj):
        ...
        try:
            ...
            return {
                ...
                "structured_allergies": structured_allergies,
            }
        except Exception:
            return None
