...
def log_visit_action(user, action, visit_id, request=None, metadata=None):
    """
...
class VisitViewSet(viewsets.ModelViewSet):
    ...
    - Audit logging
    - Multi-tenant: filtered by request.organization
    """
    ...
    def get_queryset(self):
        ...
        qs = Visit.objects.all().select_related("patient", "closed_by")
        from core.tenant import filter_by_organization

        return filter_by_organization(qs, self.request)
    ...
    def get_permissions(self):
        ...
        from core.permissions import (
            CanCreateVisit,
            CanViewVisits,
            with_tenant_permissions,
        )

        if self.action == "close":
            return with_tenant_permissions(IsDoctor)
        if self.action == "create":
            return with_tenant_permissions(CanCreateVisit)
        if self.action in ["list", "retrieve"]:
            return with_tenant_permissions(CanViewVisits)
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            if payment_type == "INSURANCE":
                ...
                initial_payment_status = serializer.validated_data.get(
            ...
            for attempt in range(max_retries):
                try:
                    with transaction.atomic():
                        ...
                        if payment_type == "INSURANCE":
                            insurance_policy = InsurancePolicy.objects.filter(
                                patient=visit.patient, is_active=True
                            ).first()
                        ...
                        bill, created = Bill.objects.get_or_create(
                            visit=visit, defaults=bill_defaults
                        )
                        ...
                        if payment_type == "INSURANCE":
                            from apps.billing.insurance_models import (
                                HMOProvider,
                                VisitInsurance,
                            )
                            ...
                            if not VisitInsurance.objects.filter(visit=visit).exists():
                                ...
                                if insurance_policy:
                                    try:
                                        ...
                                        hmo_provider, created = (
                                            HMOProvider.objects.get_or_create(
                                                name=insurance_provider.name,
                                                defaults={
                                                    "code": insurance_provider.code
                                                    or "",
                                                    "contact_person": insurance_provider.contact_person
                                                    or "",
                                                    "contact_phone": insurance_provider.contact_phone
                                                    or "",
                                                    "contact_email": insurance_provider.contact_email
                                                    or "",
                                                    "address": insurance_provider.address
                                                    or "",
                    ...
                    if "database is locked" in str(e) and attempt < max_retries - 1:
                        print(
                            f"DEBUG: Database locked, retrying visit creation (attempt {attempt + 1})..."
                        )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        logger.info(
            f"Retrieving visit {visit_id} for user {request.user.id} ({request.user.role})"
        )
    ...
    def check_user_role(self, request):
        ...
        if user_role != "DOCTOR":
            raise PermissionDenied(
                detail="Only doctors can close visits.", code="role_forbidden"
            )
