...
class Visit(models.Model):
    ...
    # Multi-tenancy: organization (denormalized for efficient filtering)
    ...
    visit_type = models.CharField(
        ...
        choices=[
            ...
            ("SANDBOX", "Sandbox (Guide Practice)"),
        ],
        help_text="Type of visit",
    )

    chief_complaint = models.TextField(
        blank=True, null=True, help_text="Chief complaint or reason for visit"
    )
    ...
    closed_at = models.DateTimeField(
        null=True, blank=True, help_text="When the visit was closed"
    )

    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the visit was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the visit was last updated"
    )
        ...
        if self.payment_status == "INSURANCE_PENDING":
            try:
                ...
                if VisitInsurance.objects.filter(
                    visit_id=self.pk, approval_status="APPROVED"
                ).exists():
