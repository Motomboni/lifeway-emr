...
def is_registration_service(service: ServiceCatalog) -> bool:
    ...
    return (
        service_code.startswith("REG-")
        or "REGISTRATION" in service_name
        or "REGISTRATION" in description
    )
...
def order_downstream_service(
    ...
    if workflow_type == "LAB_ORDER":
        ...
        return _order_pharmacy_service(
            service, visit, consultation, user, additional_data
        )
    elif workflow_type == "PROCEDURE":
        return _order_procedure_service(
            service, visit, consultation, user, additional_data
        )
    elif workflow_type == "RADIOLOGY_STUDY" or service.department == "RADIOLOGY":
        return _order_radiology_service(
            service, visit, consultation, user, additional_data
        )
    else:
        raise ValidationError(
...
def _order_lab_service(
    ...
    tests_requested = (
        additional_data.get("tests_requested", []) if additional_data else []
    )
    clinical_indication = (
        additional_data.get("clinical_indication", "") if additional_data else ""
    )

    if not tests_requested:
        raise ValidationError(
            "LAB service requires 'tests_requested' in additional_data."
        )
...
def _order_pharmacy_service(
    ...
    drug_name = (
        additional_data.get("drug") or additional_data.get("drug_name") or service.name
    )
    if not drug_name:
        raise ValidationError(
            "PHARMACY service requires 'drug' or 'drug_name' in additional_data, or service must have a name."
        )
...
def _order_radiology_service(
    ...
    # Extract additional data
    study_type = (
        additional_data.get("study_type", service.name)
        if additional_data
        else service.name
    )
    study_code = (
        additional_data.get("study_code", service.service_code)
        if additional_data
        else service.service_code
    )
    clinical_indication = (
        additional_data.get("clinical_indication", "") if additional_data else ""
    )
    ...
    patient_location = (
        additional_data.get("patient_location", "") if additional_data else ""
    )
