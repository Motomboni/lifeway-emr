...
def _create_consultation_for_visit(
    ...
    if doctor and doctor.role != "DOCTOR":
        raise ValidationError(
            f"User '{doctor.username}' is not a doctor. Only doctors can be assigned to consultations."
        )
...
def consultation_allows_clinical_orders(consultation: Consultation) -> bool:
    """
    True when downstream orders (lab, radiology, prescriptions) may proceed.

    ACTIVE consultations always qualify. PENDING qualifies once registration is paid
    (doctor is in the consultation workspace); the order handler will activate.
    """
    ...
    if is_registration_paid(visit):
        return True
    # Visits with cleared payment but no catalog registration rows (legacy/dev data)
    return visit.is_payment_cleared()


def maybe_activate_consultation_for_doctor_order(
    consultation: Consultation, doctor: User
) -> Consultation:
    """
    Auto-activate a PENDING consultation when a doctor places clinical orders.

    Mirrors service-catalog downstream ordering: registration is already paid,
    so the encounter becomes ACTIVE on first order rather than blocking the doctor.
    """
    if consultation.status == "ACTIVE":
        return consultation
    if consultation.status != "PENDING":
    ...
    if getattr(doctor, "role", None) != "DOCTOR":
        return consultation
    if not consultation_allows_clinical_orders(consultation):
        return consultation
...
def close_consultation(consultation: Consultation, doctor: User) -> Consultation:
    ...
    if consultation.created_by and consultation.created_by != doctor:
        raise ValidationError("Only the assigned doctor can close this consultation.")
