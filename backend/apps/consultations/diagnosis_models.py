...
class DiagnosisCode(models.Model):
    ...
    code = models.CharField(
        max_length=20, help_text="Diagnosis code (e.g., 'CA40.Z' for ICD-11)"
    )

    description = models.CharField(
        max_length=500, help_text="Description of the diagnosis code"
    )

    is_primary = models.BooleanField(
        default=False, help_text="True if this is the primary diagnosis"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When code was assigned"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When code was last updated"
    )
        ...
        if self.is_primary and self.consultation_id:
            existing_primary = (
                DiagnosisCode.objects.filter(
                    consultation=self.consultation, is_primary=True
                )
