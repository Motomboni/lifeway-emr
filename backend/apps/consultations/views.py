...
class ConsultationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = Consultation.objects.filter(visit_id=visit_id)
        return filter_by_visit_organization(qs, self.request)
    ...
    def check_payment_status(self, visit):
        """Registration must be paid before consultation mutations."""
        from apps.billing.payment_gates_service import is_registration_paid

        if not is_registration_paid(visit):
            raise PermissionDenied(
                detail="Registration payment is required before consultation. "
    ...
    def merge_with_patient_history(self, consultation, visit):
        ...
        timestamp = (
            consultation.created_at if consultation.created_at else timezone.now()
        )
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            try:
                ...
                merge_with_patient = serializer.validated_data.pop(
                    "merge_with_patient_record", False
                )

                consultation = serializer.save(
                    visit=visit, created_by=self.request.user
                )
            ...
            try:
                ...
                logger.error(
                    f"Failed to log consultation create action: {str(e)}", exc_info=True
                )
            ...
            logger.error(
                f"Unexpected error creating consultation: {str(e)}", exc_info=True
            )
            raise DRFValidationError(f"Error creating consultation: {str(e)}")
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            serializer = self.get_serializer(
                consultation, data=request.data, partial=partial
            )
    ...
    def get_permissions(self):
        """
        Strict payment rules (Lifeway):
        - Read (retrieve/list): Registration must be paid before access to consultation.
        - Write (create/update/partial_update): Registration must be paid (consultation fee
          may be collected during/after care).
        """
        from core.permissions import with_tenant_permissions

        if self.action in ["retrieve", "list"]:
            return with_tenant_permissions(IsRegistrationPaymentCleared)
        return with_tenant_permissions(
            IsDoctor, IsVisitOpen, IsRegistrationPaymentCleared
        )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        try:
            ...
            consultation = (
                Consultation.objects.filter(visit_id=visit.id)
            ...
            try:
                ...
                logger = logging.getLogger(__name__)
                logger.error(
                    f"Failed to log consultation read action: {str(e)}", exc_info=True
                )
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            ...
            consultation = (
                Consultation.objects.filter(visit_id=visit_id)
            ...
            if consultation:
                ...
                try:
                    ...
                    logger = logging.getLogger(__name__)
                    logger.error(
                        f"Failed to log consultation read action: {str(e)}",
                        exc_info=True,
                    )
            ...
            logger = logging.getLogger(__name__)
            logger.error(
                f"Error listing consultation: {str(e)}", exc_info=True, stack_info=True
            )
    ...
    def patient_consultations(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"count": consultations.count(), "results": serializer.data}
            )
            ...
            logger = logging.getLogger(__name__)
            logger.error(
                f"Error fetching patient consultations: {str(e)}", exc_info=True
            )
