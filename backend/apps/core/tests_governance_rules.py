...
class GovernanceRulesTests(TestCase):
    ...
    def test_prescription_dispensing_with_payment(self):
        ...
        billing_line_item.apply_payment(
            payment_amount=self.pharmacy_service.amount, payment_method="CASH"
        )
...
class ConsultationActivationForOrdersTests(TestCase):
    """PENDING consultations auto-activate when doctors place orders (registration paid)."""
    ...
    def setUp(self):
        ...
        self.doctor = User.objects.create_user(
            username="doctor_orders",
            email="doctor_orders@test.com",
            password="testpass123",
            role="DOCTOR",
            first_name="Order",
            last_name="Doctor",
    ...
    def test_pending_consultation_allows_lab_order_lock_when_registration_paid(self):
    ...
    def test_maybe_activate_consultation_for_doctor_order(self):
        from apps.consultations.gopd_workflow_service import (
            maybe_activate_consultation_for_doctor_order,
        )

        updated = maybe_activate_consultation_for_doctor_order(
            self.consultation, self.doctor
        )
