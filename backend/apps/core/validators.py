...
def validate_consultation_required(value):
    ...
    if value is None:
        raise ValidationError(
            _(
                "Lab orders require a consultation. Please ensure a consultation exists for this visit."
            ),
...
def validate_prescription_dispensing(value, emergency_override=False):
    ...
    if value.status != "PENDING":
        raise ValidationError(
            _("Only PENDING prescriptions can be dispensed. Current status: %(status)s")
            % {"status": value.status},
            code="invalid_status",
        )
