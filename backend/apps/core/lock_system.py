...
class LockEvaluator:
    ...
    @staticmethod
    def _consultation_allows_clinical_order(consultation, visit) -> bool:
        """PENDING + registration paid is allowed; orders auto-activate the consultation."""
        from apps.consultations.gopd_workflow_service import (
            consultation_allows_clinical_orders,
        )

        return consultation_allows_clinical_orders(consultation)

    @staticmethod
    def evaluate_radiology_upload_lock(
        radiology_order_id: int, visit_id: int = None
    ) -> LockResult:
        """
        ...
        if not visit.is_payment_cleared():
            return LockResult(
                ...
                details={"payment_status": visit.payment_status, "visit_id": visit.id},
                unlock_actions=[
                    "Process payment for this visit",
                    "Update payment status to PAID or SETTLED",
                ],
    ...
    def evaluate_drug_dispense_lock(
        prescription_id: int, visit_id: int = None
    ) -> LockResult:
        """
    ...
    @staticmethod
    def evaluate_lab_order_lock(
        visit_id: int, consultation_id: int = None
    ) -> LockResult:
        """
        ...
        if consultation_id:
            ...
            try:
                ...
                if not LockEvaluator._consultation_allows_clinical_order(
                    consultation, visit
                ):
                    return LockResult(
                        is_locked=True,
    ...
    def evaluate_lab_result_post_lock(lab_order_id: int) -> LockResult:
        ...
        if lab_order.status not in [
            ...
            return LockResult(
                ...
                unlock_actions=[
                    "Ensure lab order is in Ordered or Sample Collected status"
                ],
            )
    ...
    @staticmethod
    def evaluate_action_lock(action_type: str, **kwargs) -> LockResult:
        """
