...
def validate_service_catalog_origin(
    action_type: str, service_catalog_id: Optional[int] = None
) -> None:
    """
...
def validate_visit_consultation_chain(
    visit, consultation=None, action_type: str = "Action"
) -> None:
    """
    ...
    if consultation.visit_id != visit.id:
        raise ValidationError(
            "Consultation does not belong to the specified Visit. "
    ...
    if hasattr(order, "status"):
        ...
        if hasattr(order, "Status") and hasattr(order.Status, "ORDERED"):
            if order.status not in [
                ...
                getattr(order.Status, "SAMPLE_COLLECTED", None),
            ]:
                raise ValidationError(
