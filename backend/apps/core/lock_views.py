...
from .lock_system import LockEvaluator
...
class LockEvaluationViewSet(viewsets.ViewSet):
    ...
    def evaluate(self, request):
        ...
        if not action_type:
            return Response(
                {"error": "action_type is required"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def consultation(self, request):
        ...
        if not visit_id:
            return Response(
                {"error": "visit_id is required"}, status=status.HTTP_400_BAD_REQUEST
            )
