...
class PaymentIntentViewSet(viewsets.ModelViewSet):
    ...
    def verify(self, request, visit_id=None, pk=None):
        ...
        if payment_intent.visit_id != visit.id:
            raise DRFValidationError("Payment intent does not belong to this visit.")
        ...
        if reference != payment_intent.paystack_reference:
            raise DRFValidationError("Reference does not match payment intent.")
            ...
            return Response(
                {
                    ...
                    "payment_intent": PaymentIntentSerializer(payment_intent).data,
                    "payment": payment_intent.payment.id
        ...
        if metadata_visit_id != visit.id:
            payment_intent.mark_as_failed(
                reason="Visit ID mismatch in Paystack metadata"
            )
            return Response(
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        raise PermissionDenied(
            detail="Payment intents cannot be deleted.", code="immutable_payment_intent"
        )
...
def paystack_webhook(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Failed to create payment record: {str(e)}"}, status=500
        )
