...
class EmailService:
    ...
    def send_receipt_email(
        receipt_data: dict, email_address: str, pdf_bytes: Optional[bytes] = None
    ) -> bool:
        """
...
{receipt_data.get("clinic_phone", "")}
            ...
            email = EmailMessage(
                ...
                body=body,
                from_email=getattr(
                    settings, "DEFAULT_FROM_EMAIL", "noreply@clinic.com"
                ),
                to=[email_address],
            )
            ...
            logger.info(
                f"Receipt email sent to {email_address} for visit {receipt_data.get('visit_id')}"
            )
            return True
    ...
    def send_invoice_email(
        invoice_data: dict, email_address: str, pdf_bytes: Optional[bytes] = None
    ) -> bool:
        """
...
{invoice_data.get("clinic_phone", "")}
            ...
            logger.info(
                f"Invoice email sent to {email_address} for visit {invoice_data.get('visit_id')}"
            )
            return True
