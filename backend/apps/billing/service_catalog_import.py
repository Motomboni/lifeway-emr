...
def import_services(
    data: list, update_existing: bool = False, dry_run: bool = False
) -> dict:
    """
    ...
    with transaction.atomic():
        for index, row in enumerate(data, start=1):
            try:
                ...
                workflow_type = (
                    str(normalized_row.get("workflow_type", "")).strip().upper()
                )
                ...
                try:
                    ...
                    if amount <= 0:
                        stats["errors"].append(
                            f"Row {index}: Amount must be greater than zero"
                        )
                        stats["skipped"] += 1
                        continue
                except (InvalidOperation, ValueError):
                    stats["errors"].append(
                        f"Row {index}: Invalid amount '{amount_str}'"
                    )
                    stats["skipped"] += 1
                    continue

                description = str(normalized_row.get("description", "")).strip()
                requires_visit = _parse_boolean(
                    normalized_row.get("requires_visit"), default=True
                )
                requires_consultation = _parse_boolean(
                    normalized_row.get("requires_consultation"), default=False
                )
                auto_bill = _parse_boolean(
                    normalized_row.get("auto_bill"), default=True
                )
                bill_timing = (
                    str(normalized_row.get("bill_timing", "AFTER")).strip().upper()
                )
                is_active = _parse_boolean(
                    normalized_row.get("is_active"), default=True
                )
                    ...
                    if update_existing:
                        if not dry_run:
                            try:
                                ...
                                stats["errors"].append(
                                    f"Row {index}: Validation error: {str(e)}"
                                )
                                stats["skipped"] += 1
                                continue
                        stats["updated"] += 1
