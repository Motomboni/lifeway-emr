...
class BillingLineItemSerializer(serializers.ModelSerializer):
    ...
    visit_id = serializers.IntegerField(
        source="visit.id", read_only=True, help_text="Visit ID"
    )

    patient_name = serializers.CharField(
        source="visit.patient.get_full_name", read_only=True, help_text="Patient name"
    )
...
class BillingLineItemCreateSerializer(serializers.ModelSerializer):
    ...
    service_catalog_id = serializers.IntegerField(
        write_only=True, help_text="ServiceCatalog ID to create billing line item from"
    )

    visit_id = serializers.IntegerField(
        write_only=True, help_text="Visit ID this billing line item belongs to"
    )
    ...
    def validate_service_catalog_id(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError(
                f"ServiceCatalog with ID {value} does not exist."
            )
    ...
    def validate_consultation_id(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError(
                f"Consultation with ID {value} does not exist."
            )
    ...
    def create(self, validated_data):
        ...
        visit = get_org_scoped_visit(request, visit_id)
        consultation = (
            Consultation.objects.get(pk=consultation_id) if consultation_id else None
        )
...
class BillingLineItemPaymentSerializer(serializers.Serializer):
    ...
    payment_amount = serializers.DecimalField(
        max_digits=10, decimal_places=2, help_text="Payment amount to apply"
    )
