...
class VisitCharge(models.Model):
    ...
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Charge amount"
    )
    ...
    # Audit timestamps
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When charge was created (system-generated)"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When charge was last updated"
    )
    ...
    def create_lab_charge(cls, visit, lab_order, amount, description=None):
        ...
        return cls.objects.create(
            ...
            or f"Lab Order - {lab_order.test_name if hasattr(lab_order, 'test_name') else 'Lab Test'}",
            amount=amount,
            lab_order=lab_order,
            created_by_system=True,
        )
    ...
    def create_radiology_charge(cls, visit, radiology_order, amount, description=None):
        ...
        return cls.objects.create(
            ...
            category="RADIOLOGY",
            description=description
            or f"Radiology Order - {radiology_order.study_type if hasattr(radiology_order, 'study_type') else 'Radiology Study'}",
            amount=amount,
    ...
    def create_drug_charge(cls, visit, prescription, amount, description=None):
        ...
        return cls.objects.create(
            ...
            category="DRUG",
            description=description
            or f"Drug Prescription - {prescription.medication_name if hasattr(prescription, 'medication_name') else 'Medication'}",
            amount=amount,
    ...
    def get_total_charges_for_visit(cls, visit):
        ...
        """
        return cls.objects.filter(visit=visit).aggregate(total=models.Sum("amount"))[
            "total"
        ] or Decimal("0.00")
...
class PaymentIntent(models.Model):
    ...
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Payment amount (in NGN)"
    )
    ...
    verified_at = models.DateTimeField(
        null=True, blank=True, help_text="When payment was verified (server-side)"
    )
    ...
    # Audit timestamps
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When payment intent was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When payment intent was last updated"
    )
...
from apps.billing.payment_reconciliation_models import PaymentReconciliation  # noqa: E402,F401
