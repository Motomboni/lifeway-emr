...
from apps.billing.bill_models import Bill, BillItem, BillPayment
...
class VisitBillingSummaryView(APIView):
    ...
    def get(self, request, visit_id):
        ...
        insurance_view = (
            request.query_params.get("insurance_view", "false").lower() == "true"
        )
        ...
        bill_items = BillItem.objects.filter(bill=bill).order_by(
        ...
        response_data = {
            ...
            "patient_payable_after_insurance": str(patient_payable_after_insurance)
            if insurance_coverage is not None
            else None,
            "summary": {
                ...
                "can_accept_payment": bill.outstanding_balance > 0
                and visit.status == "OPEN",
                "is_fully_paid": bill.outstanding_balance <= 0,
                "is_partially_paid": bill.amount_paid > 0
                and bill.outstanding_balance > 0,
            },
        }
        ...
        if insurance_view and bill.is_insurance_backed:
            response_data["insurance_view"] = {
                "insurance_amount": str(insurance_coverage)
                if insurance_coverage
                else "0.00",
                "patient_payable": str(patient_payable_after_insurance)
                if patient_payable_after_insurance
                else str(bill.total_amount),
                "patient_paid": str(bill.amount_paid),
                "patient_outstanding": str(
                    max(
                        Decimal("0.00"),
                        patient_payable_after_insurance - bill.amount_paid,
                    )
                ),
                "insurance_status": "COVERED"
                if insurance_coverage and insurance_coverage >= bill.total_amount
                else "PARTIAL"
                if insurance_coverage
                else "PENDING",
            }
