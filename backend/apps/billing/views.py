...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def check_user_role(self, request):
        ...
        if user_role != "RECEPTIONIST":
            raise PermissionDenied(
                detail="Only Receptionists can process payments.", code="role_forbidden"
            )
    ...
    def clear(self, request, visit_id=None, pk=None):
        ...
        if payment.visit_id != visit.id:
            raise DRFValidationError("Payment does not belong to this visit.")

        # Validate payment is not already cleared
        if payment.status == "CLEARED":
            raise DRFValidationError("Payment is already cleared.")
        ...
        if serializer.validated_data.get("transaction_reference"):
            payment.transaction_reference = serializer.validated_data[
        ...
        return Response(
            {
                ...
                "visit": {"id": visit.id, "payment_status": visit.payment_status},
            },
