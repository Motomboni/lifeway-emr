    ...
    def post(self, request, visit_id):
        ...
        try:
            charge = VisitCharge.create_misc_charge(
                visit=visit, amount=amount, description=description
            )
        except Exception as e:
...
class BillingPaymentsView(BillingEndpointView):
    ...
    def get(self, request, visit_id):
        ...
        payments = (
            Payment.objects.filter(visit=visit)
    ...
    def post(self, request, visit_id):
        ...
        if serializer.validated_data.get("payment_method") in [
            "CASH",
            "POS",
            "TRANSFER",
            "WALLET",
        ...
        try:
            ...
            try:
                ...
                try:
                    bill = Bill.objects.create(
                        ...
                        amount_paid=summary.total_payments
                        + summary.total_wallet_debits,
                    ...
                    logger = logging.getLogger(__name__)
                    logger.error(
                        f"Failed to create bill for visit {visit.id}: {str(e)}",
                        exc_info=True,
                    )
            except Exception as e:
                # Log error but don't fail the payment creation
                logger = logging.getLogger(__name__)
                logger.error(
                    f"Failed to update bill for visit {visit.id}: {str(e)}",
                    exc_info=True,
                )
        except Exception as e:
            ...
            logger = logging.getLogger(__name__)
            logger.error(
                f"Failed to compute billing summary for visit {visit.id}: {str(e)}",
                exc_info=True,
            )
...
class BillingWalletDebitView(BillingEndpointView):
    ...
    def post(self, request, visit_id):
        ...
        try:
            ...
            bill.amount_paid = (
                updated_summary.total_payments + updated_summary.total_wallet_debits
            )
            ...
            try:
                bill = Bill.objects.create(
                    ...
                    amount_paid=updated_summary.total_payments
                    + updated_summary.total_wallet_debits,
            ...
            logger = logging.getLogger(__name__)
            logger.error(
                f"Failed to update bill for visit {visit.id}: {str(e)}", exc_info=True
            )
...
class BillingInsuranceView(BillingEndpointView):
    ...
    def post(self, request, visit_id):
        ...
        if (
            serializer.validated_data.get("visit")
            and serializer.validated_data["visit"].id != visit.id
        ):
            raise DRFValidationError(
                "visit_id in URL must match visit in request data."
            )
        ...
        insurance = serializer.save(visit=visit, created_by=request.user)
        ...
        return Response(
            VisitInsuranceSerializer(insurance).data, status=status.HTTP_201_CREATED
        )
