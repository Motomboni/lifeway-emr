...
class Bill(models.Model):
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When bill was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When bill was last updated"
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=Q(visit__isnull=False), name="bill_requires_visit"
            ),
        ]
        verbose_name = "Bill"
    ...
    def add_payment(
        ...
        self.recalculate_totals()
        self.save(
            update_fields=["amount_paid", "outstanding_balance", "status", "updated_at"]
        )
    ...
    def recalculate_totals(self):
        ...
        total_items = self.items.aggregate(total=Sum("amount"))["total"] or Decimal(
            "0.00"
        )
        ...
        total_payments = self.payments.aggregate(total=Sum("amount"))[
            "total"
        ] or Decimal("0.00")
...
class BillItem(models.Model):
    ...
    service_name = models.CharField(
        max_length=255, help_text="Name of the service/item"
    )

    amount = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Item amount"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When item was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When item was last updated"
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=Q(bill__isnull=False), name="bill_item_requires_bill"
            ),
        ]
    ...
    def save(self, *args, **kwargs):
        ...
        if self.bill:
            ...
            self.bill.save(
                update_fields=["total_amount", "outstanding_balance", "updated_at"]
            )

    def delete(self, *args, **kwargs):
        """Prevent deletion - bill items are immutable."""
        from core.superuser_purge import is_superuser_purge_active

        if is_superuser_purge_active():
...
class BillPayment(models.Model):
    ...
    payment_method = models.CharField(
        max_length=20, choices=PAYMENT_METHOD_CHOICES, help_text="Method of payment"
    )

    transaction_reference = models.CharField(
        max_length=255, blank=True, help_text="Transaction reference number"
    )
    ...
    # Audit timestamps
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When payment was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When payment was last updated"
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=Q(bill__isnull=False), name="bill_payment_requires_bill"
            ),
...
class InsuranceProvider(models.Model):
    ...
    name = models.CharField(
        max_length=255, unique=True, help_text="Name of the insurance provider"
    )
    ...
    contact_person = models.CharField(
        max_length=255, blank=True, help_text="Contact person name"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether the provider is currently active"
    )

    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When provider was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When provider was last updated"
    )
...
class InsurancePolicy(models.Model):
    ...
    policy_number = models.CharField(
        max_length=255, help_text="Insurance policy number"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether the policy is currently active"
    )

    valid_from = models.DateField(help_text="Policy validity start date")

    valid_to = models.DateField(
        null=True, blank=True, help_text="Policy validity end date (if applicable)"
    )

    # Audit fields
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When policy was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When policy was last updated"
    )
