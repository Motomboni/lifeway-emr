...
from typing import Any, Dict, Optional  # noqa: F401 - Optional used in dataclass
...
class BillingSummary:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        out = {
            ...
            "retainership_discount_percentage": str(
                self.retainership_discount_percentage
            ),
...
class BillingService:
    ...
    def compute_billing_summary(visit) -> BillingSummary:
        ...
        if has_retainership:
            retainership_discount = compute_retainership_discount(
                total_charges, patient
            )
            retainership_discount_percentage = get_retainership_discount_percentage(
                patient
            )
        ...
        can_be_cleared = BillingService._can_clear_payment(
            patient_payable, total_paid, is_fully_covered
        )
    ...
    def _compute_total_charges(visit) -> Decimal:
        ...
        billing_line_item_total = BillingLineItem.objects.filter(visit=visit).aggregate(
            total=Sum("amount")
        )["total"] or Decimal("0.00")
    ...
    def _compute_total_payments(visit) -> Decimal:
        ...
        """
        total = Payment.objects.filter(visit=visit, status="CLEARED").aggregate(
            total=Sum("amount")
        )["total"]
    ...
    def _compute_total_wallet_debits(visit) -> Decimal:
        ...
        total = WalletTransaction.objects.filter(
            visit=visit, transaction_type="DEBIT", status="COMPLETED"
    ...
    def _compute_insurance_amount(
        visit, total_charges: Decimal, insurance_info: Dict[str, Any]
    ) -> Decimal:
        """
    ...
    def _determine_payment_status(
        ...
        Edge Cases:
        - Zero charges and zero paid: Returns UNPAID (charges may be added later)
        - Overpayment: Returns PAID (outstanding_balance < 0)
        - Full insurance: Returns SETTLED (patient_payable == 0)
        """
        # No charges yet — do not report PAID before services are billed
        if total_charges == 0 and total_paid == 0:
            if has_insurance and insurance_status == "PENDING":
                return "INSURANCE_PENDING"
            return "UNPAID"

        # If patient_payable is 0 (fully covered by insurance or discounts)
    ...
    def _can_clear_payment(
        patient_payable: Decimal, total_paid: Decimal, is_fully_covered: bool
    ) -> bool:
        """
    ...
    def can_close_visit(visit) -> tuple[bool, str]:
        ...
        if visit.payment_type == "CASH":
            ...
            if billing_summary.payment_status not in [
                "INSURANCE_PENDING",
                "INSURANCE_CLAIMED",
                "SETTLED",
            ]:
            ...
            return (
                True,
                f"Visit can be closed. Payment status is '{billing_summary.payment_status}'.",
            )
