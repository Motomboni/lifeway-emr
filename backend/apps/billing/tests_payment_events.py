...
from apps.billing.payment_event_handlers import (
...
class PaymentConfirmedEventTests(TestCase):
    ...
    def test_billing_line_item_transition_to_paid_fires_event(self):
        ...
        with patch(
            "apps.billing.billing_line_item_signals.handle_payment_confirmed_idempotent"
        ) as mock_handler:
            # Transition to PAID
            self.billing_line_item.apply_payment(
                payment_amount=self.gopd_service.amount, payment_method="CASH"
            )
    ...
    def test_billing_line_item_partial_payment_no_event(self):
        ...
        with patch(
            ...
            self.billing_line_item.apply_payment(
                payment_amount=Decimal("2500.00"), payment_method="CASH"
            )
