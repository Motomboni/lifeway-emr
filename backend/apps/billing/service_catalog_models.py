...
class ServiceCatalog(models.Model):
    ...
    # Multi-tenancy: organization (clinic) this service belongs to
    ...
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Service price/amount"
    )

    description = models.TextField(
        blank=True, help_text="Detailed description of the service"
    )

    category = models.CharField(
        max_length=50, choices=CATEGORY_CHOICES, help_text="Service category"
    )
    ...
    requires_visit = models.BooleanField(
        default=True, help_text="Whether this service requires an active visit"
    )

    requires_consultation = models.BooleanField(
        default=False, help_text="Whether this service requires a consultation"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether this service is currently active/available"
    )

    # Metadata
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When this service was added to catalog"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When this service was last updated"
    )
    ...
    def clean(self):
        ...
        if not isinstance(self.allowed_roles, list):
            ...
            errors["allowed_roles"] = (
                "At least one role must be allowed to order this service."
            )
        else:
            valid_roles = [
                "ADMIN",
            ...
            invalid_roles = [
                role for role in self.allowed_roles if role not in valid_roles
            ]
            if invalid_roles:
                errors["allowed_roles"] = (
        ...
        if self.bill_timing not in dict(self.BILL_TIMING_CHOICES):
            errors["bill_timing"] = (
                f"Invalid bill_timing. Must be one of: {', '.join([c[0] for c in self.BILL_TIMING_CHOICES])}"
            )
    ...
    def is_registration_service(self) -> bool:
        ...
        return (
            self.service_code.upper().startswith("REG-")
            or "REGISTRATION" in (self.name or "").upper()
            or (self.description or "").upper().find("REGISTRATION") >= 0
        )

    def is_consultation_service(self) -> bool:
        """True if this service is Doctor Consultation (pre-service payment required)."""
        return (
            self.workflow_type == "GOPD_CONSULT"
            or self.department == "CONSULTATION"
            or self.service_code.upper().startswith("CONS-")
            or "CONSULTATION" in (self.name or "").upper()
            or "CONSULT" in (self.name or "").upper()
        )
    ...
    def can_be_ordered_by(self, user_role: str) -> bool:
        ...
        if user_role == "RECEPTIONIST":
            is_registration = (
                self.service_code.upper().startswith("REG-")
                or "REGISTRATION" in self.name.upper()
                or "REGISTRATION" in (self.description or "").upper()
            )
            if is_registration:
                return True
            ...
            is_consultation = (
                ...
                or self.department == "CONSULTATION"
                or self.service_code.upper().startswith("CONS-")
                or "FOLLOW UP" in self.name.upper()
                or "FOLLOW-UP" in self.name.upper()
                or "FOLLOWUP" in self.name.upper()
                or "CONSULTATION" in self.name.upper()
                or "CONSULT" in self.name.upper()
            )
            if is_consultation:
                return True
