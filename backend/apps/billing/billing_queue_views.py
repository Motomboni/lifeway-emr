...
class BillingPendingQueueView(APIView):
    ...
    def get(self, request):
        ...
        line_items_qs = BillingLineItem.objects.filter(
            bill_status__in=["PENDING", "PARTIALLY_PAID"]
        )
        if org:
            line_items_qs = line_items_qs.filter(visit__organization=org)
        visit_ids_with_pending = line_items_qs.values_list(
            "visit_id", flat=True
        ).distinct()

        visits_qs = Visit.objects.filter(id__in=visit_ids_with_pending)
        if org:
            visits_qs = visits_qs.filter(organization=org)
        visits = (
            ...
            .prefetch_related(
                Prefetch(
                    ...
                    .order_by("service_catalog__department", "created_at"),
                )
            )
            .order_by("-updated_at")
        )
            ...
            result.append(
                {
                    ...
                    "patient": {
                        "id": visit.patient.id,
                        "name": f"{visit.patient.first_name} {visit.patient.last_name}".strip()
                        or f"Patient #{visit.patient.id}",
                    },
                    "items": items,
