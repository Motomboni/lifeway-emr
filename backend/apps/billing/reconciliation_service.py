...
class ReconciliationService:
    ...
    def create_reconciliation(
        ...
        Args:
            ...
            organization_id: Organization (tenant) ID - required for multi-tenant
        ...
        # Check if reconciliation already exists for this date and org
        ...
        if existing:
            logger.info(
                f"Found existing reconciliation for {reconciliation_date}, refreshing calculations"
            )
            # Refresh the existing reconciliation to ensure it has the latest data
            ReconciliationService._perform_reconciliation(
                existing, close_active_visits=close_active_visits
            )
            return existing
        ...
        ReconciliationService._perform_reconciliation(
            reconciliation, close_active_visits=close_active_visits
        )
        ...
        try:
            ...
            reconciliation.revenue_leaks_amount = leaks_result.get(
                "total_estimated_loss", Decimal("0.00")
            )
            reconciliation.save(
                update_fields=["revenue_leaks_detected", "revenue_leaks_amount"]
            )
        except Exception as e:
            logger.warning(
                f"Error detecting leaks during reconciliation: {e}", exc_info=True
            )
    ...
    def _perform_reconciliation(
        reconciliation: EndOfDayReconciliation, close_active_visits: bool = True
    ):
        """
        ...
        # Get all visits for the date and organization
        visits = Visit.objects.filter(created_at__date=reconciliation_date)
        if organization_id:
            visits = visits.filter(organization_id=organization_id)
        ...
        if close_active_visits:
            ...
            logger.info(
                f"Closed {closed_count} active visits for {reconciliation_date}"
            )
        ...
        if organization_id:
            billing_items = billing_items.filter(visit__organization_id=organization_id)
        ...
        payments = Payment.objects.filter(
            ...
            status__in=[
                "CLEARED",
                "PARTIAL",
            ],  # Count both cleared and partial payments
        )
        if organization_id:
            payments = payments.filter(visit__organization_id=organization_id)
        payments = payments.select_related(
            "visit", "visit__patient", "processed_by"
        ...
        for payment in payments:
            ...
            if method in ["CASH", "POS", "TRANSFER"]:
                ...
                logger.info(
                    f"Payment {payment.id}: NGN {payment.amount} CASH (method={method}, visit={payment.visit_id})"
                )
            elif method == "WALLET":
                total_wallet += payment.amount
                logger.info(
                    f"Payment {payment.id}: NGN {payment.amount} WALLET (visit={payment.visit_id})"
                )
            elif method == "PAYSTACK":
                total_paystack += payment.amount
                logger.info(
                    f"Payment {payment.id}: NGN {payment.amount} PAYSTACK (visit={payment.visit_id})"
                )
                ...
                logger.info(
                    f"Payment {payment.id}: NGN {payment.amount} {method} (visit={payment.visit_id})"
                )
            else:
                logger.warning(
                    f"Payment {payment.id}: Unknown payment method '{method}', amount={payment.amount}"
                )
        ...
        reconciliation.total_revenue = (
            total_cash + total_wallet + total_paystack + total_hmo
        )
        ...
        logger.info(
            f"Total Revenue (Cash+Wallet+Paystack+HMO): NGN {reconciliation.total_revenue}"
        )
        ...
        if payment_count > 0:
            if abs(total_payment_sum - reconciliation.total_revenue) > Decimal("0.01"):
                logger.error(
                    f"ERROR: Total revenue ({reconciliation.total_revenue}) does not match sum of payments ({total_payment_sum})"
                )
                logger.error(
                    f"Difference: NGN {abs(total_payment_sum - reconciliation.total_revenue)}"
                )
            else:
                logger.info(
                    "Verification passed: Total revenue matches sum of all payments"
                )
                logger.info(
                    f"Expected: NGN {total_payment_sum}, Actual: NGN {reconciliation.total_revenue}"
                )

        # Calculate outstanding balances with detailed breakdown
        outstanding_items = (
            billing_items.filter(bill_status__in=["PENDING", "PARTIALLY_PAID"])
            .select_related("visit", "visit__patient", "service_catalog")
        ...
        reconciliation.outstanding_visits_count = (
            outstanding_items.values("visit").distinct().count()
        )
                    ...
                    outstanding_details_list.append(
                        {
                            ...
                            "visit_status": visit_outstanding_items[0].visit.status
                            if visit_outstanding_items[0].visit
                            else None,
                            "patient": {
                                "id": visit_outstanding_items[0].visit.patient.id
                                if visit_outstanding_items[0].visit
                                and visit_outstanding_items[0].visit.patient
                                else None,
                                "name": f"{visit_outstanding_items[0].visit.patient.first_name} {visit_outstanding_items[0].visit.patient.last_name}".strip()
                                if visit_outstanding_items[0].visit
                                and visit_outstanding_items[0].visit.patient
                                else None,
                                "mrn": visit_outstanding_items[
                                    0
            ...
            outstanding_details_list.append(
                {
                    ...
                    "patient": {
                        ...
                        "mrn": visit_outstanding_items[0].visit.patient.patient_id
                        if visit_outstanding_items[0].visit
                        and visit_outstanding_items[0].visit.patient
                        else None,
                    }
        ...
        logger.info(
            f"Outstanding balances: {reconciliation.outstanding_visits_count} visit(s) with NGN {outstanding_amount} total outstanding"
        )
        ...
        for payment in payments:
            ...
            visit_billing_items = BillingLineItem.objects.filter(
                visit=visit, bill_status="PAID"
            ).select_related("service_catalog")[
                :10
            ]  # Limit to 10 items to avoid huge payload
            ...
            payment_details_list.append(
                {
                    ...
                    "billing_items_count": BillingLineItem.objects.filter(
                        visit=visit, bill_status="PAID"
                    ).count()
                    if visit
                    else 0,
        ...
        reconciliation.reconciliation_details = {
            ...
            "billing": {
                ...
                "partially_paid_items": billing_items.filter(
    ...
    @staticmethod
    def get_reconciliation_for_date(
        reconciliation_date: date = None,
        organization_id: int = None,
    ) -> EndOfDayReconciliation:
        """
        Get reconciliation for a specific date and organization.
        ...
        if reconciliation_date is None:
            reconciliation_date = timezone.now().date()
        qs = EndOfDayReconciliation.objects.filter(
            reconciliation_date=reconciliation_date
        )
        if organization_id:
