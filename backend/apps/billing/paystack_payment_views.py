...
class PaystackInitiateView(APIView):
    ...
    def post(self, request):
        ...
        payment_intent = PaymentIntent.objects.create(
            ...
            amount=amount,
            status="INITIALIZED",
            paystack_authorization_url=paystack_response.get("data", {}).get(
                "authorization_url"
            ),
...
def paystack_webhook_view(request):
    ...
    if not signature:
        return JsonResponse(
            {"error": "Missing X-Paystack-Signature header"}, status=400
        )
    ...
    # SaaS subscription lifecycle (recurring plans, renewals, failures)
    from apps.organizations.paystack_saas_webhook import handle_paystack_saas_event

    if handle_paystack_saas_event(event, data):
        return JsonResponse(
            {"message": f"SaaS subscription event {event} processed"}, status=200
        )

    if event != "charge.success":
        return JsonResponse({"message": f"Event {event} not processed"}, status=200)

    reference = data.get("reference")
    if not reference:
        return JsonResponse({"error": "Missing reference in webhook data"}, status=400)

    # SaaS initial checkout (SAAS-* references)
    from apps.organizations.saas_payment_handlers import (
        handle_paystack_webhook_charge_success,
    )

    if handle_paystack_webhook_charge_success(data):
        return JsonResponse(
            {"message": "SaaS subscription payment processed"}, status=200
        )

    # Visit billing PaymentIntent
    try:
        ...
        return JsonResponse(
            {"error": f"PaymentIntent not found for reference: {reference}"}, status=404
        )
        ...
        return JsonResponse(
            {
                ...
                "payment_intent_id": payment_intent.id,
                "payment_id": payment_intent.payment.id
                if payment_intent.payment
                else None,
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Paystack verification failed: {str(e)}"}, status=500
        )
    ...
    metadata_visit_id = paystack_service.extract_visit_id_from_metadata(
        paystack_response
    )
    if metadata_visit_id != payment_intent.visit_id:
        payment_intent.mark_as_failed(reason="Visit ID mismatch in Paystack metadata")
        return JsonResponse(
            {"error": "Security check failed: Visit ID mismatch"}, status=400
        )
    ...
    if bill.is_insurance_backed:
        payment_intent.mark_as_failed(
            reason="Cannot process Paystack payment for insurance-backed bill"
        )
        return JsonResponse(
    ...
    try:
        ...
        if "data" in paystack_response:
            ...
            if "customer" in data:
                payment_intent.paystack_customer_email = data["customer"].get(
                    "email", ""
                )
        ...
        return JsonResponse(
            {"error": f"Failed to update payment intent: {str(e)}"}, status=500
        )
