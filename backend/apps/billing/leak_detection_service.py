...
class LeakDetectionService:
    ...
    def detect_lab_result_leak(lab_result_id: int) -> Optional[LeakRecord]:
        ...
        try:
            lab_result = LabResult.objects.select_related(
                "lab_order", "lab_order__visit", "lab_order__consultation"
        ...
        existing_leak = LeakRecord.objects.filter(
            entity_type="LAB_RESULT", entity_id=lab_result_id, resolved_at__isnull=True
        ).first()
        ...
        if billing_items.exists():
            ...
            if isinstance(tests_requested, list) and tests_requested:
                ...
                service_catalog = ServiceCatalog.objects.filter(
                    workflow_type="LAB_ORDER", department="LAB", is_active=True
                ).first()
        ...
        leak = LeakRecord.objects.create(
            ...
            service_name=service_catalog.name
            if service_catalog
            else "Unknown Lab Test",
            estimated_amount=estimated_amount,
            visit=visit,
            detection_context={
                "lab_order_id": lab_result.lab_order.id,
                "tests_requested": tests_requested
                if tests_requested is not None
                else [],
        ...
        logger.info(
            f"Leak detected for LabResult {lab_result_id}: {estimated_amount} NGN"
        )
        return leak
    ...
    def detect_radiology_report_leak(radiology_request_id: int) -> Optional[LeakRecord]:
        ...
        if existing_leak:
            logger.debug(
                f"Leak already detected for RadiologyRequest {radiology_request_id}"
            )
            return existing_leak
        ...
        if paid_bills.exists():
            # No leak - bill exists and is paid
            logger.debug(
                f"No leak for RadiologyRequest {radiology_request_id} - paid bill exists"
            )
            return None
        ...
        if billing_items.exists():
            ...
            # Try to find matching ServiceCatalog
            service_catalog = ServiceCatalog.objects.filter(
                workflow_type="RADIOLOGY_STUDY", department="RADIOLOGY", is_active=True
            ).first()
        ...
        logger.info(
            f"Leak detected for RadiologyRequest {radiology_request_id}: {estimated_amount} NGN"
        )
        return leak
    ...
    def detect_drug_dispense_leak(prescription_id: int) -> Optional[LeakRecord]:
        ...
        if prescription.is_emergency:
            logger.debug(
                f"Prescription {prescription_id} is emergency - excluded from leak detection"
            )
            return None
        ...
        if paid_bills.exists():
            # No leak - bill exists and is paid
            logger.debug(
                f"No leak for Prescription {prescription_id} - paid bill exists"
            )
            return None
        ...
        if billing_items.exists():
            ...
            # Try to find matching ServiceCatalog
            service_catalog = ServiceCatalog.objects.filter(
                workflow_type="DRUG_DISPENSE", department="PHARMACY", is_active=True
            ).first()
        ...
        logger.info(
            f"Leak detected for Prescription {prescription_id}: {estimated_amount} NGN"
        )
        return leak
    ...
    def detect_procedure_leak(procedure_task_id: int) -> Optional[LeakRecord]:
        ...
        estimated_amount = (
            service_catalog.amount if service_catalog else Decimal("5000.00")
        )

        # Check if there's a PAID BillingLineItem
        billing_items = BillingLineItem.objects.filter(
            visit=visit, service_catalog=service_catalog, bill_status="PAID"
        )

        if billing_items.exists():
            # No leak - bill exists and is paid
            logger.debug(
                f"No leak for ProcedureTask {procedure_task_id} - paid bill exists"
            )
            return None
        ...
        leak = LeakRecord.objects.create(
            ...
            service_name=service_catalog.name
            if service_catalog
            else procedure_task.procedure_name,
            estimated_amount=estimated_amount,
            visit=visit,
            detection_context={
        ...
        logger.info(
            f"Leak detected for ProcedureTask {procedure_task_id}: {estimated_amount} NGN"
        )
        return leak

    @staticmethod
    def detect_all_leaks(organization_id: int = None):
        """
        ...
        Args:
            organization_id: Optional - scope by organization (tenant)
        ...
        lab_results = (
            LabResult.objects.select_related("lab_order", "lab_order__visit").filter(
                lab_order__visit__organization_id=organization_id
            )
            if organization_id
            else LabResult.objects.select_related("lab_order", "lab_order__visit").all()
        )
        for lab_result in lab_results:
            try:
                ...
                logger.warning(
                    f"Error detecting leak for LabResult {lab_result.id}: {e}"
                )
                continue
        ...
        radiology_requests = (
            RadiologyRequest.objects.select_related("visit", "consultation")
            .exclude(report__isnull=True)
            .exclude(report="")
        )
        if organization_id:
            radiology_requests = radiology_requests.filter(
                visit__organization_id=organization_id
            )
        for radiology_request in radiology_requests:
            try:
                leak = LeakDetectionService.detect_radiology_report_leak(
                ...
                logger.warning(
                    f"Error detecting leak for RadiologyRequest {radiology_request.id}: {e}"
                )
                continue
        ...
        prescriptions = Prescription.objects.select_related(
            "visit", "consultation"
        ).filter(dispensed=True, is_emergency=False)
        if organization_id:
            prescriptions = prescriptions.filter(visit__organization_id=organization_id)
        for prescription in prescriptions:
            try:
                ...
                logger.warning(
                    f"Error detecting leak for Prescription {prescription.id}: {e}"
                )
                continue
        ...
        if organization_id:
            procedure_tasks = procedure_tasks.filter(
                visit__organization_id=organization_id
            )
        for procedure_task in procedure_tasks:
            try:
                ...
                logger.warning(
                    f"Error detecting leak for ProcedureTask {procedure_task.id}: {e}"
                )
                continue
    ...
    def get_daily_aggregation(date=None):
        ...
        aggregation = leaks.values("entity_type").annotate(
            count=Count("id"), total_amount=Sum("estimated_amount")
        )
        ...
        return {
            ...
            "total_estimated_loss": leaks.aggregate(Sum("estimated_amount"))[
                "estimated_amount__sum"
            ]
            or Decimal("0.00"),
            "unresolved": {
