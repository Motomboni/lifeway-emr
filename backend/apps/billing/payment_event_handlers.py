...
def handle_payment_confirmed(event: PaymentConfirmedEvent) -> None:
    ...
    with transaction.atomic():
        ...
        try:
            ...
            logger.error(
                f"Visit {event.visit_id} not found for payment confirmed event"
            )
...
def _unlock_consultation(consultation_id: int, visit: Visit) -> None:
    ...
    try:
        consultation = Consultation.objects.select_for_update().get(
            pk=consultation_id, visit=visit
        )
        ...
        logger.error(
            f"Error unlocking consultation {consultation_id}: {str(e)}", exc_info=True
        )
...
def handle_payment_confirmed_idempotent(event: PaymentConfirmedEvent) -> None:
    ...
    try:
        ...
        raise ValidationError(f"BillingLineItem {event.billing_line_item_id} not found")
