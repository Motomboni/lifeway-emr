        ...
        try:
            ...
            try:
                ...
                raise DRFValidationError(
                    f"visit_id must be a valid integer. Got: {visit_id}"
                )
            ...
            is_consultation_service = (
                ...
                or service.department == "CONSULTATION"
                or service.service_code.upper().startswith("CONS-")
                or "FOLLOW UP" in (service.name or "").upper()
                or "FOLLOW-UP" in (service.name or "").upper()
                or "FOLLOWUP" in (service.name or "").upper()
                or "CONSULTATION" in (service.name or "").upper()
                or "CONSULT" in (service.name or "").upper()
            )
            ...
            if is_consultation_service:
                ...
                try:
                    ...
                    if not consultation:
                        ...
                        if service.workflow_type == "GOPD_CONSULT":
                            try:
                                _, consultation, consultation_created = (
                                    initiate_gopd_consultation_workflow(
                ...
                if service.requires_consultation:
                    ...
                    if not consultation:
                        consultation = Consultation.objects.filter(
                            visit=visit, status__in=["PENDING", "ACTIVE"]
                        ).first()
                    ...
                    if not consultation:
                        ...
                        # If consultation is PENDING and no doctor assigned, try to assign current user if they're a doctor
                            ...
                            consultation.status == "PENDING"
                            and not consultation.created_by
            ...
            try:
                ...
                AuditLog.log(
                    ...
                    resource_id=getattr(domain_object, "id", None)
                    if domain_object
                    else None,
                    request=request,
                    metadata={
            ...
            return Response(
                {
                    ...
                    "amount": str(service.amount),
                    "workflow_type": service.workflow_type,
                    "domain_object_id": getattr(domain_object, "id", None)
                    if domain_object
                    else None,
                    "domain_object_type": type(domain_object).__name__
                    if domain_object
                    ...
                    "created_at": billing_line_item.created_at.isoformat()
                    if billing_line_item and hasattr(billing_line_item, "created_at")
                    else None,
                },
    ...
    def get(self, request):
        ...
        try:
            services = ServicePriceListManager.list_services(
                department=department, active_only=active_only
            )
        except ValidationError as e:
            raise DRFValidationError(str(e))
