...
class PDFService:
    ...
    def generate_receipt_pdf(
        receipt_data: Dict[str, Any], format_type: str = "a4"
    ) -> bytes:
        """
    ...
    def generate_invoice_pdf(
        invoice_data: Dict[str, Any], format_type: str = "a4"
    ) -> bytes:
        """
    ...
    def _get_logo_path() -> Optional[str]:
        ...
        if getattr(settings, "DEBUG", False):
            _logger.debug(
                "Looking for logo. Trying %s paths... Current file: %s BASE_DIR: %s",
                len(unique_paths),
                current_file,
                BASE_DIR,
            )
        ...
        for path in unique_paths:
            ...
            if getattr(settings, "DEBUG", False):
                _logger.debug(
                    "Checking: %s -> %s", path, "EXISTS" if exists else "NOT FOUND"
                )
            if exists:
                try:
    ...
    def _get_logo_base64() -> Optional[str]:
        ...
        if not logo_path:
            if getattr(settings, "DEBUG", False):
                _logger.debug(
                    "_get_logo_base64: No logo path returned from _get_logo_path()"
                )
            return None

        if getattr(settings, "DEBUG", False):
            _logger.debug(
                "_get_logo_base64: Converting logo to base64 from: %s", logo_path
            )
        ...
        try:
            with open(logo_path, "rb") as f:
                ...
                if getattr(settings, "DEBUG", False):
                    _logger.debug(
                        "_get_logo_base64: Read %s bytes from logo file", len(logo_data)
                    )

                if len(logo_data) == 0:
                    if getattr(settings, "DEBUG", False):
                        _logger.debug(
                            "_get_logo_base64: Logo file is empty: %s", logo_path
                        )
                    return None
                ...
                if logo_path.lower().endswith(".png"):
                    ...
                    return result
                elif logo_path.lower().endswith(".jpg") or logo_path.lower().endswith(
                    ".jpeg"
                ):
            ...
            _logger.warning(
                "_get_logo_base64: Permission denied reading: %s", logo_path
            )
            return None
        except Exception as e:
