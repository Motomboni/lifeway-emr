...
from .invoice_receipt_models import DocumentNumberSequence, InvoiceReceipt
...
class ReceiptService:
    ...
    def generate_receipt(visit, payment: Optional[Payment] = None) -> Dict[str, Any]:
            ...
            payments_data = [
                {
                    ...
                    or str(payment.processed_by),
                    "created_at": payment.created_at.isoformat(),
                }
            ]
        else:
            payments = Payment.objects.filter(visit=visit, status="CLEARED").order_by(
                "created_at"
            )
            payments_data = [
                {
                    ...
                    "notes": p.notes,
                    "processed_by": p.processed_by.get_full_name()
                    or str(p.processed_by),
        ...
        receipt_dict = {
            ...
            "total_paid": str(
                billing_summary.total_payments + billing_summary.total_wallet_debits
            ),
            "outstanding_balance": str(billing_summary.outstanding_balance),
            "payment_status": billing_summary.payment_status,
            "clinic_name": getattr(
                settings, "CLINIC_NAME", "Lifeway Medical Centre Ltd"
            ),
            "clinic_address": getattr(
                settings, "CLINIC_ADDRESS", "Plot 1593, ZONE E, APO RESETTLEMENT, ABUJA"
            ),
            "clinic_phone": getattr(
                settings, "CLINIC_PHONE", "07058893439, 08033145080, 08033114417"
            ),
    ...
    @staticmethod
    def generate_receipt_with_pdf(
        visit, payment: Optional[Payment] = None, user=None, format_type: str = "a4"
    ) -> tuple:
        """
        ...
        try:
            ...
            pdf_bytes = PDFService.generate_receipt_pdf(
                receipt_data, format_type=format_type
            )
    ...
    def generate_invoice(visit) -> Dict[str, Any]:
        ...
        """
        try:
            insurance = VisitInsurance.objects.select_related("provider").get(
                visit=visit
            )
        ...
        policy_num = getattr(insurance, "policy_number", "") or getattr(
        ...
        invoice_dict = {
            ...
            "approved_amount": str(insurance.approved_amount)
            if insurance.approved_amount
            else None,
    ...
    def generate_invoice_with_pdf(visit, user=None, format_type: str = "a4") -> tuple:
        ...
        try:
            ...
            pdf_bytes = PDFService.generate_invoice_pdf(
                invoice_data, format_type=format_type
            )
        except (ImportError, Exception) as e:
            # WeasyPrint not available or error, return empty bytes
            import logging
    ...
    def generate_billing_statement(visit) -> Dict[str, Any]:
        ...
        wallet_transactions = WalletTransaction.objects.filter(
            visit=visit, transaction_type="DEBIT"
        ).order_by("created_at")
        wallet_data = [
            {
                "id": wt.id,
        ...
        return {
            ...
            **self._clinic_branding_for_visit(visit),
        }

    @staticmethod
    def _clinic_branding_for_visit(visit):
        from core.org_branding import get_org_branding_for_visit

        branding = get_org_branding_for_visit(visit)
        return {
            "clinic_name": branding["clinic_name"],
            "clinic_address": branding["clinic_address"],
            "clinic_phone": branding["clinic_phone"],
            "clinic_email": branding["clinic_email"],
