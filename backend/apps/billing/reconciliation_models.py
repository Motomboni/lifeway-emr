...
class EndOfDayReconciliation(models.Model):
    ...
    # Organization (tenant) - one reconciliation per org per day
    ...
    outstanding_visits_count = models.PositiveIntegerField(
        default=0, help_text="Number of visits with outstanding balances"
    )

    # Revenue leaks
    revenue_leaks_detected = models.PositiveIntegerField(
        default=0, help_text="Number of revenue leaks detected"
    )
    ...
    total_visits = models.PositiveIntegerField(
        default=0, help_text="Total number of visits for the day"
    )

    active_visits_closed = models.PositiveIntegerField(
        default=0, help_text="Number of active visits closed during reconciliation"
    )

    # Mismatches
    has_mismatches = models.BooleanField(
        default=False, help_text="Whether there are payment mismatches"
    )

    mismatch_details = models.JSONField(
        default=dict, blank=True, help_text="Details of payment mismatches"
    )
    ...
    prepared_at = models.DateTimeField(
        auto_now_add=True, help_text="When the reconciliation was prepared"
    )

    reviewed_at = models.DateTimeField(
        null=True, blank=True, help_text="When the reconciliation was reviewed"
    )

    finalized_at = models.DateTimeField(
        null=True, blank=True, help_text="When the reconciliation was finalized"
    )

    # Notes
    notes = models.TextField(
        blank=True, help_text="Additional notes about the reconciliation"
    )

    # Reconciliation details (JSON)
    reconciliation_details = models.JSONField(
        default=dict, blank=True, help_text="Detailed breakdown of reconciliation"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["organization", "reconciliation_date"]),
        ]
        constraints = [
    ...
    def clean(self):
            ...
            if old_instance.status == "FINALIZED":
                ...
                for field in self._meta.fields:
                    ...
                    if getattr(self, field.name) != getattr(old_instance, field.name):
                        errors[field.name] = (
                            f"Cannot modify {field.name} after finalization."
                        )
        ...
        calculated_total = (
            self.total_cash + self.total_wallet + self.total_paystack + self.total_hmo
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "prepared_by": self.prepared_by.get_full_name()
            if self.prepared_by
            else None,
            "finalized_by": self.finalized_by.get_full_name()
            if self.finalized_by
            else None,
