...
class ServiceCatalogViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Multi-tenant: scope to current organization (or global if no org)."""
        qs = ServiceCatalog.objects.all()
        if hasattr(self.request, "organization") and self.request.organization:
            # Show org-specific services + global (organization=null) templates
            from django.db.models import Q
    ...
    def departments(self, request):
        ...
        for dept in departments:
            ...
            result.append(
                {
                    "code": dept_name,
                    "name": dict(ServiceCatalog.DEPARTMENT_CHOICES).get(
                        dept_name, dept_name
                    ),
                    "count": count,
                }
        ...
        return Response(
            {
                "departments": [
                    {"value": c[0], "label": c[1]}
                    for c in ServiceCatalog.DEPARTMENT_CHOICES
                ],
                "categories": [
                    {"value": c[0], "label": c[1]}
                    for c in ServiceCatalog.CATEGORY_CHOICES
                ],
                "workflow_types": [
                    {"value": c[0], "label": c[1]}
                    for c in ServiceCatalog.WORKFLOW_TYPE_CHOICES
                ],
                "bill_timing": [
                    {"value": c[0], "label": c[1]}
                    for c in ServiceCatalog.BILL_TIMING_CHOICES
                ],
            }
        )
    ...
    def _parse_request_data(self, data, instance=None):
        ...
        department = (
            (data.get("department") or (instance.department if instance else ""))
            .strip()
            .upper()
        )
        service_code = (
            data.get("service_code") or (instance.service_code if instance else "")
        ).strip()
        ...
        ).strip().upper() or self._department_default_category(department)
        workflow_type = (
            (
                data.get("workflow_type")
                or (instance.workflow_type if instance else "OTHER")
            )
            .strip()
            .upper()
        )
        ...
        return {
            ...
            "description": (
                ...
                or (instance.description if instance else "")
                or ""
            )[:5000],
            "category": category,
            "workflow_type": workflow_type,
            "requires_visit": data.get(
                "requires_visit", True if not instance else instance.requires_visit
            ),
            "requires_consultation": data.get(
                "requires_consultation",
                False if not instance else instance.requires_consultation,
            ),
            "auto_bill": data.get(
                "auto_bill", True if not instance else instance.auto_bill
            ),
            "bill_timing": (
                data.get("bill_timing")
                or (instance.bill_timing if instance else "AFTER")
            ).strip()
            or "AFTER",
            "restricted_service_flag": data.get(
                "restricted_service_flag",
                False if not instance else instance.restricted_service_flag,
            ),
            "allowed_roles": allowed_roles,
            "is_active": data.get(
                "is_active", True if not instance else instance.is_active
            ),
        }
    ...
    def create(self, request):
        ...
        try:
            ...
            return Response(
                ServiceCatalogSerializer.serialize(service),
    ...
    def update(self, request, pk=None):
        ...
        try:
            ...
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
        other = (
            ServiceCatalog.objects.filter(service_code=data["service_code"])
            .exclude(pk=pk)
            .first()
        )
        if other:
