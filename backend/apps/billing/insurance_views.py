...
class VisitInsuranceViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        insurance = serializer.save(visit=visit, created_by=self.request.user)
    ...
    def perform_update(self, serializer):
        ...
        # Pre-auth workflow timestamps
        if "pre_auth_status" in serializer.validated_data:
            new_status = serializer.validated_data["pre_auth_status"]
            old_status = insurance.pre_auth_status
            if new_status == "PENDING" and old_status != "PENDING":
                serializer.validated_data["pre_auth_requested_at"] = timezone.now()
            if new_status in ("APPROVED", "REJECTED") and old_status != new_status:
                serializer.validated_data["pre_auth_decided_at"] = timezone.now()
