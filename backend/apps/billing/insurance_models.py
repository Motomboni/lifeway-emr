...
class HMOProvider(models.Model):
    ...
    name = models.CharField(
        max_length=255, unique=True, help_text="Name of the HMO/Insurance provider"
    )
...
class VisitInsurance(models.Model):
    ...
    policy_number = models.CharField(
        max_length=100, help_text="Patient's insurance policy number"
    )
    ...
    billing_scheme = models.CharField(
        max_length=20,
        choices=BILLING_SCHEME_CHOICES,
        default="FEE_FOR_SERVICE",
        help_text="HMO billing scheme: fee-for-service, capitation, or retainer",
    )

    pre_auth_status = models.CharField(
        max_length=20,
        choices=PRE_AUTH_STATUS_CHOICES,
        default="NOT_REQUIRED",
        help_text="Pre-authorization status for this visit",
    )

    pre_auth_number = models.CharField(
        max_length=128,
        blank=True,
        help_text="HMO pre-authorization reference number",
    )

    pre_auth_requested_at = models.DateTimeField(null=True, blank=True)
    pre_auth_decided_at = models.DateTimeField(null=True, blank=True)
    pre_auth_notes = models.TextField(blank=True)

    employer_name = models.CharField(
        max_length=255,
        blank=True,
        help_text="Employer or retainer organization name",
    )
    ...
    approval_reference = models.CharField(
        max_length=255, blank=True, help_text="Insurance approval reference number"
    )

    approval_date = models.DateTimeField(
        null=True, blank=True, help_text="Date of insurance approval"
    )
    ...
    notes = models.TextField(
        blank=True, help_text="Additional notes about insurance coverage"
    )
    ...
    def clean(self):
        ...
        if self.coverage_type == "FULL" and self.coverage_percentage != 100:
            raise ValidationError("FULL coverage must have coverage_percentage = 100.")
...
class ClaimPolicy(models.Model):
    ...
    coverage_details = models.JSONField(
        default=dict, blank=True, help_text="Coverage limits, type, etc."
    )
...
class Claim(models.Model):
    ...
    response_payload = models.JSONField(
        default=dict, blank=True, help_text="Insurer response (stub)"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
