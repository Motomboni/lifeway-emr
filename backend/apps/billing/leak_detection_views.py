...
class LeakRecordViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            super()
            .get_queryset()
            .select_related("visit", "visit__patient", "resolved_by")
        )
        if hasattr(self.request, "organization") and self.request.organization:
            queryset = queryset.filter(visit__organization=self.request.organization)
    ...
    def resolve(self, request, pk=None):
        ...
        return Response(LeakRecordSerializer(leak).data, status=status.HTTP_200_OK)
    ...
    def detect_all(self, request):
        ...
        return Response(
            {"message": "Leak detection completed", "results": results},
    ...
    def summary(self, request):
        ...
        by_entity_type = LeakRecord.objects.values("entity_type").annotate(
            ...
            unresolved_amount=Sum(
                "estimated_amount", filter=Q(resolved_at__isnull=True)
            ),
        )
