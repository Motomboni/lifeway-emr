...
class ReceiptView(APIView):
    ...
    def get(self, request, visit_id):
        ...
        pdf_format = request.query_params.get(
            "pdf_format", "a4"
        ).lower()  # 'a4' or 'pos'
        ...
        if format_type == "pdf":
            ...
            response["Content-Disposition"] = (
                f'inline; filename="receipt_{receipt_data["receipt_number"]}.pdf"'
            )
            return response
        else:
            # Return JSON
    ...
    def get(self, request, visit_id):
        ...
        try:
            if format_type == "pdf":
                ...
                response = HttpResponse(pdf_bytes, content_type="application/pdf")
                response["Content-Disposition"] = (
                    f'inline; filename="invoice_{invoice_data["invoice_number"]}.pdf"'
                )
                return response
            else:
                # Return JSON
...
def test_logo_view(request):
    ...
    if BASE_DIR:
        ...
        possible_paths.append(
            str(Path(BASE_DIR).parent / "frontend" / "public" / "logo.png")
        )
    ...
    return Response(
        {
            ...
            if logo_base64
            else "Logo not found - check paths above",
        },
