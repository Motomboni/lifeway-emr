...
class InvoiceReceipt(models.Model):
    ...
    qr_code_data = models.TextField(
        blank=True, help_text="QR code data for document verification"
    )
    ...
    generated_at = models.DateTimeField(
        auto_now_add=True, help_text="When this document was generated"
    )

    # Email/SMS tracking
    emailed_to = models.EmailField(
        blank=True, null=True, help_text="Email address document was sent to"
    )

    sms_sent_to = models.CharField(
        max_length=20, blank=True, null=True, help_text="Phone number SMS was sent to"
    )

    emailed_at = models.DateTimeField(
        null=True, blank=True, help_text="When document was emailed"
    )

    sms_sent_at = models.DateTimeField(
        null=True, blank=True, help_text="When SMS was sent"
    )
    ...
    def delete(self, *args, **kwargs):
        ...
        raise ValidationError(
            "Invoice/Receipt records cannot be deleted. They are append-only for audit purposes."
        )
...
class DocumentNumberSequence(models.Model):
    ...
    prefix = models.CharField(
        max_length=10, help_text="Prefix for document number (e.g., REC, INV)"
    )
