    ...
    def post(self, request):
        if "file" not in request.FILES:
            raise DRFValidationError(
                "No file uploaded. Please provide an Excel (.xlsx, .xls) or CSV (.csv) file."
            )
        ...
        try:
            ...
            if name.endswith(".csv"):
                try:
                    df = pd.read_csv(
                        BytesIO(content), encoding="utf-8", sep=None, engine="python"
                    )
                except UnicodeDecodeError:
                    df = pd.read_csv(
                        BytesIO(content), encoding="latin-1", sep=None, engine="python"
                    )
            else:
