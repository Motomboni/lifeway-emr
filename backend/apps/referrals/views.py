...
from .permissions import CanCreateReferral, CanUpdateReferral, CanViewReferral
...
def log_referral_action(
    user, action, visit_id, referral_id=None, request=None, metadata=None
):
    """
...
class ReferralViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Referral.objects.filter(visit_id=visit_id).select_related(
            "visit", "consultation", "referred_by"
        )
    ...
    def perform_create(self, serializer):
        ...
        if visit.status != "OPEN":
            raise DRFValidationError("Referrals can only be created for OPEN visits.")
