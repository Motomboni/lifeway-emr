...
class Referral(models.Model):
    ...
    specialty = models.CharField(
        max_length=50, choices=SPECIALTY_CHOICES, help_text="Specialty to refer to"
    )

    specialist_name = models.CharField(
        max_length=200, help_text="Name of the specialist or clinic"
    )

    specialist_contact = models.CharField(
        max_length=200, blank=True, help_text="Contact information for the specialist"
    )

    reason = models.TextField(help_text="Reason for referral")

    clinical_summary = models.TextField(
        blank=True, help_text="Clinical summary for the specialist"
    )
    ...
    accepted_at = models.DateTimeField(
        null=True, blank=True, help_text="When the referral was accepted"
    )

    completed_at = models.DateTimeField(
        null=True, blank=True, help_text="When the referral was completed"
    )

    specialist_notes = models.TextField(
        blank=True, help_text="Notes from the specialist"
    )

    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the referral was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the referral was last updated"
    )
    ...
    def clean(self):
        ...
        if self.visit and self.visit.status == "CLOSED":
            if not self.pk:  # New referral
                raise ValidationError("Cannot create referral for a CLOSED visit.")
