...
class Appointment(models.Model):
    ...
    duration_minutes = models.IntegerField(
        default=30, help_text="Duration of appointment in minutes"
    )
    ...
    reason = models.TextField(
        blank=True, help_text="Reason for appointment or chief complaint"
    )
    ...
    cancelled_at = models.DateTimeField(
        null=True, blank=True, help_text="When the appointment was cancelled"
    )
        ...
        if self.visit_id and self.patient_id:
            if self.visit.patient_id != self.patient_id:
                raise ValidationError(
                    "Visit must belong to the same patient as the appointment."
                )
