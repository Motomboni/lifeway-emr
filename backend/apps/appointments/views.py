...
Enforcement:
1. Receptionist: Can create/manage all appointments (including those scheduled by doctors)
2. Doctor: Can create appointments for themselves; view/update their own appointments
...
from .permissions import CanCreateAppointments, CanManageAppointments
...
def log_appointment_action(user, action, appointment_id, request=None, metadata=None):
    """
...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        Receptionist: See all appointments (in current org)
        Doctor: See only their own appointments (in current org)
        Multi-tenant: filter by patient.organization
        """
        queryset = super().get_queryset()

        # Multi-tenant: scope to current organization
        if hasattr(self.request, "organization") and self.request.organization:
            queryset = queryset.filter(patient__organization=self.request.organization)
    ...
    def upcoming(self, request):
        ...
        queryset = self.get_queryset().filter(
            appointment_date__gte=timezone.now(), status__in=["SCHEDULED", "CONFIRMED"]
        )
    ...
    def today(self, request):
        ...
        queryset = self.get_queryset().filter(
            appointment_date__gte=today_start, appointment_date__lte=today_end
        )
