...
class MedicalDocument(models.Model):
    ...
    document_type = models.CharField(
        max_length=50, choices=DOCUMENT_TYPES, help_text="Type of medical document"
    )

    title = models.CharField(max_length=200, help_text="Document title/name")

    description = models.TextField(
        blank=True, help_text="Document description or notes"
    )
    ...
    deleted_at = models.DateTimeField(
        null=True, blank=True, help_text="When document was soft-deleted"
    )
    ...
    def __str__(self):
        return (
            f"{self.title} ({self.get_document_type_display()}) - Visit {self.visit_id}"
        )
