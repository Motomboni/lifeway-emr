...
def restore_from_backup(restore_instance):
    ...
    for item in data:
        model_name = (
            item.get("model", "").split(".")[-1] if "." in item.get("model", "") else ""
        )
        ...
        if model_name in ["patient"] and restore_instance.restore_patients:
            ...
            model_name in ["laborder", "labresult"]
            and restore_instance.restore_lab_data
        ):
            should_restore = True
        elif (
            model_name in ["radiologyorder", "radiologyresult"]
            and restore_instance.restore_radiology_data
        ):
            should_restore = True
        elif (
            model_name in ["prescription", "drug", "druginventory", "stockmovement"]
            and restore_instance.restore_prescriptions
        ):
            should_restore = True
