...
from .tasks import copy_backup_offsite


def log_backup_action(user, action, backup_id, request=None, metadata=None):
    """
...
class BackupViewSet(viewsets.ModelViewSet):
    ...
    def download(self, request, pk=None):
        ...
        response = FileResponse(
            open(backup.file_path, "rb"), content_type="application/json"
        )
        response["Content-Disposition"] = (
            f'attachment; filename="backup_{backup.id}.json"'
        )
        return response
...
class RestoreViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        restore = serializer.save(created_by=self.request.user, status="PENDING")
