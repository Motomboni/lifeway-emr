...
class Backup(models.Model):
    ...
    backup_type = models.CharField(
        max_length=20, choices=BACKUP_TYPES, default="FULL", help_text="Type of backup"
    )
    ...
    file_size = models.BigIntegerField(
        null=True, blank=True, help_text="Size of backup file in bytes"
    )
    ...
    includes_patients = models.BooleanField(
        default=True, help_text="Whether backup includes patient data"
    )

    includes_visits = models.BooleanField(
        default=True, help_text="Whether backup includes visit data"
    )

    includes_consultations = models.BooleanField(
        default=True, help_text="Whether backup includes consultation data"
    )

    includes_lab_data = models.BooleanField(
        default=True, help_text="Whether backup includes lab data"
    )

    includes_radiology_data = models.BooleanField(
        default=True, help_text="Whether backup includes radiology data"
    )

    includes_prescriptions = models.BooleanField(
        default=True, help_text="Whether backup includes prescription data"
    )

    includes_audit_logs = models.BooleanField(
        default=True, help_text="Whether backup includes audit logs"
    )

    # Metadata
    description = models.TextField(
        blank=True, help_text="Description or notes about this backup"
    )
    ...
    started_at = models.DateTimeField(
        null=True, blank=True, help_text="When the backup started"
    )

    completed_at = models.DateTimeField(
        null=True, blank=True, help_text="When the backup completed"
    )

    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the backup record was created"
    )
...
class Restore(models.Model):
    ...
    restore_patients = models.BooleanField(
        default=True, help_text="Whether to restore patient data"
    )

    restore_visits = models.BooleanField(
        default=True, help_text="Whether to restore visit data"
    )

    restore_consultations = models.BooleanField(
        default=True, help_text="Whether to restore consultation data"
    )

    restore_lab_data = models.BooleanField(
        default=True, help_text="Whether to restore lab data"
    )

    restore_radiology_data = models.BooleanField(
        default=True, help_text="Whether to restore radiology data"
    )

    restore_prescriptions = models.BooleanField(
        default=True, help_text="Whether to restore prescription data"
    )
    ...
    # Metadata
    description = models.TextField(
        blank=True, help_text="Description or notes about this restore"
    )

    error_message = models.TextField(
        blank=True, help_text="Error message if restore failed"
    )
    ...
    # Timestamps
    started_at = models.DateTimeField(
        null=True, blank=True, help_text="When the restore started"
    )

    completed_at = models.DateTimeField(
        null=True, blank=True, help_text="When the restore completed"
    )

    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the restore record was created"
    )
