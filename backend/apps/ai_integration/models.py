...
class AIRequest(models.Model):
    ...
    user_role = models.CharField(
        max_length=50, help_text="Role of the user at time of request"
    )
    ...
    provider = models.CharField(
        max_length=50, choices=AIProvider.choices, help_text="AI provider used"
    )
    model_name = models.CharField(
        max_length=200, help_text="Specific model used (e.g., 'gpt-4', 'claude-3-opus')"
    )
    ...
    total_tokens = models.IntegerField(
        default=0, validators=[MinValueValidator(0)], help_text="Total tokens used"
    )
    ...
    request_payload = models.JSONField(
        default=dict, blank=True, help_text="Request payload (sanitized, no PHI)"
    )
    response_payload = models.JSONField(
        default=dict, blank=True, help_text="Response payload (sanitized, no PHI)"
    )
    success = models.BooleanField(
        default=True, help_text="Whether the request was successful"
    )
    ...
    timestamp = models.DateTimeField(
        default=timezone.now, db_index=True, help_text="When the request was made"
    )
...
class AIConfiguration(models.Model):
    ...
    default_model = models.CharField(
        max_length=200, help_text="Default model for this feature"
    )
    enabled = models.BooleanField(
        default=True, help_text="Whether this feature is enabled"
    )
    ...
    configuration = models.JSONField(
        default=dict, blank=True, help_text="Additional provider-specific configuration"
    )
...
class AICache(models.Model):
    ...
    feature_type = models.CharField(
        max_length=50, choices=AIFeatureType.choices, db_index=True
    )
    prompt_hash = models.CharField(
        max_length=64, db_index=True, help_text="SHA-256 hash of the prompt"
    )
    response = models.JSONField(help_text="Cached AI response")
    expires_at = models.DateTimeField(
        db_index=True, help_text="When this cache entry expires"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    hit_count = models.IntegerField(
        default=0, help_text="Number of times this cache was hit"
    )
