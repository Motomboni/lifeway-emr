...
from .models import AIConfiguration, AIRequest, ClinicalNote
...
class ClinicalDecisionSupportResponseSerializer(serializers.Serializer):
    ...
    treatment_suggestions = serializers.ListField(
        child=serializers.DictField(), required=False, help_text="Treatment suggestions"
    )
...
class NLPSummarizationRequestSerializer(serializers.Serializer):
    ...
    summary_type = serializers.ChoiceField(
        choices=["brief", "detailed", "structured"], default="brief"
    )
...
class NLPSummarizationResponseSerializer(serializers.Serializer):
    ...
    key_points = serializers.ListField(
        child=serializers.CharField(), required=False, help_text="Key points extracted"
    )
    request_id = serializers.IntegerField(help_text="AI request ID for audit trail")
...
class AutomatedCodingResponseSerializer(serializers.Serializer):
    ...
    cpt_codes = serializers.ListField(
        child=serializers.DictField(), required=False, help_text="Suggested CPT codes"
    )
    request_id = serializers.IntegerField(help_text="AI request ID for audit trail")
...
class DrugInteractionCheckRequestSerializer(serializers.Serializer):
    ...
    new_medication = serializers.CharField(
        required=True, help_text="New medication to check"
    )
...
class DrugInteractionCheckResponseSerializer(serializers.Serializer):
    ...
    description = serializers.CharField(
        required=False, help_text="Description of interaction"
    )
    recommendations = serializers.ListField(
        child=serializers.CharField(), required=False, help_text="Recommendations"
    )
    request_id = serializers.IntegerField(help_text="AI request ID for audit trail")
...
class GenerateNoteResponseSerializer(serializers.Serializer):
    ...
    template_used = serializers.CharField(required=False)
    ...
    code_validation = serializers.DictField(required=False)
    parsed_sections = serializers.DictField(required=False)
    icd11_apply_payload = serializers.ListField(
