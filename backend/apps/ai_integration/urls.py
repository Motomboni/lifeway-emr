...
from .views import AIClinicalViewSet, AIRequestViewSet
...
ai_clinical_viewset = AIClinicalViewSet.as_view({"post": "clinical_decision_support"})

ai_nlp_viewset = AIClinicalViewSet.as_view({"post": "nlp_summarize"})

ai_coding_viewset = AIClinicalViewSet.as_view({"post": "automated_coding"})

ai_interaction_viewset = AIClinicalViewSet.as_view({"post": "drug_interaction_check"})
...
urlpatterns = [
    ...
    path(
        ...
        ai_interaction_viewset,
        name="ai-drug-interaction-check",
    ),
