"""
Organization and SaaS billing models for multi-tenant EMR.

Per SaaS Architecture:
- Organization = Tenant (clinic/hospital)
- OrganizationUser = User membership in organizations (many-to-many)
- Plan = Subscription tier (Starter, Professional, Enterprise)
- Subscription = Organization's active plan and billing status
"""
...
class Organization(models.Model):
    """
    Organization (Tenant) - represents a clinic/hospital in the SaaS.

    Each organization has isolated data. All tenant-scoped models
    (Patient, Visit, etc.) reference organization_id.
    """
...
class Plan(models.Model):
    """
    SaaS subscription plan (e.g., Starter, Professional, Enterprise).
    """
    ...
    # Legacy Stripe price IDs (only if SAAS_PAYMENT_PROVIDER=stripe)
    ...
    # Paystack recurring plan (Nigeria SaaS billing)
...
class Subscription(models.Model):
    ...
    # Legacy Stripe IDs (only if SAAS_PAYMENT_PROVIDER=stripe)
    ...
    # Paystack recurring subscription (Nigeria SaaS billing)
...
class SaasSubscriptionPayment(models.Model):
    """
    SaaS plan upgrade payment (Paystack / Flutterwave).
    Separate from visit-level PaymentIntent — no PHI in metadata.
    """
