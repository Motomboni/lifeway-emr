...
from apps.radiology.models import RadiologyResult
...
def log_patient_portal_action(user, action, patient_id, request=None, metadata=None):
    """
...
class PatientPortalViewSet(viewsets.ViewSet):
    ...
    def visits(self, request):
        ...
        visits = (
            ...
            .select_related("patient", "closed_by")
            .order_by("-created_at")
        )
    ...
    def lab_results(self, request):
        ...
        try:
            ...
            lab_results = (
                LabResult.objects.filter(lab_order__visit__patient=patient)
                .select_related(
                    "lab_order",
                    "lab_order__visit",
    ...
    def radiology_results(self, request):
        ...
        radiology_results = (
            RadiologyResult.objects.filter(radiology_order__visit__patient=patient)
            .select_related("radiology_order", "radiology_order__visit", "reported_by")
    ...
    def prescriptions(self, request):
        ...
        try:
            ...
            prescriptions = (
                Prescription.objects.filter(visit__patient=patient)
                .select_related("visit", "consultation", "prescribed_by")
                .order_by("-created_at")
            )
    ...
    @action(detail=False, methods=["get"], url_path="outstanding-bills")
    def outstanding_bills(self, request):
        """
        GET /api/v1/patient-portal/outstanding-bills/
        Visits with unpaid balance the patient can pay online (Paystack).
        """
        ...
        patient = get_patient_from_user(request.user)
        visits = Visit.objects.filter(patient=patient, status="OPEN").order_by(
            "-created_at"
        )
        bills = []
        for visit in visits:
            ...
            if outstanding and outstanding > Decimal("0"):
                bills.append(
                    {
                        "visit_id": visit.id,
        ...
        log_patient_portal_action(
            user=request.user,
            action="view_outstanding_bills",
            patient_id=patient.id,
            request=request,
        )
        return Response({"bills": bills, "count": len(bills)})

    @action(detail=False, methods=["post"], url_path="pay-bill")
    def pay_bill(self, request):
        """
        POST /api/v1/patient-portal/pay-bill/
        Body: { visit_id, callback_url? }
        ...
        try:
            visit = Visit.objects.get(pk=visit_id, patient=patient)
        except Visit.DoesNotExist:
            return Response(
        ...
        if visit.status != "OPEN":
            return Response(
                {"detail": "This visit is no longer open for payment."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not amount or amount <= Decimal("0"):
            return Response(
                {"detail": "No outstanding balance on this visit."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        reference = f"VISIT-{visit.id}-PT-{uuid.uuid4().hex[:12].upper()}"
        callback_url = request.data.get("callback_url") or (
            getattr(settings, "FRONTEND_URL", "http://localhost:3000")
            + "/patient-portal/dashboard?payment=success"
        )
        customer_email = patient.email or request.user.email

        try:
            paystack = PaystackVisitService()
        ...
        PaymentIntent.objects.create(
            ...
            paystack_authorization_url=data.get("authorization_url", ""),
            paystack_access_code=data.get("access_code", ""),
            paystack_customer_email=customer_email or "",
            status="INITIALIZED",
            created_by=request.user,
        )
        log_patient_portal_action(
            user=request.user,
            action="initiate_bill_payment",
            patient_id=patient.id,
            request=request,
            metadata={"visit_id": visit.id, "reference": reference},
        )
        return Response(
            {
    ...
    def medical_history(self, request):
        ...
        lab_results = (
            LabResult.objects.filter(lab_order__visit__patient=patient)
            .select_related("lab_order", "recorded_by")
            .order_by("-recorded_at")
        )
        radiology_results = (
            RadiologyResult.objects.filter(radiology_order__visit__patient=patient)
        ...
        prescriptions = (
            Prescription.objects.filter(visit__patient=patient)
            .select_related("visit", "prescribed_by")
            .order_by("-created_at")
        )
        ...
        return Response(
            {
                ...
                "radiology_results": RadiologyResultSerializer(
                    radiology_results, many=True
                ).data,
    ...
    @action(detail=False, methods=["get"], url_path="immunizations")
    def immunizations(self, request):
        """
        Get patient's own immunization schedule (read-only).

        GET /api/v1/patient-portal/immunizations/
        """
        from apps.clinical.immunization_models import ImmunizationRecord
        from apps.clinical.immunization_views import serialize_immunization_record

        patient = get_patient_from_user(request.user)
        records = ImmunizationRecord.objects.filter(patient=patient).order_by(
            "scheduled_date", "vaccine", "dose_number"
        )
        ...
        return Response([serialize_immunization_record(record) for record in records])
