...
class Patient(models.Model):
    ...
    first_name = models.CharField(max_length=255, help_text="Patient first name (PHI)")

    last_name = models.CharField(max_length=255, help_text="Patient last name (PHI)")

    middle_name = models.CharField(
        max_length=255, blank=True, null=True, help_text="Patient middle name (PHI)"
    )

    date_of_birth = models.DateField(
        null=True, blank=True, help_text="Patient date of birth (PHI)"
    )
    ...
    phone = models.CharField(
        max_length=20, blank=True, null=True, help_text="Patient phone number (PHI)"
    )

    email = models.EmailField(
        blank=True, null=True, help_text="Patient email address (PHI)"
    )

    address = models.TextField(blank=True, null=True, help_text="Patient address (PHI)")

    # Emergency Contact Information
    emergency_contact_name = models.CharField(
        max_length=255, blank=True, null=True, help_text="Emergency contact name (PHI)"
    )
    ...
    # Multi-tenancy: organization (clinic) this patient belongs to
    ...
    allergies = models.TextField(
        blank=True, null=True, help_text="Known allergies (PHI)"
    )

    medical_history = models.TextField(
        blank=True, null=True, help_text="Medical history summary (PHI)"
    )
    ...
    verified_at = models.DateTimeField(
        null=True, blank=True, help_text="Timestamp when patient account was verified"
    )
    ...
    has_retainership = models.BooleanField(
        default=False, help_text="Whether patient has an active retainership agreement"
    )
    ...
    retainership_start_date = models.DateField(
        null=True, blank=True, help_text="Retainership start date"
    )

    retainership_end_date = models.DateField(
        null=True, blank=True, help_text="Retainership end date (if applicable)"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When patient was registered"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When patient record was last updated"
    )
    ...
    @classmethod
    def generate_patient_id(cls, organization=None):
        """
        Generate the next sequential patient ID in format PREFIX000001.

        Uses organization.patient_id_prefix if provided, else 'LMC'.
        Scoped per organization when multi-tenant.

        Returns:
            str: Next available patient ID (e.g., LMC000001, ABC000001, etc.)
        """
        prefix = "LMC"
        if organization:
            prefix = (organization.patient_id_prefix or "LMC")[:10]

        qs = cls.objects
        if organization:
            qs = qs.filter(organization=organization)
        existing_ids = qs.filter(patient_id__startswith=prefix).values_list(
        ...
        for patient_id in existing_ids:
            try:
                numeric_part = patient_id[prefix_len:]
    ...
    def clean(self):
        ...
        if not self.patient_id:
            self.patient_id = self.generate_patient_id(organization=self.organization)
