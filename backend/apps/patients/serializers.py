...
class PatientSerializer(serializers.ModelSerializer):
    ...
    structured_allergies = serializers.SerializerMethodField()
    ...
    def get_has_active_insurance(self, obj):
        ...
        return active_insurance.valid_from <= today and (
            active_insurance.valid_to is None or active_insurance.valid_to >= today
        )

    def get_structured_allergies(self, obj):
        from .allergy_models import PatientAllergy

        return list(
            PatientAllergy.objects.filter(patient=obj, is_active=True)
...
class PatientCreateSerializer(PatientSerializer):
    ...
    insurance_provider_id = serializers.IntegerField(
    ...
    insurance_coverage_percentage = serializers.DecimalField(
        max_digits=5, decimal_places=2, required=False, allow_null=True, write_only=True
    )
    insurance_valid_from = serializers.DateField(
        required=False, allow_null=True, write_only=True
    )
    insurance_valid_to = serializers.DateField(
        required=False, allow_null=True, write_only=True
    )

    # Retainership fields (optional)
    has_retainership = serializers.BooleanField(
        required=False, default=False, write_only=True
    )
    retainership_type = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, write_only=True
    )
    retainership_start_date = serializers.DateField(
        required=False, allow_null=True, write_only=True
    )
    retainership_end_date = serializers.DateField(
        required=False, allow_null=True, write_only=True
    )
    ...
    def validate_national_id(self, value):
        ...
        if value:
            if (
                Patient.objects.filter(national_id=value)
    ...
    def validate(self, attrs):
        ...
        first_name = (
            attrs.get("first_name", "").strip() if attrs.get("first_name") else ""
        )
        ...
        if create_portal_account:
            portal_email = (
                attrs.get("portal_email", "").strip()
                if attrs.get("portal_email")
                else ""
            )
            if not portal_email:
                raise serializers.ValidationError(
    ...
    def create(self, validated_data):
        ...
        insurance_coverage_percentage = validated_data.pop(
        ...
        # Set organization from request context (multi-tenancy)
        request = self.context.get("request")
        if request and hasattr(request, "organization") and request.organization:
            org = request.organization
            validated_data["organization"] = org
            # Enforce plan limits
            from apps.organizations.utils import check_patient_limit

            check_patient_limit(org)
        ...
        try:
            with transaction.atomic():
                logger.info(
                    f"Creating patient with data: {list(validated_data.keys())}"
                )
                ...
                if create_portal_account and portal_email:
                    try:
                        ...
                        portal_created = True
                        logger.info(
                            f"Patient portal account created for patient {patient.id}, user {portal_user.id}"
                        )

                    except Exception as e:
                        logger.error(
                            f"Error creating portal account: {str(e)}", exc_info=True
                        )
            ...
            logger.error(
                f"Error in patient creation transaction: {str(e)}", exc_info=True
            )
            import traceback
        ...
        if has_insurance:
            if (
                not insurance_provider_id
                or not insurance_policy_number
                or not insurance_valid_from
            ):
                ...
                from apps.billing.bill_models import InsurancePolicy, InsuranceProvider
                ...
                try:
                    ...
                    InsurancePolicy.objects.create(
                        ...
                        coverage_percentage=Decimal(str(insurance_coverage_percentage))
                    ...
                    logger = logging.getLogger(__name__)
                    logger.error(
                        f"Error creating insurance policy: {str(e)}", exc_info=True
                    )
