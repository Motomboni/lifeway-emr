...
class PatientPortalAccess(permissions.BasePermission):
    ...
    message = (
        "Access denied. Patient portal access requires a verified patient account."
    )
    ...
    def has_permission(self, request, view):
        ...
        if user_role != "PATIENT":
            self.message = (
                "Access denied. Only patient accounts can access the patient portal."
            )
            return False
        ...
        if hasattr(patient, "is_verified") and not patient.is_verified:
            self.message = (
                "Access denied. Your patient account is pending verification."
            )
