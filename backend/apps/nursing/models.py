...
class NursingNote(models.Model):
    ...
    care_provided = models.TextField(
        blank=True, help_text="Description of nursing care provided"
    )

    patient_response = models.TextField(
        blank=True, help_text="Patient's response to care provided"
    )

    # Timestamps
    recorded_at = models.DateTimeField(
        auto_now_add=True, help_text="When the nursing note was recorded"
    )
...
class MedicationAdministration(models.Model):
    ...
    administration_time = models.DateTimeField(
        default=timezone.now, help_text="When the medication was actually administered"
    )

    dose_administered = models.CharField(
        max_length=255, help_text="Actual dose administered (e.g., '500mg', '1 tablet')"
    )
    ...
    # Timestamps
    recorded_at = models.DateTimeField(
        auto_now_add=True, help_text="When this administration record was created"
    )
        ...
        if self.visit_id and not self.pk:  # New administration
            ...
            if visit.status == "CLOSED":
                raise ValidationError(
                    "Cannot record medication administration for a CLOSED visit."
                )
...
class LabSampleCollection(models.Model):
    ...
    collection_time = models.DateTimeField(
        default=timezone.now, help_text="When the sample was actually collected"
    )
    ...
    # Timestamps
    recorded_at = models.DateTimeField(
        auto_now_add=True, help_text="When this collection record was created"
    )
    ...
    def clean(self):
        ...
        if self.visit_id and not self.pk:  # New collection
            ...
            if visit.status == "CLOSED":
                raise ValidationError(
                    "Cannot record sample collection for a CLOSED visit."
                )
        ...
        if self.status == "FAILED" and not self.reason_if_failed:
            raise ValidationError(
                "Reason is required when collection status is FAILED."
            )
