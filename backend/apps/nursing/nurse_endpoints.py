...
from apps.clinical.serializers import VitalSignsCreateSerializer, VitalSignsSerializer
from apps.visits.models import Visit

from core.tenant import get_org_scoped_visit

from .models import LabSampleCollection, MedicationAdministration, NursingNote
from .permissions import CanViewNursingRecords, IsNurse, IsVisitActiveAndPaid
...
class NurseVitalSignsEndpoint(viewsets.ViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            ...
            vital_signs = (
                VitalSigns.objects.filter(visit_id=visit.id)
                    ...
                    vital_signs_data.append(
                        {
                            "id": vs.id,
                            "visit": vs.visit_id,
                            "recorded_by": vs.recorded_by_id
                            if vs.recorded_by
                            else None,
                            "recorded_by_name": getattr(
                                vs.recorded_by, "username", "Unknown"
                            )
                            if vs.recorded_by
                            else "Unknown",
                            "temperature": str(vs.temperature)
                            if vs.temperature
                            else None,
                            ...
                            "oxygen_saturation": str(vs.oxygen_saturation)
            ...
            logger = logging.getLogger(__name__)
            logger.error(
                f"Error listing vital signs for visit {kwargs.get('visit_id')}: {str(e)}",
                exc_info=True,
            )
            return Response(
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            logger.info(
                f"Creating vital signs for visit {kwargs.get('visit_id')}, user: {request.user.username}"
            )
            ...
            vital_signs = serializer.save(visit=visit, recorded_by=request.user)
            ...
            try:
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"Failed to create clinical alerts for vital signs {vital_signs.id}: {str(e)}"
                )
            ...
            try:
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"Failed to create audit log for vital signs {vital_signs.id}: {str(e)}"
                )

            return Response(
                VitalSignsSerializer(vital_signs).data, status=status.HTTP_201_CREATED
            )
...
class NurseNursingNotesEndpoint(viewsets.ViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        visit = self.get_visit()
        notes = (
            NursingNote.objects.filter(visit_id=visit.id)
            .select_related("visit", "recorded_by")
            .order_by("-recorded_at")
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        note = serializer.save(visit=visit, recorded_by=request.user)
        ...
        return Response(
            NursingNoteSerializer(note).data, status=status.HTTP_201_CREATED
        )
...
class NurseMedicationAdministrationEndpoint(viewsets.ViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        visit = self.get_visit()
        administrations = (
            MedicationAdministration.objects.filter(visit_id=visit.id)
            .select_related("visit", "prescription", "administered_by")
    ...
    def create(self, request, *args, **kwargs):
        ...
        administration = serializer.save(visit=visit, administered_by=request.user)
...
class NurseLabSamplesEndpoint(viewsets.ViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        visit = self.get_visit()
        collections = (
            LabSampleCollection.objects.filter(visit_id=visit.id)
            .select_related("visit", "lab_order", "collected_by")
    ...
    def create(self, request, *args, **kwargs):
        ...
        if lab_order and lab_order.status not in ["ORDERED", "SAMPLE_COLLECTED"]:
            raise DRFValidationError(
                "Can only collect samples for ORDERED or SAMPLE_COLLECTED lab orders."
            )

        collection = serializer.save(visit=visit, collected_by=request.user)
        ...
        return Response(
            LabSampleCollectionSerializer(collection).data,
            status=status.HTTP_201_CREATED,
        )
