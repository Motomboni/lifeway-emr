...
class MedicationAdministrationSerializer(serializers.ModelSerializer):
    ...
    prescription_drug = serializers.CharField(
        source="prescription.drug", read_only=True
    )
    prescription_dosage = serializers.CharField(
        source="prescription.dosage", read_only=True
    )
    visit_id = serializers.IntegerField(source="visit.id", read_only=True)
