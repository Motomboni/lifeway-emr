...
class Ward(models.Model):
    ...
    code = models.CharField(
        max_length=50, unique=True, help_text="Ward code (e.g., 'GW', 'ICU', 'MAT')"
    )

    description = models.TextField(blank=True, help_text="Description of the ward")

    capacity = models.IntegerField(
        default=0, help_text="Total number of beds in this ward"
    )

    is_active = models.BooleanField(
        default=True, help_text="Whether this ward is currently active"
    )
...
class Bed(models.Model):
    ...
    bed_number = models.CharField(
        max_length=50, help_text="Bed number or identifier (e.g., 'A1', 'B5', 'ICU-1')"
    )
    ...
    is_available = models.BooleanField(
        default=True, help_text="Whether this bed is currently available"
    )

    is_active = models.BooleanField(
        default=True, help_text="Whether this bed is active (not decommissioned)"
    )
...
class Admission(models.Model):
    ...
    admission_date = models.DateTimeField(
        default=timezone.now, help_text="Date and time of admission"
    )
    ...
    admission_notes = models.TextField(
        blank=True, help_text="Additional notes about the admission"
    )
        ...
        if not self.pk and self.bed and self.bed.pk:
            ...
            existing_admission = (
                Admission.objects.filter(
                    bed_id=self.bed.pk, admission_status="ADMITTED"
                )
                .exclude(visit=self.visit if self.visit and self.visit.pk else None)
                .first()
            )
                ...
                if old_admission.admission_status != self.admission_status:
                    if self.admission_status in ["DISCHARGED", "TRANSFERRED"]:
                        if self.bed and not self.bed.is_available:
                            ...
                            self.bed.save(update_fields=["is_available", "updated_at"])
                    elif old_admission.admission_status in [
                        "DISCHARGED",
                        "TRANSFERRED",
                    ]:
    ...
    def transfer(self, new_ward, new_bed, transfer_notes=None):
        ...
        with transaction.atomic():
            ...
            old_bed = (
                Bed.objects.select_for_update().get(pk=self.bed.pk)
                if self.bed
                else None
            )
            ...
            if not new_bed_locked.is_available:
                raise ValidationError(
                    f"Bed {new_bed_locked.bed_number} is not available."
                )
