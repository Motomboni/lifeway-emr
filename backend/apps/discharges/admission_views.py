...
from .admission_models import Admission, Bed, Ward
...
class WardViewSet(viewsets.ModelViewSet):
    ...
    def available_beds(self, request, pk=None):
        ...
        beds = Bed.objects.filter(
            ward=ward, is_available=True, is_active=True
        ).order_by("bed_number")
        serializer = BedListSerializer(beds, many=True)
        return Response(serializer.data)
...
class AdmissionViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"Error creating admission: {type(e).__name__}: {str(e)}", exc_info=True
            )
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            logger.info(
                f"Creating admission for visit {visit.id}, user {self.request.user.id}"
            )
            logger.debug(f"Request data: {self.request.data}")

            # Create admission
            admission = serializer.save(visit=visit, admitting_doctor=self.request.user)
            ...
            try:
                ...
                logger.error(
                    f"Failed to create audit log for admission {admission.id}: {e}"
                )
    ...
    def discharge(self, request, visit_id=None, pk=None):
        ...
        if discharge_date:
            try:
                discharge_date = timezone.datetime.fromisoformat(
                    discharge_date.replace("Z", "+00:00")
                )
        ...
        if (
            hasattr(admission.visit, "discharge_summary")
            and admission.visit.discharge_summary
        ):
...
class InpatientListViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = (
        Admission.objects.filter(admission_status="ADMITTED")
        .select_related("visit", "ward", "bed", "admitting_doctor", "visit__patient")
