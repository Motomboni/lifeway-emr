...
from apps.discharges.admission_models import Bed, Ward
    ...
    def handle(self, *args, **options):
        ...
        for ward_data in wards_data:
            ...
            if ward_created:
                created_wards += 1
                self.stdout.write(
                    self.style.SUCCESS(f"Created ward: {ward.name} ({ward.code})")
                )
            else:
                updated_wards += 1
                self.stdout.write(
                    self.style.WARNING(f"Updated ward: {ward.name} ({ward.code})")
                )
            ...
            for bed_data in beds_data:
                ...
                if bed_created:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"  Created bed: {bed.bed_number} ({bed.bed_type})"
                        )
                    )
                else:
                    updated_beds += 1
                    ...
                    if not bed.is_active:
                        ...
                        bed.save()
                        self.stdout.write(
                            self.style.WARNING(f"  Reactivated bed: {bed.bed_number}")
                        )
