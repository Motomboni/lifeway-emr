...
class AdmissionSerializer(serializers.ModelSerializer):
    ...
    patient_id = serializers.CharField(
        source="visit.patient.patient_id", read_only=True
    )
    ...
    def validate_bed(self, value):
        ...
        if not value.is_available:
            raise serializers.ValidationError(
                f"Bed {value.bed_number} is not available."
            )
            ...
            if not admission_date:
                ...
                try:
                    ...
                    if "T" in admission_date:
                        admission_date = datetime.fromisoformat(
                            admission_date.replace("Z", "+00:00")
                        )
                    else:
                        # Just date, add time
...
class AdmissionUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Admission
        fields = ["admission_status", "admission_notes", "discharge_date"]
    ...
    def update(self, instance, validated_data):
        ...
        admission_status = validated_data.get(
        ...
        if (
            ...
            and instance.admission_status != "DISCHARGED"
        ):
