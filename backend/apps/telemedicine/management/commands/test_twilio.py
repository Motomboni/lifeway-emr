...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR("[ERROR] Twilio package is NOT installed")
            )
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"[ERROR] Twilio Video SDK import failed: {e}")
            )
            sys.exit(1)
        ...
        for key, value, name in checks:
            if value:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "   Set " + key + " in .env or environment variables"
                    )
                )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS("[OK] Successfully connected to Twilio")
            )
            self.stdout.write(f"   Account Name: {account.friendly_name}")
            self.stdout.write(f"   Account Status: {account.status}")
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"[ERROR] Failed to connect to Twilio: {e}")
            )
            self.stdout.write(
                self.style.WARNING("   Check your Account SID and Auth Token")
            )
            sys.exit(1)
        ...
        try:
            ...
            try:
                ...
                if api_key_account_sid != account_sid:
                    self.stdout.write(
                        self.style.ERROR("[ERROR] API Key Account SID mismatch!")
                    )
                    self.stdout.write(
                        self.style.WARNING(
                            f"   Account SID in .env: {account_sid[:8]}...{account_sid[-4:]}"
                        )
                    )
                    self.stdout.write(
                        self.style.WARNING(
                            f"   API Key belongs to: {api_key_account_sid[:8]}...{api_key_account_sid[-4:]}"
                        )
                    )
                    self.stdout.write(
                        self.style.WARNING(
                            "   The API Key must belong to the same account as the Account SID"
                        )
                    )
                    self.stdout.write(
                        self.style.WARNING(
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            "[OK] API Key belongs to the correct account"
                        )
                    )
                    self.stdout.write(
                        f"   API Key Friendly Name: {api_key_obj.friendly_name or 'N/A'}"
                    )
            except Exception as e:
                error_str = str(e)
                ...
                if "403" in error_str or "not permitted" in error_str.lower():
                    self.stdout.write(
                        self.style.WARNING(
                            "[INFO] Cannot verify API Key account (permission denied)"
                        )
                    )
                    self.stdout.write(
                        self.style.WARNING(
                            "   This is normal - will verify by testing token generation instead"
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f"[WARNING] Could not verify API Key account: {e}"
                        )
                    )
                    self.stdout.write(
                        self.style.WARNING(
                            "   Continuing with token generation test..."
                        )
                    )
            ...
            if jwt_token:
                ...
                self.stdout.write(
                    self.style.ERROR("[ERROR] Failed to generate test token")
                )
                sys.exit(1)
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"[ERROR] API Key test failed: {e}"))
            self.stdout.write(
                self.style.WARNING("   Check your API Key SID and API Secret")
            )
            self.stdout.write(
                self.style.WARNING(
                    "   Also verify the API Key belongs to the same account as your Account SID"
                )
            )
            sys.exit(1)
        ...
        self.stdout.write(
            self.style.SUCCESS("[SUCCESS] Telemedicine is ready to use!\n")
        )
