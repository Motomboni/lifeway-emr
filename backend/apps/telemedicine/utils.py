...
try:
    ...
    logger.warning(
        "Twilio package not installed. Telemedicine features will be limited."
    )
...
def generate_twilio_access_token(user, room_name=None, room_sid=None):
    ...
    try:
        ...
        logger.info(
            f"Generating token for room: {room_identifier} (type: {identifier_type}), user: {user.id}"
        )
        logger.info(
            f"Using Account SID: {account_sid[:8]}...{account_sid[-4:] if account_sid else 'None'}, API Key: {api_key[:8]}...{api_key[-4:] if api_key else 'None'}"
        )
...
def create_twilio_room(
    room_name, max_participants=2, record_participants_on_connect=None
):
    """
    ...
    try:
        ...
        if record_participants_on_connect is None:
            record_participants_on_connect = getattr(
                settings, "TWILIO_RECORDING_ENABLED", False
            )

        # Log credential info for debugging (masked)
        logger.info(
            f"Creating Twilio room with Account SID: {account_sid[:8]}...{account_sid[-4:] if account_sid else 'None'}, recording={record_participants_on_connect}"
        )
        ...
        logger.info(
            f"Room created - SID: {room.sid}, Unique Name: {room.unique_name}, Status: {room.status}"
        )
        ...
        logger.error(
            f"Account SID configured: {bool(account_sid)}, Auth Token configured: {bool(auth_token)}"
        )
        # Re-raise with a more user-friendly message
        if "Authentication Error" in error_msg or "invalid username" in error_msg:
            raise ValueError(
                f"Twilio authentication failed. Please verify your Account SID and Auth Token in the .env file. Error: {error_msg}"
            )
        raise


def update_twilio_room_max_participants(room_sid, max_participants):
    """Best-effort raise of Twilio room capacity for virtual clinic invites."""
    if not TWILIO_AVAILABLE:
        raise ImportError(
            "twilio package not installed. Install with: pip install twilio"
        )
    ...
    client = Client(account_sid, auth_token)
    room = client.video.rooms(room_sid).update(max_participants=max_participants)
    logger.info(
        "Twilio room %s max_participants updated to %s",
        room_sid,
        max_participants,
    )
    return {"room_sid": room.sid, "max_participants": max_participants}
...
def end_twilio_room(room_sid):
    ...
    try:
        ...
        try:
            ...
            if e.code == 20404:
                logger.warning(
                    f"Room {room_sid} not found - may have been auto-deleted or expired"
                )
...
def get_room_recordings(room_sid):
    ...
    try:
        ...
        try:
            ...
            recordings = client.video.recordings.list(grouping_key_sid=[room_sid])
        ...
        for rec in recordings:
            ...
            entry = {
                ...
                if getattr(rec, "date_created", None)
                else None,
            }
            if rec_status == "completed":
                result.append(entry)
            elif rec_status in ("processing", "queued", "in-progress", None):
                # Keep SID so auto-transcription can retry when media is ready
                result.append(entry)
        return result
...
def refresh_session_recording(session) -> bool:
    """
    Refresh recording_sid/url on a session from Twilio (e.g. after end while processing).
    Returns True if recording_sid is set.
    """
    if not session.twilio_room_sid:
        return False
    try:
        recordings = get_room_recordings(session.twilio_room_sid)
    except Exception as exc:
        logger.warning("Could not refresh recordings for session %s: %s", session.id, exc)
    ...
    latest = recordings[0]
    session.recording_sid = latest.get("sid") or session.recording_sid
    session.recording_url = (
        latest.get("media_url") or latest.get("url") or session.recording_url
    )
