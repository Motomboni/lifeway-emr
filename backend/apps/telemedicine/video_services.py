...
Note: Session creation uses apps.telemedicine.utils.create_twilio_room directly.
This module is retained for optional provider abstraction only.
"""
...
class TwilioVideoService(BaseVideoService):
    ...
    def generate_video_room(
        ...
        **kwargs,
        ...
        meeting_link = (
            f"{base_url}/telemedicine/room/{kwargs.get('session_id', '')}"
            if base_url
            else room_info.get("room_name", name)
        )
        return {
...
class LiveKitVideoService(BaseVideoService):
    """LiveKit — self-hosted or cloud WebRTC rooms."""
