...
class ProcedureTask(models.Model):
    ...
    procedure_name = models.CharField(
        max_length=255, help_text="Name of the procedure (snapshot from ServiceCatalog)"
    )

    procedure_description = models.TextField(
        blank=True, help_text="Description of the procedure"
    )

    clinical_indication = models.TextField(
        blank=True, help_text="Clinical reason for performing the procedure"
    )
    ...
    execution_notes = models.TextField(
        blank=True, help_text="Notes from the nurse executing the procedure"
    )

    execution_date = models.DateTimeField(
        null=True, blank=True, help_text="When the procedure was executed"
    )

    # Timestamps
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the procedure task was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the procedure task was last updated"
    )
    ...
    def __str__(self):
        return (
            f"Procedure Task {self.id}: {self.procedure_name} for Visit {self.visit_id}"
        )
    ...
    def clean(self):
        ...
        if self.service_catalog_id:
            ...
            is_registration = (
                (service.service_code or "").upper().startswith("REG-")
                or "REGISTRATION" in (service.name or "").upper()
                or "REGISTRATION" in (service.description or "").upper()
            )
