...
class OperationNote(models.Model):
    ...
    operation_name = models.CharField(
        max_length=255, help_text="Name of the operation/procedure"
    )

    preoperative_diagnosis = models.TextField(help_text="Preoperative diagnosis")

    postoperative_diagnosis = models.TextField(
        blank=True, help_text="Postoperative diagnosis (if different)"
    )
    ...
    anesthesia_notes = models.TextField(
        blank=True, help_text="Additional anesthesia notes"
    )
    ...
    complications = models.TextField(
        blank=True, help_text="Any complications encountered"
    )
    ...
    specimens_sent = models.TextField(
        blank=True, help_text="Specimens sent for pathology (if any)"
    )

    # Postoperative details
    postoperative_plan = models.TextField(
        blank=True, help_text="Postoperative care plan"
    )

    postoperative_instructions = models.TextField(
        blank=True, help_text="Postoperative instructions for patient"
    )
    ...
    operation_duration_minutes = models.IntegerField(
        null=True, blank=True, help_text="Duration of operation in minutes"
    )

    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the operation note was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the operation note was last updated"
    )
    ...
    def __str__(self):
        return (
            f"Operation Note {self.id}: {self.operation_name} for Visit {self.visit_id}"
        )
        ...
        if self.visit_id and not self.pk:  # New note
            ...
            if visit.status == "CLOSED":
                raise ValidationError(
                    "Cannot create operation note for a CLOSED visit."
                )
