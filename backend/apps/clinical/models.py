...
class VitalSigns(models.Model):
    ...
    lmp = models.DateField(
        null=True, blank=True, help_text="Last Menstrual Period (LMP)"
    )

    edd = models.DateField(null=True, blank=True, help_text="Expected Due Date (EDD)")
    ...
    recorded_at = models.DateTimeField(
        auto_now_add=True, help_text="When vital signs were recorded"
    )
        ...
        if self.height is not None:
            try:
                ...
                if height_val > 0 and height_val < Decimal("10"):
                    self.height = (height_val * Decimal("100")).quantize(
...
class ClinicalTemplate(models.Model):
    ...
    history_template = models.TextField(
        blank=True, help_text="Pre-filled history section"
    )

    examination_template = models.TextField(
        blank=True, help_text="Pre-filled examination section"
    )

    diagnosis_template = models.TextField(
        blank=True, help_text="Pre-filled diagnosis section"
    )

    clinical_notes_template = models.TextField(
        blank=True, help_text="Pre-filled clinical notes section"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether this template is active and available for use"
    )
...
class ClinicalAlert(models.Model):
    ...
    alert_type = models.CharField(
        max_length=50, choices=ALERT_TYPES, help_text="Type of clinical alert"
    )
    ...
    related_resource_id = models.IntegerField(
        null=True, blank=True, help_text="ID of related resource"
    )
...
from apps.clinical.immunization_models import ImmunizationRecord  # noqa: E402,F401
