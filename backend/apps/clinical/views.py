...
class VitalSignsViewSet(viewsets.ModelViewSet):
        ...
        try:
            ...
            for vs in queryset:
                try:
                    ...
                    logger.warning(
                        f"Error serializing vital signs {vs.id}: {str(e)}",
                        exc_info=True,
                    )
    ...
    def perform_create(self, serializer):
        ...
        vital_signs = serializer.save(visit=visit, recorded_by=self.request.user)
...
class ClinicalTemplateViewSet(viewsets.ModelViewSet):
    ...
    queryset = ClinicalTemplate.objects.filter(is_active=True).select_related(
        "created_by"
    )
...
class ClinicalAlertViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def acknowledge_alert(self, request, visit_id=None, pk=None):
        ...
        try:
            ...
            logger.error(
                f"Failed to acknowledge alert {alert.id}: {str(e)}", exc_info=True
            )
        ...
        try:
            ...
            logger.error(
                f"Serialization failed for alert {alert.id}: {str(e)}", exc_info=True
            )
