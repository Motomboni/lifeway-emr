...
from .models import ClinicalAlert, ClinicalTemplate, VitalSigns
        ...
        if obj.recorded_by:
            ...
            try:
                ...
                return (
                    ...
                    if hasattr(obj.recorded_by, "username")
                    else "Unknown"
                )
        return "Unknown"
...
class VitalSignsCreateSerializer(serializers.ModelSerializer):
        ...
        try:
            ...
            if temp < 30.0 or temp > 45.0:
                raise serializers.ValidationError(
                    "Temperature must be between 30.0 and 45.0 degrees Celsius."
                )
            return temp
    ...
    def validate(self, attrs):
        ...
        if self.instance is None:
            ...
            if visit:
                try:
                    check_vital_signs_duplicate(visit=visit, window_minutes=3)
...
class ClinicalAlertSerializer(serializers.ModelSerializer):
    ...
    def get_acknowledged_by_name(self, obj):
        ...
        if obj.acknowledged_by:
            ...
            try:
                return getattr(obj.acknowledged_by, "username", None) or str(
                    obj.acknowledged_by
                )
            except Exception:
                return (
                    str(obj.acknowledged_by.id)
                    if hasattr(obj.acknowledged_by, "id")
                    else "Unknown"
                )
        return None
...
class OperationNoteSerializer(serializers.ModelSerializer):
    ...
    def get_surgeon_name(self, obj):
        ...
        if obj.surgeon:
            try:
                ...
                return (
                    obj.surgeon.username
                    if hasattr(obj.surgeon, "username")
                    else "Unknown"
                )
        return None
    ...
    def get_assistant_surgeon_name(self, obj):
        ...
        if obj.assistant_surgeon:
            try:
                return (
                    obj.assistant_surgeon.get_full_name()
                    or obj.assistant_surgeon.username
                )
            except (AttributeError, Exception):
                return (
                    obj.assistant_surgeon.username
                    if hasattr(obj.assistant_surgeon, "username")
                    else "Unknown"
                )
        return None
    ...
    def get_anesthetist_name(self, obj):
        ...
        if obj.anesthetist:
            try:
                ...
                return (
                    obj.anesthetist.username
                    if hasattr(obj.anesthetist, "username")
                    else "Unknown"
                )
        return None
