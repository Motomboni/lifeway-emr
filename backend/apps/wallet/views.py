...
from .models import PaymentChannel, Wallet, WalletTransaction
...
class WalletViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        elif user_role == "ADMIN" or getattr(self.request.user, "is_superuser", False):
            qs = Wallet.objects.all()
        else:
            return Wallet.objects.none()

        from core.tenant import filter_by_patient_organization

        qs = filter_by_patient_organization(qs, self.request)
        ...
        patient_pk = self.request.query_params.get("patient")
        if patient_pk:
            qs = qs.filter(patient_id=patient_pk)
        elif not search:
            # Staff must search — avoid loading every wallet in the clinic
            return Wallet.objects.none()

        return qs.order_by("patient__last_name", "patient__first_name")[:50]
    ...
    def list(self, request, *args, **kwargs):
        ...
        if user_role == "PATIENT":
            ...
            if patient_created or wallet_created:
                ...
                action = (
                    "PATIENT_AND_WALLET_AUTO_CREATED"
                    if patient_created and wallet_created
                    else (
                        "PATIENT_AUTO_CREATED"
                        if patient_created
                        else "WALLET_AUTO_CREATED"
                    )
                )
                AuditLog.log(
                    user=request.user,
    ...
    def top_up(self, request, pk=None):
        ...
        # Get payment channel
        payment_channel = get_object_or_404(
            PaymentChannel, id=payment_channel_id, is_active=True
        )
        ...
        try:
            ...
            # Get patient email
            patient_email = (
                wallet.patient.user.email
                if wallet.patient.user
                else f"patient{wallet.patient.id}@example.com"
            )
            ...
            return Response(
                {
                    ...
                    "reference": reference,
                    "authorization_url": gateway_response.get("data", {}).get(
                        "authorization_url"
                    ),
    ...
    def verify_payment(self, request, pk=None):
        ...
        transaction = WalletTransaction.objects.filter(
            wallet=wallet, gateway_transaction_id=reference
        ).first()
        ...
        try:
            ...
            gateway_service = PaymentGatewayService(
                transaction.payment_channel.channel_type
            )
    ...
    def pay_visit(self, request, pk=None):
        ...
        amount = serializer.validated_data.get(
            "amount"
        )  # Optional - will calculate if not provided
        description = serializer.validated_data.get(
            "description", f"Payment for visit {visit_id}"
        )
        ...
        try:
            ...
            AuditLog.log(
                ...
                metadata={
                    ...
                    "outstanding_balance_after": str(
                        updated_summary.outstanding_balance
                    ),
...
class PaymentChannelViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        from .channel_defaults import ONLINE_TOP_UP_CHANNEL_TYPES, ensure_paystack_channel

        ensure_paystack_channel()
        return PaymentChannel.objects.filter(
            is_active=True, channel_type__in=ONLINE_TOP_UP_CHANNEL_TYPES
        ).order_by("name")
