...
class PaymentChannelSerializer(serializers.ModelSerializer):
    ...
    supports_online_topup = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            "channel_type",
            "is_active",
            "supports_online_topup",
            "config",
            "created_at",
            "updated_at",
        ]
        read_only_fields = ["id", "created_at", "updated_at", "supports_online_topup"]

    def get_supports_online_topup(self, obj) -> bool:
        from .channel_defaults import ONLINE_TOP_UP_CHANNEL_TYPES

        return obj.channel_type in ONLINE_TOP_UP_CHANNEL_TYPES


class WalletTransactionSerializer(serializers.ModelSerializer):
    """Serializer for WalletTransaction model."""

    wallet_patient_name = serializers.CharField(
        source="wallet.patient.full_name", read_only=True
    )
    payment_channel_name = serializers.CharField(
        source="payment_channel.name", read_only=True
    )
