...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if count == 0:
            self.stdout.write(self.style.SUCCESS("All patients already have wallets."))
            return
        ...
        for patient in patients_without_wallets:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Created wallet for patient {patient.patient_id} ({patient.get_full_name()})"
                    )
                )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(
                        f"Failed to create wallet for patient {patient.patient_id}: {e}"
                    )
                )
