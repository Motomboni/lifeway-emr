...
class WalletTopUpView(APIView):
    ...
    def post(self, request):
        ...
        wallet, created = Wallet.objects.get_or_create(
            patient=patient,
            defaults={"balance": Decimal("0.00"), "currency": "NGN", "is_active": True},
        )
...
class WalletPayView(APIView):
    ...
    def post(self, request):
        ...
        if visit.patient.id != patient.id:
            raise DRFValidationError("Visit does not belong to the specified patient.")
        ...
        try:
            ...
            wallet.credit(amount, f"Reversal of failed payment for Visit {visit.id}")
