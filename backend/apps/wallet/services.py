...
class PaystackService:
    ...
    def initiate_transfer(
        ...
        response = requests.post(
            f"{self.base_url}/transfer", json=payload, headers=self._get_headers()
        )
...
class PaymentGatewayService:
    ...
    def initialize_payment(
        ...
        if self.channel_type == "PAYSTACK":
            ...
            raise ValueError(
                f"Payment initialization not supported for {self.channel_type}"
            )
    ...
    def verify_payment(self, reference: str) -> Dict[str, Any]:
        ...
        if self.channel_type == "PAYSTACK":
            ...
            raise ValueError(
                f"Payment verification not supported for {self.channel_type}"
            )
