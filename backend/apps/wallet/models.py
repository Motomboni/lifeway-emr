...
class Wallet(models.Model):
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When wallet was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When wallet was last updated"
    )
    ...
    def debit(self, amount: Decimal, visit, description: str = "", created_by=None):
        ...
        if self.balance - amount < 0:
            raise ValidationError(
                "Debit would result in negative balance, which is forbidden."
            )
...
class PaymentChannel(models.Model):
    ...
    name = models.CharField(max_length=50, unique=True, help_text="Channel name")

    channel_type = models.CharField(
        max_length=20, choices=CHANNEL_TYPES, help_text="Type of payment channel"
    )
...
class WalletTransaction(models.Model):
    ...
    transaction_type = models.CharField(
        max_length=10, choices=TRANSACTION_TYPES, help_text="Type of transaction"
    )
    ...
    gateway_response = models.JSONField(
        default=dict, blank=True, help_text="Response from payment gateway"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            raise ValueError(
                "Wallet transactions are append-only and cannot be modified."
            )
