...
from core.tenant import filter_by_radiology_order_visit_organization
...
from .upload_session_models import ImageUploadItem, ImageUploadSession
...
class ImageUploadSessionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = filter_by_radiology_order_visit_organization(
    ...
    def create(self, request, *args, **kwargs):
        ...
        if existing_session:
            logger.info(
                f"Upload session {session_uuid} already exists, returning existing"
            )
            return Response(
        ...
        logger.info(
            f"Created new upload session {session_uuid} for order {session.radiology_order_id}"
        )

        return Response(
            ImageUploadSessionSerializer(session).data, status=status.HTTP_201_CREATED
        )
    ...
    def upload_metadata(self, request, pk=None):
        ...
        try:
                    ...
                    return Response(
                        {
                            ...
                            "metadata": OfflineImageMetadataSerializer(
                                existing_metadata
                            ).data,
                        },
                        status=status.HTTP_200_OK,
                    )
                elif existing_metadata.status in [
                    "METADATA_UPLOADED",
                    "BINARY_UPLOADED",
                ]:
            ...
            upload_item, created = ImageUploadItem.objects.get_or_create(
                ...
                defaults={
                    "metadata": metadata,
                    "upload_status": "METADATA_UPLOADED"
                    if metadata.status == "METADATA_UPLOADED"
                    else "PENDING",
                },
            )
            ...
            logger.info(
                f"Metadata uploaded for image {image_uuid} in session {session.session_uuid}"
            )
    ...
    def upload_binary(self, request, pk=None):
        ...
        try:
            # Get metadata
            metadata = get_object_or_404(OfflineImageMetadata, image_uuid=image_uuid)
            ...
            logger.info(
                f"Binary uploaded for image {image_uuid} in session {session.session_uuid}"
            )
    ...
    def acknowledge(self, request, pk=None):
        ...
        if not image_uuid:
            return Response(
                {"detail": "image_uuid is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            logger.info(
                f"ACK received for image {image_uuid} in session {session.session_uuid}"
            )

            return Response(
                {"detail": "Upload acknowledged"}, status=status.HTTP_200_OK
            )
    ...
    def get_status(self, request, pk=None):
        ...
        session = self.get_object()
        return Response(
            ImageUploadSessionSerializer(session).data, status=status.HTTP_200_OK
        )
