...
from core.tenant import filter_catalog_by_organization, get_request_organization

from .study_types_models import RadiologyStudyType
from .study_types_permissions import (
    CanManageRadiologyStudyTypes,
    CanViewRadiologyStudyTypes,
)
...
def log_radiology_study_type_action(
    user, action, study_type_id=None, request=None, metadata=None
):
    """
...
class RadiologyStudyTypeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if contrast_required is not None:
            queryset = queryset.filter(
                contrast_required=contrast_required.lower() == "true"
            )
    ...
    def perform_create(self, serializer):
        ...
        study_type = (
            serializer.save(organization=org) if org else serializer.save()
        )
    ...
    def categories(self, request):
        ...
        categories = RadiologyStudyType.objects.values_list(
            "category", flat=True
        ).distinct()
