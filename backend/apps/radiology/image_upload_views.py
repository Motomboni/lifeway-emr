...
from core.tenant import filter_by_radiology_order_visit_organization, get_org_scoped_visit
...
class ImageUploadSessionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def create_session(self, request):
        ...
        try:
            order_id = serializer.validated_data["radiology_order_id"]
            radiology_order = get_object_or_404(RadiologyRequest, pk=order_id)
            get_org_scoped_visit(request, radiology_order.visit_id)
    ...
    def pending(self, request):
        ...
        sessions = filter_by_radiology_order_visit_organization(sessions, request)
    ...
    def retry(self, request, session_id=None):
        ...
        try:
            session = get_object_or_404(
                filter_by_radiology_order_visit_organization(
                    ImageUploadSession.objects.all(), request
                ),
                session_id=session_id,
            )
