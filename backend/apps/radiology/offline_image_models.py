...
class OfflineImageMetadata(models.Model):
    ...
    filename = models.CharField(
        max_length=255, help_text="Original filename of the image"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When metadata was queued (client-side timestamp)"
    )
    ...
    failed_at = models.DateTimeField(
        null=True, blank=True, help_text="When upload failed (if status is FAILED)"
    )

    failure_reason = models.TextField(
        blank=True, help_text="Reason for failure (if status is FAILED)"
    )

    # Retry tracking
    retry_count = models.IntegerField(
        default=0, help_text="Number of times upload has been retried"
    )

    last_retry_at = models.DateTimeField(
        null=True, blank=True, help_text="When upload was last retried"
    )
    ...
    def __str__(self):
        return (
            f"OfflineImageMetadata {self.image_uuid} - {self.filename} ({self.status})"
        )
