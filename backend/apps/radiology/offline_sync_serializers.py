...
class OfflineImageMetadataSerializer(serializers.ModelSerializer):
    ...
    radiology_order_id = serializers.IntegerField(
        source="radiology_order.id", read_only=True
    )
...
class OfflineImageMetadataUploadSerializer(serializers.Serializer):
    ...
    image_uuid = serializers.UUIDField(
        help_text="UUID of the image (generated client-side)"
    )
    ...
    mime_type = serializers.CharField(
        max_length=100, help_text="MIME type (e.g., 'image/jpeg', 'application/dicom')"
    )
    checksum = serializers.CharField(
        max_length=64, min_length=64, help_text="SHA-256 checksum (64 hex characters)"
    )
    image_metadata = serializers.JSONField(
        default=dict, help_text="Image metadata (DICOM tags, JPEG EXIF, etc.)"
    )
    ...
    def validate_checksum(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError("Checksum must be hexadecimal.")
        return value.lower()
...
class RadiologyImageSerializer(serializers.ModelSerializer):
    ...
    image_uuid = serializers.UUIDField(
        source="offline_metadata.image_uuid", read_only=True
    )
    ...
    radiology_order_id = serializers.IntegerField(
        source="series.study.radiology_order.id", read_only=True
    )
