...
from apps.core.validators import (
    validate_consultation_required,
)
...
class RadiologyRequest(models.Model):
    ...
    study_code = models.CharField(
        max_length=100, blank=True, help_text="Radiology study code/identifier"
    )

    clinical_indication = models.TextField(
        blank=True, help_text="Clinical indication for the radiology study"
    )

    instructions = models.TextField(
        blank=True, help_text="Special instructions for the radiology study"
    )
    ...
    report_date = models.DateTimeField(
        null=True, blank=True, help_text="When the report was posted"
    )

    # Image metadata (no raw DICOM for now)
    image_count = models.IntegerField(
        null=True, blank=True, help_text="Number of images in the study"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the radiology request was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the radiology request was last updated"
    )
...
class RadiologyOrder(models.Model):
    ...
    imaging_type = models.CharField(
        max_length=10, choices=IMAGING_CHOICES, help_text="Type of imaging study"
    )

    body_part = models.CharField(max_length=100, help_text="Body part to be imaged")
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the radiology order was created"
    )
...
class RadiologyResult(models.Model):
    ...
    image_count = models.IntegerField(
        default=0, help_text="Number of images in the study"
    )
    ...
    reported_at = models.DateTimeField(
        auto_now_add=True, help_text="When the result was reported"
    )
    ...
    def clean(self):
        ...
        if not visit.is_payment_cleared():
            raise ValidationError(
                "Cannot record radiology result before payment clearance."
            )
        ...
        if user_role != "RADIOLOGY_TECH":
            raise ValidationError(
                "Only Radiology Technicians can record radiology results."
            )
