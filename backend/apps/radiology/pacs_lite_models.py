...
class RadiologyStudy(models.Model):
    ...
    # Link to radiology order (legacy) or request (Service Catalog)
    ...
    radiology_request = models.OneToOneField(
    ...
    study_date = models.DateField(
        null=True, blank=True, help_text="Study date (from DICOM or imaging date)"
    )

    study_description = models.CharField(
        max_length=255, blank=True, help_text="Study description"
    )

    modality = models.CharField(
        max_length=10, blank=True, help_text="Modality (e.g., CR, CT, MR, US)"
    )

    # Patient information (snapshot from visit)
    patient_name = models.CharField(
        max_length=255, blank=True, help_text="Patient name (snapshot from visit)"
    )

    patient_id = models.CharField(
        max_length=100, blank=True, help_text="Patient ID (snapshot from visit)"
    )

    # Audit fields
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When study was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When study was last updated"
    )
...
class RadiologySeries(models.Model):
    ...
    series_number = models.IntegerField(
        null=True, blank=True, help_text="Series number (from DICOM)"
    )

    series_description = models.CharField(
        max_length=255, blank=True, help_text="Series description"
    )
    ...
    # Audit fields
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When series was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When series was last updated"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["study", "series_uid"], name="unique_series_per_study"
            )
        ]

    def __str__(self):
        return (
            f"Series {self.series_uid} - {self.series_description or 'No description'}"
        )
...
class RadiologyImage(models.Model):
    ...
    filename = models.CharField(
        max_length=255, null=True, blank=True, help_text="Original filename"
    )
    ...
    instance_number = models.IntegerField(
        null=True, blank=True, help_text="Instance number (from DICOM)"
    )
    ...
    uploaded_at = models.DateTimeField(
        auto_now_add=True, help_text="When image was successfully uploaded"
    )

    validated_at = models.DateTimeField(
        null=True, blank=True, help_text="When checksum was validated"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["checksum"], name="unique_radiology_image_checksum"
            )
        ]
    ...
    def radiology_order(self):
        """Get legacy radiology order through study, if linked."""
        if self.series and self.series.study.radiology_order_id:
            return self.series.study.radiology_order
        return None

    @property
    def radiology_request(self):
        """Get Service Catalog request through study, if linked."""
        if self.series and self.series.study.radiology_request_id:
            return self.series.study.radiology_request
        return None
    ...
    def clean(self):
        ...
        if not self.pk and not self.offline_metadata:
            if not self.series:
                errors["series"] = (
                    "Series is required for new images. Per PACS-lite, images must belong to a series."
                )
            if not self.file_key:
                errors["file_key"] = (
                    "File key is required. Per PACS-lite, images must have a file_key for storage."
                )
