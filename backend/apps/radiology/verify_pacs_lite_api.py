...
Run with: python manage.py shell < apps/radiology/verify_pacs_lite_api.py
...
def run_pacs_lite_integration():
    ...
    try:
        series = PACSLiteService.create_series_for_study(
            study=study, series_number=1, series_description="PA View", modality="CR"
        )
    ...
    try:
        image = OfflineImageSyncService.upload_binary(
            image_uuid=image_uuid, file_content=test_file_content, user=radiology_tech
        )
    ...
    print("\n7. Testing ACK...")
    try:
        ack_metadata = OfflineImageSyncService.acknowledge_upload(image_uuid=image_uuid)
