...
class ImageUploadService:
    ...
    def create_upload_session(
        ...
        if existing:
            logger.info(
                f"Duplicate upload detected for checksum {checksum}, returning existing session"
            )
            return existing
    ...
    def upload_metadata(session_id: str) -> dict:
        ...
        """
        try:
            session = ImageUploadSession.objects.select_for_update().get(
        ...
        try:
            ...
            if not os.path.exists(session.local_file_path):
                raise ValidationError(
                    f"Local file no longer exists: {session.local_file_path}"
                )
        ...
        try:
            ...
            with open(session.local_file_path, "rb") as f:
                ...
                while bytes_uploaded < session.file_size:
                    ...
                    session.upload_progress_percent = (
            ...
            logger.info(
                f"Binary uploaded for session {session_id}, image ID: {image.id}"
            )
    ...
    def acknowledge_upload(session_id: str, server_image_id: int) -> dict:
        ...
        logger.info(
            f"Upload acknowledged for session {session_id}, image ID: {server_image_id}"
        )
    ...
    def get_failed_uploads(radiology_order_id: int = None) -> list:
        ...
        """
        queryset = ImageUploadSession.objects.filter(status="FAILED").filter(
    ...
    def cleanup_completed_sessions(days_old: int = 7):
        ...
        deleted_count = ImageUploadSession.objects.filter(
            status="ACK_RECEIVED", server_ack_at__lt=cutoff_date
        ).delete()[0]
