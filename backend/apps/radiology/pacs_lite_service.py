...
from .pacs_lite_models import RadiologyImage, RadiologySeries, RadiologyStudy
...
class PACSLiteService:
    ...
    def create_study_for_request(
        ...
        """Create a PACS-lite study for a Service Catalog RadiologyRequest."""
        if hasattr(radiology_request, "pacs_study"):
            try:
                return radiology_request.pacs_study
            except RadiologyStudy.DoesNotExist:
                pass

        existing = RadiologyStudy.objects.filter(
        ...
        study = RadiologyStudy.objects.create(
            ...
            radiology_request=radiology_request,
            study_date=study_date or timezone.now().date(),
            study_description=study_description or radiology_request.study_type,
            modality=modality or "",
            patient_name=patient.get_full_name(),
            patient_id=str(patient.id),
        )
        logger.info(
            f"Created study {study.study_uid} for request {radiology_request.id}"
        )
        return study
    ...
    def ingest_dicom_instance(
        *,
        ...
        """Store one DICOM instance under the request's study/series."""
        from .pacs_lite_models import RadiologyImage

        if RadiologyImage.objects.filter(checksum=checksum).exists():
            existing = RadiologyImage.objects.get(checksum=checksum)
            return existing

        study = PACSLiteService.create_study_for_request(
            radiology_request,
        ...
        image_uid = meta.get("sop_instance_uid") or str(uuid.uuid4())
        ...
        PACSLiteService.store_image(file_content, file_key, mime_type)
        ...
        if OrthancService.is_enabled():
            orthanc_info = OrthancService.stow_instance(file_content)
            if orthanc_info:
                image_metadata["orthanc"] = {
                    "instance_id": orthanc_info.get("ID"),
                    "study_id": orthanc_info.get("ParentStudy"),
                    "series_id": orthanc_info.get("ParentSeries"),
                    "path": orthanc_info.get("Path"),
                }
    ...
    def create_series_for_study(
        ...
        if series_uid:
            existing_series = RadiologySeries.objects.filter(
                study=study, series_uid=series_uid
            ).first()
            if existing_series:
                logger.info(
                    f"Series {series_uid} already exists for study {study.study_uid}, returning existing"
                )
    ...
    def get_storage_backend():
        ...
        if hasattr(settings, "RADIOLOGY_STORAGE") and settings.RADIOLOGY_STORAGE:
            ...
            try:
                ...
                logger.warning(
                    f"Could not import custom storage {settings.RADIOLOGY_STORAGE}: {e}. Using default storage."
                )
    ...
    def generate_viewer_url(
        ...
        if ohif_viewer_url:
            separator = "&" if "?" in ohif_viewer_url else "?"
            viewer_url = f"{ohif_viewer_url}{separator}StudyInstanceUIDs={study_uid}"
        else:
        ...
        if (
            hasattr(settings, "RADIOLOGY_SIGNED_URLS")
            and settings.RADIOLOGY_SIGNED_URLS
        ):
    ...
    def get_study_images(study_uid: str) -> List[RadiologyImage]:
        ...
        try:
            ...
            images = (
                RadiologyImage.objects.filter(series__study=study)
    ...
    def get_series_images(series_uid: str) -> List[RadiologyImage]:
        ...
        try:
            ...
            images = RadiologyImage.objects.filter(series=series).order_by(
                "instance_number"
            )
            return list(images)
