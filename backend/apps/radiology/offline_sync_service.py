...
class OfflineImageSyncService:
    ...
    def upload_metadata(
        ...
        if existing:
            if existing.status == "ACK_RECEIVED":
                ...
                existing.save()
                logger.info(
                    f"Updated metadata for image {image_uuid} (retry {existing.retry_count})"
                )
                return existing
        ...
        logger.info(
            f"Metadata uploaded for image {image_uuid} (order {radiology_order_id})"
        )
        return metadata
    ...
    def upload_binary(
        ...
        try:
            metadata = OfflineImageMetadata.objects.get(
                image_uuid=image_uuid, status="METADATA_UPLOADED"
            )
        ...
        existing_image = RadiologyImage.objects.filter(
        ...
        if existing_image:
            ...
            metadata.save()
            logger.info(
                f"Image {image_uuid} already exists (checksum match), returning existing"
            )
        ...
        orthanc_meta = {}
        if OrthancService.is_enabled() and (
            "dicom" in mime or (metadata.filename or "").lower().endswith(".dcm")
        ):
        ...
        image_metadata = {**(metadata.image_metadata or {})}
        if orthanc_meta:
            image_metadata["orthanc"] = orthanc_meta
        ...
        logger.info(
            f"Binary uploaded for image {image_uuid}, stored at {stored_file_key}, ready for ACK"
        )
    ...
    def acknowledge_upload(
        ...
        try:
            metadata = OfflineImageMetadata.objects.get(
                image_uuid=image_uuid, status="BINARY_UPLOADED"
            )
        except OfflineImageMetadata.DoesNotExist:
            raise ValidationError(
