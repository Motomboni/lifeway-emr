...
class ImageUploadSession(models.Model):
    ...
    device_info = models.JSONField(
        default=dict, blank=True, help_text="Device information (OS, browser, etc.)"
    )
    ...
    total_images = models.PositiveIntegerField(
        default=0, help_text="Total number of images in this session"
    )

    images_uploaded = models.PositiveIntegerField(
        default=0, help_text="Number of images successfully uploaded"
    )

    images_failed = models.PositiveIntegerField(
        default=0, help_text="Number of images that failed to upload"
    )
    ...
    started_at = models.DateTimeField(
        null=True, blank=True, help_text="When the upload started"
    )
    ...
    def clean(self):
        ...
        if self.images_uploaded + self.images_failed > self.total_images:
            errors["images_uploaded"] = (
                "Uploaded + failed images cannot exceed total images."
            )
...
class ImageUploadItem(models.Model):
    ...
    def __str__(self):
        return (
            f"Upload Item {self.sequence_number} - {self.get_upload_status_display()}"
        )
