...
class EPrescriptionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if getattr(user, "role", None) == "DOCTOR":
            return (
                EPrescription.objects.filter(doctor=user)
    ...
    def create(self, request, *args, **kwargs):
        ...
        for m in data["medications"]:
            ...
            item = EPrescriptionItem.objects.create(
                eprescription=ep,
                medication=med,
                dosage=m.get("dosage", ""),
        ...
        from apps.clinical.alert_service import evaluate_eprescription_alerts

        open_visit = (
            Visit.objects.filter(patient=patient, status="OPEN")
            .order_by("-created_at")
            .first()
        )
        clinical_alerts = []
        if open_visit:
            clinical_alerts = evaluate_eprescription_alerts(open_visit, patient, items)
        ...
        return Response(
            {
                **EPrescriptionSerializer(ep).data,
                "clinical_alerts_count": len(clinical_alerts),
            },
