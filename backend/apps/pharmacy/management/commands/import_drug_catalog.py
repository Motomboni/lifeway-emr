...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "file_path", type=str, help="Path to CSV or Excel file containing drugs"
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--sheet", type=str, default=0, help="Sheet name or index for Excel files"
        )
    ...
    def handle(self, *args, **options):
        ...
        if file_ext in [".xlsx", ".xls"]:
            ...
            raise CommandError(f"Unsupported format: {file_ext}. Use .csv or .xlsx")

        if dry_run:
            self.stdout.write(self.style.WARNING("DRY RUN - No changes will be saved"))

        stats = self._import_drugs(data, dry_run, update_existing, with_inventory, user)
                ...
                try:
                    ...
                    if existing:
                        if update_existing:
                            ...
                            stats["updated"] += 1
                            self.stdout.write(
                                f"  Updated: {name} - N{sales_price or 0:,.2f}"
                            )
                            drug = existing
                        else:
                            stats["skipped"] += 1
                        ...
                        stats["created"] += 1
                        self.stdout.write(
                            f"  Created: {name} - N{sales_price or 0:,.2f}"
                        )
    ...
    def _print_summary(self, stats, dry_run):
        ...
        if len(stats["errors"]) > 15:
            self.stdout.write(
                self.style.ERROR(f"  ... +{len(stats['errors']) - 15} more")
            )
        if dry_run:
            self.stdout.write(
