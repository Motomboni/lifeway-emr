...
def log_dispense_action(
    user, action, visit_id, prescription_id, request=None, metadata=None
):
    """
...
class DispenseViewSet(viewsets.ViewSet):
    ...
    def create(self, request, visit_id=None):
        ...
        try:
            prescription = Prescription.objects.get(id=prescription_id, visit=visit)
        ...
        if prescription.status == "CANCELLED":
            raise DRFValidationError(
                {"prescription_id": "Cannot dispense a cancelled prescription."}
            )
        ...
        alert_count = ClinicalAlert.objects.filter(
            ...
            related_resource_id=prescription.id,
            is_resolved=False,
        ).count()
        ...
        return Response(
            {
                ...
                "clinical_alerts_count": alert_count,
            },
