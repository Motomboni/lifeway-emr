...
from .models import MedicationInteraction
...
def check_drug_interactions(medication_list: List[int]) -> List[Dict[str, Any]]:
    ...
    for ia in qs:
        key = (
            min(ia.medication_a_id, ia.medication_b_id),
            max(ia.medication_a_id, ia.medication_b_id),
        )
        if key in seen:
            continue
        seen.add(key)
        warnings.append(
            {
                ...
                or f"Known {ia.severity} interaction between {ia.medication_a.name} and {ia.medication_b.name}.",
