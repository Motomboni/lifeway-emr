...
from core.tenant import filter_catalog_by_organization, get_org_scoped_visit, get_request_organization

from .models import Drug, Prescription
from .permissions import CanDispensePrescription, CanManageDrugs, IsDoctor
...
def log_prescription_action(
    user, action, visit_id, prescription_id=None, request=None, metadata=None
):
    """
...
class PrescriptionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Prescription.objects.filter(visit_id=visit_id).select_related(
            "consultation", "prescribed_by", "dispensed_by"
        )
    ...
    def check_payment_status(self, visit):
        ...
        if not visit.is_payment_cleared():
            raise PermissionDenied(
                ...
                "Current payment status: {status}".format(status=visit.payment_status),
    ...
    def perform_create(self, serializer):
        ...
        if user_role != "DOCTOR":
            raise PermissionDenied(
                detail="Only doctors can create prescriptions.", code="role_forbidden"
            )
        ...
        from apps.clinical.alert_service import evaluate_prescription_alerts

        evaluate_prescription_alerts(visit, prescription)

        return prescription

    def create(self, request, *args, **kwargs):
        """Create prescription and return count of generated clinical alerts."""
        response = super().create(request, *args, **kwargs)
        if response.status_code == 201 and response.data.get("id"):
            from apps.clinical.models import ClinicalAlert

            alert_count = ClinicalAlert.objects.filter(
                visit_id=kwargs.get("visit_id"),
                related_resource_type="prescription",
                related_resource_id=response.data["id"],
                is_resolved=False,
            ).count()
            response.data["clinical_alerts_count"] = alert_count
        return response
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            ...
            log_prescription_action(
                user=request.user, action="list", visit_id=visit.id, request=request
            )
    ...
    def perform_update(self, serializer):
        ...
        if user_role == "PHARMACIST":
            ...
            evaluate_prescription_alerts(visit, updated_prescription)
    ...
    def update(self, request, *args, **kwargs):
        """Update prescription and return count of generated clinical alerts."""
        response = super().update(request, *args, **kwargs)
        if response.status_code == 200 and response.data.get("id"):
            from apps.clinical.models import ClinicalAlert
    ...
    def partial_update(self, request, *args, **kwargs):
        """Partial update with clinical alert count."""
...
class DrugViewSet(viewsets.ModelViewSet):
    ...
    queryset = (
        Drug.objects.all().select_related("created_by").prefetch_related("inventory")
    )
    permission_classes = [CanManageDrugs]
    pagination_class = (
        None  # Disable pagination for drug catalog (typically small dataset)
    )
    ...
    def get_queryset(self):
        """Scope drug catalog to current organization plus global templates."""
        queryset = filter_catalog_by_organization(
            Drug.objects.all().select_related("created_by").prefetch_related("inventory"),
            self.request,
        )
        return queryset
    ...
    def perform_create(self, serializer):
        ...
        drug = serializer.save(**save_kwargs)

        # Audit log
        user_role = (
            getattr(self.request.user, "role", None)
            or getattr(self.request.user, "get_role", lambda: "UNKNOWN")()
        )
