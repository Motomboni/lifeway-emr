...
from .models import Drug, DrugInventory, Prescription, StockMovement
...
class DrugUpdateSerializer(DrugSerializer):
    ...
    def validate_name(self, value):
        ...
        if value:
            ...
            if self.instance:
                if (
                    Drug.objects.filter(name__iexact=value)
    ...
    def validate_drug_code(self, value):
        ...
        if value:
            ...
            if self.instance:
                if (
                    Drug.objects.filter(drug_code=value)
                    .exclude(id=self.instance.id)
                    .exists()
                ):
                    raise serializers.ValidationError(
...
class StockMovementSerializer(serializers.ModelSerializer):
    ...
    inventory_drug_name = serializers.CharField(
        source="inventory.drug.name", read_only=True
    )
