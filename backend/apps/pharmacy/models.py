...
class Drug(models.Model):
    ...
    generic_name = models.CharField(
        max_length=255, blank=True, help_text="Generic name of the drug"
    )
    ...
    description = models.TextField(
        blank=True, help_text="Description of the drug, indications, etc."
    )

    # Status tracking
    is_active = models.BooleanField(
        default=True, help_text="Whether the drug is currently available/active"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the drug entry was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the drug entry was last updated"
    )
    ...
    def profit_margin(self):
        ...
        if (
            ...
            and self.sales_price is not None
...
class DrugInventory(models.Model):
    ...
    current_stock = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, help_text="Current stock quantity"
    )
    ...
    batch_number = models.CharField(
        max_length=100, blank=True, help_text="Batch or lot number (optional)"
    )

    expiry_date = models.DateField(
        null=True, blank=True, help_text="Expiry date (optional)"
    )
    ...
    last_restocked_at = models.DateTimeField(
        null=True, blank=True, help_text="When the inventory was last restocked"
    )
    ...
    # Audit timestamps
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the inventory record was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the inventory record was last updated"
    )
...
class StockMovement(models.Model):
    ...
    movement_type = models.CharField(
        max_length=20, choices=MOVEMENT_TYPES, help_text="Type of stock movement"
    )
    ...
    notes = models.TextField(
        blank=True, help_text="Additional notes about this movement"
    )
    ...
    # Audit timestamps
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the movement was recorded"
    )
    ...
    def clean(self):
        ...
        if self.movement_type in ["IN", "RETURNED"] and self.quantity <= 0:
            raise ValidationError(
                "Stock IN and RETURNED movements must have positive quantity."
            )
        if (
            self.movement_type in ["OUT", "DISPENSED", "EXPIRED", "DAMAGED"]
            and self.quantity >= 0
        ):
            raise ValidationError(
                "Stock OUT, DISPENSED, EXPIRED, and DAMAGED movements must have negative quantity."
            )
...
class Prescription(models.Model):
    ...
    drug = models.CharField(max_length=255, help_text="Name of the medication/drug")

    drug_code = models.CharField(
        max_length=100, blank=True, help_text="Drug code/identifier (e.g., NDC code)"
    )

    dosage = models.CharField(
        max_length=255, help_text="Dosage instructions (e.g., '500mg twice daily')"
    )
    ...
    instructions = models.TextField(
        blank=True, help_text="Additional instructions for the patient"
    )
    ...
    dispensed = models.BooleanField(
        default=False, help_text="Whether the medication has been dispensed"
    )

    dispensed_date = models.DateTimeField(
        null=True, blank=True, help_text="When the medication was dispensed"
    )

    dispensing_notes = models.TextField(
        blank=True, help_text="Notes from pharmacist during dispensing"
    )
    ...
    # Audit timestamps
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the prescription was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the prescription was last updated"
    )
...
class MedicationInteraction(models.Model):
    ...
    medication_a = models.ForeignKey(
        Medication, on_delete=models.CASCADE, related_name="interactions_from"
    )
    medication_b = models.ForeignKey(
        Medication, on_delete=models.CASCADE, related_name="interactions_to"
    )
    severity = models.CharField(
        max_length=20, choices=SEVERITY_CHOICES, default="Moderate"
    )
...
from apps.pharmacy.nafdac_models import NAFDACFormularyEntry  # noqa: E402,F401
