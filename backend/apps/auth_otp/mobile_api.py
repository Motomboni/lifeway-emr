...
def mobile_profile(request):
    ...
    data = {
        ...
        "last_updated": (
            patient.updated_at if hasattr(patient, "updated_at") else timezone.now()
        ).isoformat(),
    }
...
def mobile_prescriptions(request):
    ...
    prescriptions = (
        Prescription.objects.filter(visit_id__in=visits)
    ...
    data = [
        {
            ...
            "prescribed_by": rx.prescribed_by.get_full_name()
            if rx.prescribed_by
            else None,
...
def mobile_lab_results(request):
    ...
    lab_orders = (
        LabOrder.objects.filter(visit_id__in=visits, status="COMPLETED")
    ...
    data = [
        {
            ...
            "status": lab.status,
            "notes": lab.notes,
            "updated_at": getattr(lab, "updated_at", None)
            and lab.updated_at.isoformat()
            or (lab.created_at.isoformat() if lab.created_at else None),
        }
        for lab in page
    ]
    AuditLog.log(
        ...
        data.append(
            {
                ...
                "updated_at": getattr(visit, "updated_at", None)
                and visit.updated_at.isoformat(),
                "charges": [
...
def mobile_dashboard(request):
    ...
    # Open visits
    open_visits = Visit.objects.filter(patient=patient, status="OPEN").count()

    # Unpaid bills
    visits_with_balance = Visit.objects.filter(
        patient=patient, payment_status__in=["UNPAID", "PARTIALLY_PAID"]
    ).count()
...
def mobile_sync(request):
    ...
    if not device_id:
        return Response(
            {"error": "device_id query required"}, status=status.HTTP_400_BAD_REQUEST
        )
    try:
