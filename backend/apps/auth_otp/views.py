...
from .models import LoginAuditLog, LoginOTP
...
def check_rate_limit(identifier: str) -> bool:
    ...
    if identifier not in _otp_request_counts:
        _otp_request_counts[identifier] = {"count": 0, "last_request": now}
...
def request_otp(request):
    ...
    try:
        with transaction.atomic():
            ...
            if getattr(settings, "E2E_OTP_EXPOSE", False):
...
def logout(request):
    ...
    try:
        ...
        return Response(
            {"success": True, "message": "Logged out (token may have been invalid)"},
            status=status.HTTP_200_OK,
        )
...
@rate_limit(
    requests_per_minute=BIOMETRIC_RATE_LIMIT[0],
    requests_per_hour=BIOMETRIC_RATE_LIMIT[1],
)
def register_biometric(request):
    ...
    if getattr(user, "role", None) != "PATIENT":
        return Response(
            {
                "success": False,
                "error": "Biometric is only available for patient portal users.",
            },
            status=status.HTTP_403_FORBIDDEN,
        )
...
def biometric_login(request):
    ...
    try:
        user = User.objects.get(
            device_id=device_id, biometric_enabled=True, role="PATIENT", is_active=True
        )
    except User.DoesNotExist:
        LoginAuditLog.log_action(
        ...
        return Response(
            {
                "success": False,
                "error": "Invalid credentials",
                "detail": "Device not recognized. Use OTP login.",
            },
            status=status.HTTP_401_UNAUTHORIZED,
        )

    if not user.biometric_key or not _constant_time_compare(
        biometric_token, user.biometric_key
    ):
        LoginAuditLog.log_action(
            action="BIOMETRIC_LOGIN_FAILED",
            user=user,
