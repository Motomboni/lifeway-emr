...
from .views import (
    EmbryoInventoryViewSet,
    EmbryoTransferViewSet,
    EmbryoViewSet,
    IVFConsentViewSet,
    IVFCycleViewSet,
    IVFMedicationViewSet,
    IVFOutcomeViewSet,
    IVFPatientsListView,
    IVFVisitsListView,
    OocyteRetrievalViewSet,
    OvarianStimulationViewSet,
    SpermAnalysisViewSet,
)
...
router.register(
    r"embryo-inventory", EmbryoInventoryViewSet, basename="embryo-inventory"
)
...
urlpatterns = [
    ...
    path(
        "cycles/<int:cycle_pk>/stimulation/",
        OvarianStimulationViewSet.as_view({"get": "list", "post": "create"}),
        name="cycle-stimulation-list",
    ),
    path(
        "cycles/<int:cycle_pk>/stimulation/<int:pk>/",
        OvarianStimulationViewSet.as_view(
            {
                "get": "retrieve",
                "put": "update",
    ...
    path(
        "cycles/<int:cycle_pk>/retrieval/",
        OocyteRetrievalViewSet.as_view({"get": "list", "post": "create"}),
        name="cycle-retrieval-list",
    ),
    path(
        "cycles/<int:cycle_pk>/retrieval/<int:pk>/",
        OocyteRetrievalViewSet.as_view(
    ...
    path(
        "cycles/<int:cycle_pk>/medications/",
        IVFMedicationViewSet.as_view({"get": "list", "post": "create"}),
        name="cycle-medication-list",
    ),
    path(
        "cycles/<int:cycle_pk>/medications/<int:pk>/",
        IVFMedicationViewSet.as_view(
            {
                "get": "retrieve",
                "put": "update",
                "patch": "partial_update",
    ...
    path(
        "cycles/<int:cycle_pk>/consents/",
        IVFConsentViewSet.as_view({"get": "list", "post": "create"}),
        name="cycle-consent-list",
    ),
    path(
        "cycles/<int:cycle_pk>/consents/<int:pk>/",
        IVFConsentViewSet.as_view(
            {"get": "retrieve", "put": "update", "patch": "partial_update"}
        ),
        name="cycle-consent-detail",
    ),
    path(
        "cycles/<int:cycle_pk>/consents/<int:pk>/sign/",
        IVFConsentViewSet.as_view({"post": "sign"}),
        name="cycle-consent-sign",
    ),
    path(
        "cycles/<int:cycle_pk>/consents/<int:pk>/revoke/",
