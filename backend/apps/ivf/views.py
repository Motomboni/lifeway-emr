...
from .permissions import (
    CanManageEmbryoDisposition,
    CanRecordMedicationAdministration,
    CanRecordStimulationMonitoring,
    CanViewIVFRecords,
    IsEmbryologist,
    IsIVFNurse,
    IsIVFSpecialist,
...
from .serializers import (
    EmbryoDetailSerializer,
    EmbryoInventorySerializer,
    EmbryoListSerializer,
    EmbryoTransferSerializer,
    IVFConsentSerializer,
    IVFCycleCreateSerializer,
    IVFCycleDetailSerializer,
    IVFCycleFullSerializer,
    IVFCycleListSerializer,
    IVFCycleUpdateSerializer,
    IVFMedicationSerializer,
    IVFOutcomeSerializer,
    IVFPatientListSerializer,
    OocyteRetrievalSerializer,
    OvarianStimulationSerializer,
    SpermAnalysisDetailSerializer,
    SpermAnalysisListSerializer,
)
...
class IVFCycleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = IVFCycle.objects.select_related(
            "patient", "partner", "created_by"
        ).prefetch_related("embryos", "medications", "consents")
        if hasattr(self.request, "organization") and self.request.organization:
            qs = qs.filter(patient__organization=self.request.organization)
        return qs
    ...
    def destroy(self, request, *args, **kwargs):
        """Delete an IVF cycle — IVF Specialist or superuser."""
    ...
    def sign_consent(self, request, pk=None):
        ...
        if consent_type == "patient":
            ...
            return Response(
                {"error": "Invalid consent type"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def advance_status(self, request, pk=None):
        ...
        try:
            ...
            cycle.save()
            return Response(
                {"message": "Cycle status updated", "new_status": cycle.status}
            )
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def statistics(self, request):
        ...
        live_births = completed_cycles.filter(pregnancy_outcome="LIVE_BIRTH").count()
        ...
        return Response(
            {
                ...
                "pregnancy_rate": (pregnancies / completed_count * 100)
                if completed_count > 0
                else 0,
                "clinical_pregnancy_rate": (
                    clinical_pregnancies / completed_count * 100
                )
                if completed_count > 0
                else 0,
                "live_birth_rate": (live_births / completed_count * 100)
                if completed_count > 0
                else 0,
...
class OvarianStimulationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return OvarianStimulation.objects.select_related("cycle", "recorded_by").filter(
            cycle_id=self.kwargs.get("cycle_pk")
        )

    def perform_create(self, serializer):
        serializer.save(
            cycle_id=self.kwargs.get("cycle_pk"), recorded_by=self.request.user
        )
...
class OocyteRetrievalViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return OocyteRetrieval.objects.select_related("cycle", "performed_by").filter(
            cycle_id=self.kwargs.get("cycle_pk")
        )

    def perform_create(self, serializer):
        serializer.save(
            cycle_id=self.kwargs.get("cycle_pk"), performed_by=self.request.user
        )
...
class SpermAnalysisViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = SpermAnalysis.objects.select_related("patient", "cycle", "analyzed_by")
        if hasattr(self.request, "organization") and self.request.organization:
            qs = qs.filter(patient__organization=self.request.organization)
        return qs
...
class EmbryoViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        cycle_pk = self.kwargs.get("cycle_pk")
        serializer.save(cycle_id=cycle_pk, created_by=self.request.user)
    ...
    @action(
        detail=True,
        methods=["post"],
        permission_classes=[IsAuthenticated, CanManageEmbryoDisposition],
    )
...
class EmbryoTransferViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        cycle_pk = self.kwargs.get("cycle_pk")
        transfer = serializer.save(cycle_id=cycle_pk, performed_by=self.request.user)
...
class IVFMedicationViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        cycle_pk = self.kwargs.get("cycle_pk")
        serializer.save(cycle_id=cycle_pk, prescribed_by=self.request.user)
...
class IVFConsentViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        cycle_pk = self.kwargs.get("cycle_pk")
        serializer.save(cycle_id=cycle_pk, recorded_by=self.request.user)
    ...
    def revoke(self, request, pk=None, **kwargs):
        ...
        return Response({"message": "Consent revoked", "consent_id": consent.id})
...
class EmbryoInventoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    search_fields = [
        "cycle__patient__first_name",
        "cycle__patient__last_name",
        "lab_id",
    ]
    ...
    def thaw(self, request, pk=None):
        ...
        return Response(
            {"message": "Embryo thawed successfully", "embryo_id": embryo.id}
        )
    ...
    def dispose(self, request, pk=None):
        ...
        return Response(
            {"message": "Embryo disposed successfully", "embryo_id": embryo.id}
        )
...
class IVFPatientsListView(APIView):
    ...
    def get(self, request):
        ...
        ivf_patient_ids = IVFCycle.objects.values_list(
