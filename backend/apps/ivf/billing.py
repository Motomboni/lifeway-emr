...
class IVFPriceList(models.Model):
    ...
    code = models.CharField(max_length=50, unique=True, help_text="Service code")

    name = models.CharField(max_length=200, help_text="Service name")

    category = models.CharField(
        max_length=30, choices=CATEGORY_CHOICES, help_text="Service category"
    )

    description = models.TextField(blank=True, help_text="Service description")

    base_price = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Base price in NGN"
    )

    # Package pricing
    is_package = models.BooleanField(default=False, help_text="Is this a package deal?")

    package_includes = models.JSONField(
        default=list, blank=True, help_text="Services included in package"
    )

    # Insurance
    insurance_covered = models.BooleanField(
        default=False, help_text="Typically covered by insurance"
    )

    insurance_code = models.CharField(
        max_length=50, blank=True, help_text="Insurance billing code"
    )
...
class IVFCycleCharge(models.Model):
    ...
    cycle = models.ForeignKey(
        "ivf.IVFCycle", on_delete=models.CASCADE, related_name="charges"
    )

    service = models.ForeignKey(
        IVFPriceList, on_delete=models.PROTECT, related_name="charges"
    )

    description = models.CharField(max_length=255, help_text="Charge description")

    quantity = models.PositiveIntegerField(default=1, help_text="Quantity of service")

    unit_price = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Unit price at time of charge"
    )

    total_amount = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Total charge amount"
    )
    ...
    final_amount = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Final amount after discount"
    )
    ...
    patient_responsibility = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Amount patient must pay"
    )

    # Payment status
    is_paid = models.BooleanField(default=False, help_text="Has this charge been paid")

    paid_at = models.DateTimeField(
        null=True, blank=True, help_text="When charge was paid"
    )
    ...
    created_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="ivf_charges_created"
    )
...
class IVFPaymentPlan(models.Model):
    ...
    cycle = models.OneToOneField(
        "ivf.IVFCycle", on_delete=models.CASCADE, related_name="payment_plan"
    )

    total_amount = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Total amount to be paid"
    )

    down_payment = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Initial down payment"
    )
    ...
    installment_amount = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Amount per installment"
    )

    total_paid = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="Total amount paid so far"
    )

    remaining_balance = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Remaining balance"
    )
    ...
    notes = models.TextField(blank=True, help_text="Payment plan notes")

    # Audit
    created_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="ivf_payment_plans_created"
    )
...
class IVFPaymentRecord(models.Model):
    ...
    payment_plan = models.ForeignKey(
        IVFPaymentPlan, on_delete=models.CASCADE, related_name="payments"
    )
    ...
    transaction_reference = models.CharField(
        max_length=100, blank=True, help_text="Transaction reference"
    )
    ...
    recorded_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="ivf_payments_recorded"
    )
...
def create_retrieval_charge(cycle, retrieval, user):
    ...
    try:
        service = IVFPriceList.objects.get(
            category=IVFServiceCategory.RETRIEVAL, is_active=True
        )
    except IVFPriceList.DoesNotExist:
...
def create_transfer_charge(cycle, transfer, user):
    ...
    try:
        service = IVFPriceList.objects.get(
            category=IVFServiceCategory.TRANSFER, is_active=True
        )
    except IVFPriceList.DoesNotExist:
        service = None
...
def create_cryopreservation_charge(cycle, embryo, user):
    ...
    try:
        service = IVFPriceList.objects.get(
            category=IVFServiceCategory.CRYOPRESERVATION, is_active=True
        )
    except IVFPriceList.DoesNotExist:
        service = None
...
def create_pgt_charge(cycle, embryo, user):
    ...
    try:
        service = IVFPriceList.objects.get(
            category=IVFServiceCategory.PGT, is_active=True
        )
    except IVFPriceList.DoesNotExist:
        service = None
