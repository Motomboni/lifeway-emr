...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(f"Creating {num_cycles} IVF test cycles...")
        )
        ...
        try:
            ...
            self.stdout.write("Using existing IVF Specialist user: ivf_specialist")
            ...
            ivf_specialist.save()
            self.stdout.write(
                self.style.SUCCESS("Created IVF Specialist user: ivf_specialist")
            )

        # Get or create embryologist user
        try:
            embryologist = User.objects.get(username="embryologist")
            self.stdout.write("Using existing Embryologist user: embryologist")
        except User.DoesNotExist:
            embryologist = User(
            ...
            embryologist.save()
            self.stdout.write(
                self.style.SUCCESS("Created Embryologist user: embryologist")
            )
        ...
        if len(patients) < 5:
            ...
            for i in range(5 - len(patients)):
                patient = Patient.objects.create(
                    ...
                    - timedelta(days=random.randint(9000, 14000)),
                    gender="F",
        ...
        cycle_configs = [
            ...
            },
            {
                "status": "COMPLETED",
                "cycle_type": "ICSI",
                "pregnancy_outcome": "NEGATIVE",
            },
            {
                "status": "CANCELLED",
                "cycle_type": "FRESH_IVF",
                "cancellation_reason": "POOR_RESPONSE",
            },
        ]
        ...
        for i in range(min(num_cycles, len(cycle_configs))):
            ...
            self.stdout.write(
                f"  Created cycle #{cycle.cycle_number}: {patient.first_name} {patient.last_name} - {config['status']}"
            )
                ...
                for day in range(1, random.randint(3, 8)):
                    OvarianStimulation.objects.create(
                        ...
                        right_ovary_follicles=[
                            random.randint(8, 22) for _ in range(random.randint(2, 6))
                        ],
            ...
            if config["status"] not in ["PLANNED", "STIMULATION"]:
                ...
                for e in range(1, num_embryos + 1):
                    ...
                    Embryo.objects.create(
                        ...
                        if "IVF" in config["cycle_type"]
                        else "ICSI",
                        fertilization_date=start_date + timedelta(days=12),
                        day1_pn_status=random.choice(["2PN", "1PN", "3PN"]),
                        day3_cell_count=random.randint(6, 10)
                        if random.random() > 0.3
                        else None,
                        day3_grade=random.choice(["A", "B", "C"])
                        if random.random() > 0.3
                        else "",
                        blastocyst_day=random.choice([5, 6])
                        if random.random() > 0.4
                        else None,
                        blastocyst_grade=random.choice(
                            ["4AA", "4AB", "3BA", "3BB", "3BC"]
                        )
                        if random.random() > 0.4
                        else "",
            ...
            for med_name, category, dose, unit in random.sample(
                medication_names, random.randint(1, 3)
            ):
            ...
            if config["status"] == "COMPLETED" and config.get("pregnancy_outcome"):
                ...
                IVFOutcome.objects.create(
                    ...
                    clinical_pregnancy_date=start_date + timedelta(days=35)
                    if is_live_birth
                    else None,
                    fetal_heartbeat=is_live_birth,
                    fetal_heartbeat_date=start_date + timedelta(days=42)
                    if is_live_birth
                    else None,
                    gestational_sacs=1 if is_live_birth else 0,
                    fetal_poles=1 if is_live_birth else 0,
                    delivery_date=start_date + timedelta(days=280)
                    if is_live_birth
                    else None,
        ...
        for i, cycle in enumerate(created_cycles[:3]):
            SpermAnalysis.objects.create(
                ...
                assessment=random.choice(
                    ["NORMAL", "OLIGOZOOSPERMIA", "ASTHENOZOOSPERMIA"]
                ),
        ...
        self.stdout.write(
            "  IVF Specialist: username=ivf_specialist, password=ivfspecialist123"
        )
        self.stdout.write(
            "  Embryologist: username=embryologist, password=embryologist123"
        )
