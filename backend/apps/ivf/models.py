...
class IVFCycle(models.Model):
    ...
    cycle_number = models.PositiveIntegerField(
        default=1, help_text="Sequential cycle number for this patient"
    )
    ...
    planned_start_date = models.DateField(
        null=True, blank=True, help_text="Planned cycle start date"
    )

    actual_start_date = models.DateField(
        null=True, blank=True, help_text="Actual stimulation start date"
    )

    lmp_date = models.DateField(
        null=True, blank=True, help_text="Last Menstrual Period date"
    )
    ...
    consent_signed = models.BooleanField(
        default=False, help_text="Patient consent form signed"
    )

    consent_date = models.DateField(
        null=True, blank=True, help_text="Date consent was signed"
    )

    partner_consent_signed = models.BooleanField(
        default=False, help_text="Partner consent form signed"
    )

    partner_consent_date = models.DateField(
        null=True, blank=True, help_text="Date partner consent was signed"
    )
    ...
    cancellation_notes = models.TextField(
        blank=True, help_text="Detailed notes on cancellation"
    )

    cancelled_at = models.DateTimeField(
        null=True, blank=True, help_text="When the cycle was cancelled"
    )

    # Outcome
    pregnancy_test_date = models.DateField(
        null=True, blank=True, help_text="Date of pregnancy test"
    )
    ...
    insurance_pre_auth = models.BooleanField(
        default=False, help_text="Insurance pre-authorization obtained"
    )

    insurance_pre_auth_number = models.CharField(
        max_length=100, blank=True, help_text="Insurance pre-authorization number"
    )

    # Notes
    clinical_notes = models.TextField(
        blank=True, help_text="Clinical notes and observations"
    )
    ...
    def clean(self):
        ...
        if (
            self.cycle_type in [self.CycleType.FRESH_IVF, self.CycleType.ICSI]
            and self.partner
        ):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk and not self.cycle_number:
            last_cycle = (
                IVFCycle.objects.filter(patient=self.patient)
    ...
    def cancel(self, reason, notes="", user=None):
        ...
        if self.status in [self.Status.COMPLETED, self.Status.CANCELLED]:
            raise ValidationError(
                "Cannot cancel an already completed or cancelled cycle."
            )
...
class OvarianStimulation(models.Model):
    ...
    cycle = models.ForeignKey(
        IVFCycle, on_delete=models.CASCADE, related_name="stimulation_records"
    )
    ...
    right_ovary_follicles = models.JSONField(
        default=list, blank=True, help_text="Right ovary follicle sizes (mm) as array"
    )

    left_ovary_follicles = models.JSONField(
        default=list, blank=True, help_text="Left ovary follicle sizes (mm) as array"
    )
    ...
    next_appointment = models.DateTimeField(
        null=True, blank=True, help_text="Next monitoring appointment"
    )

    # Audit
    recorded_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="stimulation_records"
    )
    ...
    def total_follicle_count(self):
        ...
        return len(self.right_ovary_follicles or []) + len(
            self.left_ovary_follicles or []
        )

    @property
    def leading_follicles(self):
        """Count of follicles >= 14mm."""
        all_follicles = (self.right_ovary_follicles or []) + (
            self.left_ovary_follicles or []
        )
...
class OocyteRetrieval(models.Model):
    ...
    cycle = models.OneToOneField(
        IVFCycle, on_delete=models.CASCADE, related_name="oocyte_retrieval"
    )
    ...
    procedure_time = models.TimeField(
        null=True, blank=True, help_text="Time procedure started"
    )
    ...
    trigger_time = models.DateTimeField(
        null=True, blank=True, help_text="Time of trigger injection"
    )
    ...
    anesthesiologist = models.CharField(
        max_length=100, blank=True, help_text="Anesthesiologist name"
    )

    # Retrieval Results
    right_ovary_oocytes = models.PositiveIntegerField(
        default=0, help_text="Oocytes retrieved from right ovary"
    )

    left_ovary_oocytes = models.PositiveIntegerField(
        default=0, help_text="Oocytes retrieved from left ovary"
    )

    total_oocytes_retrieved = models.PositiveIntegerField(
        default=0, help_text="Total oocytes retrieved"
    )

    mature_oocytes = models.PositiveIntegerField(
        default=0, help_text="Number of mature (MII) oocytes"
    )

    immature_oocytes = models.PositiveIntegerField(
        default=0, help_text="Number of immature oocytes"
    )

    degenerated_oocytes = models.PositiveIntegerField(
        default=0, help_text="Number of degenerated oocytes"
    )

    # Complications
    complications = models.TextField(
        blank=True, help_text="Any complications during procedure"
    )
    ...
    recovery_notes = models.TextField(
        blank=True, help_text="Post-procedure recovery notes"
    )

    discharge_time = models.TimeField(
        null=True, blank=True, help_text="Time patient discharged"
    )

    # Performing doctor
    performed_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="oocyte_retrievals"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        self.total_oocytes_retrieved = (
            self.right_ovary_oocytes + self.left_ovary_oocytes
        )
        super().save(*args, **kwargs)
...
class SpermAnalysis(models.Model):
    ...
    patient = models.ForeignKey(
        "patients.Patient", on_delete=models.PROTECT, related_name="sperm_analyses"
    )
    ...
    collection_time = models.TimeField(
        null=True, blank=True, help_text="Time of sample collection"
    )

    abstinence_days = models.PositiveIntegerField(
        null=True, blank=True, help_text="Days of abstinence before collection"
    )
    ...
    analyzed_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="sperm_analyses"
    )
...
class Embryo(models.Model):
    ...
    cycle = models.ForeignKey(
        IVFCycle, on_delete=models.CASCADE, related_name="embryos"
    )
    ...
    lab_id = models.CharField(
        max_length=50, unique=True, help_text="Unique laboratory identifier"
    )
    ...
    fertilization_time = models.TimeField(
        null=True, blank=True, help_text="Time of insemination/ICSI"
    )
    ...
    day2_cell_count = models.PositiveIntegerField(
        null=True, blank=True, help_text="Cell count at Day 2"
    )

    day3_cell_count = models.PositiveIntegerField(
        null=True, blank=True, help_text="Cell count at Day 3"
    )

    day3_grade = models.CharField(
        max_length=10, blank=True, help_text="Day 3 embryo grade (e.g., 8A, 6B)"
    )

    fragmentation = models.PositiveIntegerField(
        null=True, blank=True, help_text="Fragmentation percentage"
    )
    ...
    blastocyst_grade = models.CharField(
        max_length=10, blank=True, help_text="Blastocyst grade (e.g., 4AA, 3BB)"
    )
    ...
    pgt_performed = models.BooleanField(
        default=False, help_text="PGT performed on this embryo"
    )
    ...
    frozen_date = models.DateField(
        null=True, blank=True, help_text="Date embryo was frozen"
    )

    storage_location = models.CharField(
        max_length=100, blank=True, help_text="Cryostorage tank/canister/position"
    )

    straw_id = models.CharField(
        max_length=50, blank=True, help_text="Cryostraw identifier"
    )

    thaw_date = models.DateField(
        null=True, blank=True, help_text="Date embryo was thawed"
    )

    survived_thaw = models.BooleanField(
        null=True, blank=True, help_text="Did embryo survive thawing?"
    )
    ...
    disposition_date = models.DateField(
        null=True, blank=True, help_text="Date of disposition"
    )
    ...
    # Audit
    created_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="embryos_created"
    )
...
class EmbryoTransfer(models.Model):
    ...
    cycle = models.ForeignKey(
        IVFCycle, on_delete=models.CASCADE, related_name="embryo_transfers"
    )
    ...
    transfer_time = models.TimeField(
        null=True, blank=True, help_text="Time of transfer"
    )
    ...
    embryos = models.ManyToManyField(
        Embryo, related_name="transfers", help_text="Embryos transferred"
    )

    embryos_transferred_count = models.PositiveIntegerField(
        default=1, help_text="Number of embryos transferred"
    )
    ...
    catheter_type = models.CharField(
        max_length=100, blank=True, help_text="Transfer catheter used"
    )

    ultrasound_guided = models.BooleanField(
        default=True, help_text="Transfer performed under ultrasound guidance"
    )
    ...
    blood_on_catheter = models.BooleanField(
        default=False, help_text="Blood observed on catheter"
    )

    mucus_on_catheter = models.BooleanField(
        default=False, help_text="Mucus observed on catheter"
    )

    embryos_retained = models.BooleanField(
        default=False, help_text="Were any embryos retained in catheter?"
    )

    # Post-procedure
    bed_rest_duration = models.PositiveIntegerField(
        default=30, help_text="Recommended bed rest (minutes)"
    )

    medications_prescribed = models.JSONField(
        default=list, blank=True, help_text="Post-transfer medications"
    )
    ...
    # Performing doctor
    performed_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="embryo_transfers"
    )
...
class IVFMedication(models.Model):
    ...
    cycle = models.ForeignKey(
        IVFCycle, on_delete=models.CASCADE, related_name="medications"
    )

    # Medication Details
    medication_name = models.CharField(max_length=100, help_text="Medication name")
    ...
    dose = models.DecimalField(max_digits=10, decimal_places=2, help_text="Dose amount")

    unit = models.CharField(max_length=20, help_text="Dose unit (e.g., IU, mg, mcg)")
    ...
    frequency = models.CharField(
        max_length=50, help_text="Frequency (e.g., Once daily, Twice daily)"
    )

    start_date = models.DateField(help_text="Start date")

    end_date = models.DateField(null=True, blank=True, help_text="End date (if known)")
...
class IVFOutcome(models.Model):
    ...
    cycle = models.OneToOneField(
        IVFCycle, on_delete=models.CASCADE, related_name="outcome"
    )
    ...
    clinical_pregnancy_date = models.DateField(
        null=True, blank=True, help_text="Date clinical pregnancy confirmed"
    )

    fetal_heartbeat = models.BooleanField(
        default=False, help_text="Fetal heartbeat detected"
    )

    fetal_heartbeat_date = models.DateField(
        null=True, blank=True, help_text="Date fetal heartbeat detected"
    )

    # Multiple Pregnancy
    gestational_sacs = models.PositiveIntegerField(
        default=0, help_text="Number of gestational sacs"
    )

    fetal_poles = models.PositiveIntegerField(
        default=0, help_text="Number of fetal poles with heartbeat"
    )
    ...
    miscarriage_date = models.DateField(
        null=True, blank=True, help_text="Date of miscarriage"
    )

    miscarriage_gestational_age = models.PositiveIntegerField(
        null=True, blank=True, help_text="Gestational age at miscarriage (weeks)"
    )
    ...
    delivery_date = models.DateField(
        null=True, blank=True, help_text="Date of delivery"
    )

    gestational_age_at_delivery = models.PositiveIntegerField(
        null=True, blank=True, help_text="Gestational age at delivery (weeks)"
    )
    ...
    stillbirths = models.PositiveIntegerField(
        default=0, help_text="Number of stillbirths"
    )

    neonatal_deaths = models.PositiveIntegerField(
        default=0, help_text="Number of neonatal deaths"
    )

    # Birth weights (JSON array for multiples)
    birth_weights = models.JSONField(
        default=list, blank=True, help_text="Birth weights in grams [3200, 2800]"
    )

    # Complications
    maternal_complications = models.TextField(
        blank=True, help_text="Maternal complications during pregnancy/delivery"
    )

    neonatal_complications = models.TextField(
        blank=True, help_text="Neonatal complications"
    )
    ...
    # Audit
    recorded_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="ivf_outcomes_recorded"
    )
...
class IVFConsent(models.Model):
    ...
    cycle = models.ForeignKey(
        IVFCycle, on_delete=models.CASCADE, related_name="consents"
    )

    consent_type = models.CharField(
        max_length=30, choices=ConsentType.choices, help_text="Type of consent"
    )

    patient = models.ForeignKey(
        "patients.Patient", on_delete=models.PROTECT, related_name="ivf_consents"
    )

    # Consent Status
    signed = models.BooleanField(default=False, help_text="Has been signed")

    signed_date = models.DateField(null=True, blank=True, help_text="Date signed")

    signed_time = models.TimeField(null=True, blank=True, help_text="Time signed")

    # Witness Information
    witness_name = models.CharField(
        max_length=100, blank=True, help_text="Witness name"
    )
    ...
    revoked_date = models.DateField(
        null=True, blank=True, help_text="Date consent was revoked"
    )
    ...
    # Audit
    recorded_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="ivf_consents_recorded"
    )
