...
def send_appointment_reminder(appointment):
    ...
    if patient.phone:
        try:
            from apps.notifications.nigeria_comms import send_appointment_reminder_sms

            send_appointment_reminder_sms(appointment, created_by=appointment.created_by)
        except Exception:
            pass
...
def send_lab_result_notification(lab_result):
    ...
    # Only send email if patient has email; always attempt SMS when phone present
    ...
    if patient.phone:
        try:
            from apps.notifications.nigeria_comms import send_lab_result_ready_sms

            send_lab_result_ready_sms(
                patient=patient, visit=visit, created_by=None
            )
        except Exception:
            pass
...
def send_patient_verification_notification(patient, verified_by_user):
    ...
    context = {
        ...
        "verified_date": patient.verified_at.strftime("%B %d, %Y at %I:%M %p")
        if patient.verified_at
        else timezone.now().strftime("%B %d, %Y at %I:%M %p"),
