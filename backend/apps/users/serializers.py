...
from apps.organizations.utils import get_default_clinic_organization
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate_role(self, value):
        """Validate role is allowed for public self-registration."""
        ...
        if not settings.PUBLIC_REGISTRATION_ENABLED:
            raise serializers.ValidationError("Public registration is disabled.")

        allowed_roles = settings.PUBLIC_REGISTRATION_ALLOWED_ROLES
        if value not in allowed_roles:
            raise serializers.ValidationError(
                "This role cannot be registered publicly. "
                "Contact your clinic administrator for staff access."
            )
    ...
    def create(self, validated_data):
        ...
        if role == "PATIENT":
            ...
            org = get_default_clinic_organization()
            ...
            org = get_default_clinic_organization()
            ensure_staff_organization_membership(user, org, role)
