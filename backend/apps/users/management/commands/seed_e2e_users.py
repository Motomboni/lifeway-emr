...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for spec in E2E_USERS:
            ...
            if spec.get("portal"):
                self._ensure_portal_patient(org, user, spec)
    ...
    def _ensure_portal_patient(self, org, user, spec):
        from apps.patients.models import Patient

        patient = Patient.objects.filter(user=user).first()
        if not patient:
            patient = Patient.objects.filter(email=spec["email"], organization=org).first()
        if not patient:
            patient_id = Patient.generate_patient_id(organization=org)
            ...
            patient.user = user
            patient.email = spec["email"]
            patient.save(
                update_fields=["is_verified", "portal_enabled", "user", "email"]
            )

        user.patient = patient
        user.portal_enabled = True
        user.save(update_fields=["patient", "portal_enabled"])
        self.stdout.write(
            self.style.SUCCESS(f"  Portal patient linked: {spec['email']}")
        )
