...
class User(AbstractUser):
    ...
    last_login_device = models.CharField(
        max_length=255, blank=True, help_text="Last device used for login"
    )
    ...
    last_login_attempt = models.DateTimeField(
        null=True, blank=True, help_text="Timestamp of last login attempt"
    )

    # Biometric authentication (device secure enclave; no raw biometric data stored)
    biometric_enabled = models.BooleanField(
        default=False, help_text="Whether biometric login is enabled for this user"
    )
    ...
    def clean(self):
        ...
        if hasattr(self, "role") and self.role:
            ...
            if self.role not in valid_roles:
                raise ValidationError(
                    f"Invalid role: {self.role}. Valid roles are: {', '.join(valid_roles)}"
                )
        ...
        if self.role != "PATIENT" and self.patient:
            raise ValidationError(
                "Only PATIENT role users can be linked to a patient record"
            )
