...
class LabResultCreateSerializer(serializers.ModelSerializer):
    ...
    lab_order = serializers.PrimaryKeyRelatedField(
        queryset=LabOrder.objects.all(), help_text="Lab order this result belongs to"
    )
    ...
    def validate_result_data(self, value):
        ...
        if not value or not value.strip():
            raise serializers.ValidationError("Result data is required.")
        return value
    ...
    def create(self, validated_data):
        ...
        lab_result = LabResult.objects.create(
            recorded_by=request.user, **validated_data
        )
