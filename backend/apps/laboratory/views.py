...
def log_lab_order_action(
    user, action, visit_id, lab_order_id=None, request=None, metadata=None
):
    """
...
class LabOrderViewSet(viewsets.ModelViewSet):
    ...
    def check_payment_status(self, visit):
        """Registration must be paid; post-consult lab charges are collected later."""
        from apps.billing.payment_gates_service import is_registration_paid

        if not is_registration_paid(visit):
            raise PermissionDenied(
                detail="Registration payment is required before creating lab orders. "
