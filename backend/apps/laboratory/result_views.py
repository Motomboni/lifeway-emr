...
from .permissions import CanUpdateLabResult
...
class LabResultViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return LabResult.objects.filter(lab_order_id__in=lab_order_ids).select_related(
            "lab_order", "recorded_by"
        )
    ...
    def check_payment_status(self, visit):
        """Registration must be paid; lab service fees are collected post-consultation."""
        from apps.billing.payment_gates_service import is_registration_paid

        if not is_registration_paid(visit):
            raise PermissionDenied(
                detail="Registration payment is required before lab results. "
    ...
    def perform_create(self, serializer):
        ...
        from apps.clinical.alert_service import evaluate_lab_result_alerts

        evaluate_lab_result_alerts(visit, lab_result)
