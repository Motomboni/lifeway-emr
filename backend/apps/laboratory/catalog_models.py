...
class LabTestCatalog(models.Model):
    ...
    test_name = models.CharField(max_length=200, help_text="Full name of the lab test")

    category = models.CharField(
        max_length=50, choices=CATEGORY_CHOICES, help_text="Category of the lab test"
    )

    description = models.TextField(
        blank=True, help_text="Description of what the test measures"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether this test is currently available for ordering"
    )

    requires_fasting = models.BooleanField(
        default=False, help_text="Whether patient must fast before this test"
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="When the test was added to the catalog"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the test was last updated"
    )
    ...
    def clean(self):
        ...
        if (
            self.reference_range_min is not None
            and self.reference_range_max is not None
        ):
