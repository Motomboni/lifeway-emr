...
class AntenatalRecord(models.Model):
    ...
    pregnancy_number = models.PositiveIntegerField(
        default=1, help_text="Sequential pregnancy number for this patient"
    )
    ...
    gravida = models.PositiveIntegerField(
        default=1, help_text="Number of pregnancies (including current)"
    )

    para = models.PositiveIntegerField(
        default=0, help_text="Number of deliveries at 37+ weeks"
    )

    abortions = models.PositiveIntegerField(
        default=0, help_text="Number of abortions/miscarriages"
    )

    living_children = models.PositiveIntegerField(
        default=0, help_text="Number of living children"
    )

    # Medical history
    past_medical_history = models.TextField(
        blank=True, help_text="Past medical history relevant to pregnancy"
    )

    past_surgical_history = models.TextField(
        blank=True, help_text="Past surgical history"
    )

    family_history = models.TextField(
        blank=True, help_text="Family history relevant to pregnancy"
    )
    ...
    previous_cs = models.BooleanField(
        default=False, help_text="Previous Cesarean Section"
    )

    previous_cs_count = models.PositiveIntegerField(
        default=0, help_text="Number of previous Cesarean Sections"
    )

    previous_complications = models.TextField(
        blank=True, help_text="Previous pregnancy complications"
    )
    ...
    delivery_date = models.DateField(
        null=True, blank=True, help_text="Date of delivery (if delivered)"
    )
    ...
    delivery_gestational_age_days = models.PositiveIntegerField(
        null=True, blank=True, help_text="Gestational age at delivery (days)"
    )
    ...
    def clean(self):
        ...
        if self.patient.gender != "FEMALE":
            raise ValidationError(
                "Antenatal records can only be created for female patients."
            )
        ...
        if self.edd and self.lmp:
            ...
            if abs((self.edd - calculated_edd).days) > 14:  # Allow 2 weeks variance
                raise ValidationError(
                    "EDD should be approximately 40 weeks (280 days) from LMP."
                )
...
class AntenatalVisit(models.Model):
    ...
    gestational_age_days = models.PositiveIntegerField(
        default=0, help_text="Gestational age at visit (days)"
    )

    # Chief complaint
    chief_complaint = models.TextField(
        blank=True, help_text="Chief complaint for this visit"
    )
    ...
    weight = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True, help_text="Weight (kg)"
    )
    ...
    clinical_notes = models.TextField(
        blank=True, help_text="Clinical notes for this visit"
    )

    # Next appointment
    next_appointment_date = models.DateField(
        null=True, blank=True, help_text="Next scheduled appointment date"
    )
...
class AntenatalUltrasound(models.Model):
    ...
    gestational_age_days = models.PositiveIntegerField(
        default=0, help_text="Gestational age at scan (days)"
    )
    ...
    number_of_fetuses = models.PositiveIntegerField(
        default=1, help_text="Number of fetuses"
    )
    ...
    placenta_location = models.CharField(
        max_length=50, blank=True, help_text="Placenta location"
    )
...
class AntenatalLab(models.Model):
    ...
    test_name = models.CharField(max_length=255, help_text="Name of lab test")
    ...
    results = models.TextField(
        blank=True, help_text="Test results (free text or structured)"
    )
...
class AntenatalMedication(models.Model):
    ...
    dose = models.CharField(
        max_length=100, help_text="Dose (e.g., '200mg', '1 tablet')"
    )

    frequency = models.CharField(
        max_length=100, help_text="Frequency (e.g., 'Once daily', 'Twice daily')"
    )
    ...
    end_date = models.DateField(
        null=True, blank=True, help_text="End date (if applicable)"
    )
...
class AntenatalOutcome(models.Model):
    ...
    delivery_gestational_age_days = models.PositiveIntegerField(
        default=0, help_text="Gestational age at delivery (days)"
    )

    # Birth information
    number_of_babies = models.PositiveIntegerField(
        default=1, help_text="Number of babies delivered"
    )

    live_births = models.PositiveIntegerField(
        default=1, help_text="Number of live births"
    )
    ...
    maternal_complications = models.TextField(
        blank=True, help_text="Maternal complications"
    )
    ...
    notes = models.TextField(
        blank=True, help_text="Additional notes about delivery and outcome"
    )
