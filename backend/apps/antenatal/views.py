...
from .permissions import (
    ...
    CanManageAntenatalRecords,
    CanRecordAntenatalVisits,
)
from .schedule_service import build_anc_schedule, format_anc_whatsapp_reminder
from .serializers import (
    AntenatalLabCreateSerializer,
    AntenatalLabSerializer,
    AntenatalMedicationCreateSerializer,
    AntenatalMedicationSerializer,
    AntenatalOutcomeCreateSerializer,
    AntenatalOutcomeSerializer,
    AntenatalRecordCreateSerializer,
    AntenatalRecordDetailSerializer,
    AntenatalRecordListSerializer,
    AntenatalRecordUpdateSerializer,
    AntenatalUltrasoundCreateSerializer,
    AntenatalUltrasoundSerializer,
    AntenatalVisitCreateSerializer,
...
class AntenatalRecordViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = AntenatalRecord.objects.select_related(
            "patient", "created_by"
        ).prefetch_related("visits", "delivery_outcome")
    ...
    def visits(self, request, pk=None):
        ...
        visits = AntenatalVisit.objects.filter(antenatal_record=record).order_by(
            "-visit_date"
        )
    ...
    def schedule(self, request, pk=None):
        """WHO/Nigeria ANC schedule — IPTp, TT, routine visits, danger signs."""
        record = self.get_object()
        return Response(build_anc_schedule(record))

    @action(detail=True, methods=["post"])
    def send_schedule_reminder(self, request, pk=None):
        """Send WhatsApp reminder for next due/overdue ANC schedule item."""
        ...
        target = schedule.get("overdue") or schedule.get("upcoming") or []
        if not target:
            return Response({"detail": "No due ANC items to remind."}, status=400)
        item = target[0]
        message = format_anc_whatsapp_reminder(record, item)
        ok = send_whatsapp_message(patient.phone, message)
        if not ok:
            return Response({"detail": "Failed to send WhatsApp message."}, status=502)
        return Response({"detail": "ANC reminder sent.", "item": item})
...
class AntenatalVisitViewSet(viewsets.ModelViewSet):
    ...
    def ultrasounds(self, request, pk=None):
        ...
        ultrasounds = AntenatalUltrasound.objects.filter(
            antenatal_visit=visit
        ).order_by("-scan_date")
    ...
    def medications(self, request, pk=None):
        ...
        visit = self.get_object()
        medications = AntenatalMedication.objects.filter(
            antenatal_visit=visit
        ).order_by("-start_date")
...
class AntenatalLabViewSet(viewsets.ModelViewSet):
    ...
    queryset = AntenatalLab.objects.select_related("antenatal_visit", "ordered_by")
