...
/** Backend POST /patients/ returns { patient: {...} }; unwrap for callers. */
export function normalizePatientPayload(
  response: Patient | CreatePatientResponse,
): Patient {
  if (
    response &&
...
export async function createPatient(patientData: PatientCreateData): Promise<Patient> {
  const response = await apiRequest<Patient | CreatePatientResponse>('/patients/', {
    method: 'POST',
...
/** Verify National Health ID / NIN against configured provider */
export async function verifyNationalHealthId(data: {
  patient_id: number;
  id_number: string;
  name: string;
  dob: string;
}): Promise<{ detail: string; id_verified: boolean }> {
  return apiRequest<{ detail: string; id_verified: boolean }>('/nhid/verify/', {
    method: 'POST',
...
/**
 * Archive (soft-delete) patient, or permanently delete if caller is superuser.
 */
export async function archivePatient(patientId: number): Promise<{ detail: string } | void> {
  return apiRequest<{ detail: string } | void>(`/patients/${patientId}/`, {
    method: 'DELETE',
  });
}
