...
export default function CreateVisitPage() {
  ...
  useEffect(() => {
    const rawPatientId = searchParams.get('patient');
    if (!rawPatientId) {
      return;
    }
    const patientId = parseInt(rawPatientId, 10);
    if (Number.isNaN(patientId)) {
      showError('Invalid patient link. Search for the patient below.');
      return;
    }
  ...
  const handleCreateVisit = async () => {
    ...
    try {
      if (isOffline) {
        await queueOfflineAction('CHECK_IN_VISIT', {
        ...
        showSuccess('Visit check-in queued — will sync when you are back online');
        navigate('/dashboard');
        return;
      }
            ...
            <div className={styles.visitOptions}>
              ...
              <div className={styles.formGroup}>
                ...
                {selectedPatient.has_active_insurance && paymentType === 'INSURANCE' && (
                  <p className={`${styles.helpText} ${styles.hintSuccess}`}>
                ...
                {selectedPatient.has_active_insurance && paymentType === 'CASH' && (
                  <p className={`${styles.helpText} ${styles.hintWarning}`}>
