...
export function canManageLabCatalog(user: User | null | undefined): boolean {
  return isSuperuser(user) || hasRole(user, ['DOCTOR', 'LAB_TECH', 'ADMIN']);
}
...
export function canManageRadiologyCatalog(user: User | null | undefined): boolean {
  return isSuperuser(user) || hasRole(user, ['DOCTOR', 'RADIOLOGY_TECH', 'ADMIN']);
}
...
/** Django superuser — may permanently delete records (test-data cleanup). */
export function canHardDeleteRecords(user: User | null | undefined): boolean {
  return isSuperuser(user);
}

/** Receptionists handle billing/front desk — not clinical consultation or admission charts. */
export function canViewConsultationAndAdmissionRecords(user: User | null | undefined): boolean {
